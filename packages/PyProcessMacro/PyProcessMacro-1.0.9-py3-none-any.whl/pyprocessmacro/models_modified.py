            ...
            if "t" in results.keys():  # Model has t-stats rather than z-stats
                coeffs = np.array(
                    [
                        ...
                        results["se"],
                        results["t"],
                        np.array(results["p"][:len(results["betas"])]),
                        results["llci"],
                        results["ulci"],
                    ]
                ).T
...
class OLSOutcomeModel(BaseOutcomeModel):
    ...
    def _estimate(self):
        ...
        n_obs = self._n_obs
        n_vars = self._n_vars
        inv_xx = np.linalg.inv(np.matmul(x.T, x))
        #xy = np.matmul(x.T, y)
        betas = np.matmul(np.matmul(inv_xx, x.T), y)
        df_e = n_obs - n_vars
        df_r = n_vars - 1
        #print(x.shape, betas.shape) #####
        temp_ = np.matmul(x, betas.T)
        resid = y - temp_
        ...
        betas = betas.squeeze()
        #print(vcv)  #####
        temp1 = np.diagonal(vcv)
        temp1 = np.array(temp1, dtype = np.float64)
        #print(temp1) #####
        se = np.sqrt(temp1).squeeze()
        t = betas / se
        temp2 = np.array(np.abs(t), dtype = np.float64)
        #print(temp2, df_e)  #####
        #print(temp2.dtype)  #####
        p = stats.t.sf(temp2, df_e) * 2  #p = [1, 1, 1]
...
class ParallelMediationModel(object):
    ...
    def _estimate_true_params(self):
        ...
        #print("_ind_y: ", self._ind_y)
        #print("_exog_inds_y: ", self._exog_inds_y)
        #print("endog_y: ",endog_y)  #####
        #print("exog_y: ", exog_y)  #####
