...
def scatter_sta_list(sta0_list,map_extend = None,add_county_line = False,add_worldmap = False,
                ...
                fix_size = True,threshold = None,mean_value = None,
                print_max = 0,print_min = 0,save_dir = None,
                save_path=None,show = False,dpi = 300,sub_title_list=None,title = None,
                sup_fontsize = 10,
                height = None,width = None,
                best_value = 0,grid = False,subplot = None,ncol = None):
    ...
    height_hspace_rate = 0.08

    width_wspace_rate = 0.08
    width_colorbar = 0.5
    ...
    if width is None:
        ...
        height_map = height_all_plot / (nrow + (nrow -1)*height_hspace_rate)
        width_map = height_map * rlon / rlat
        width_all_plot = width_map * (ncol + (ncol-1) * width_wspace_rate)
        width = width_all_plot + width_colorbar + width_left_yticks
    else:
        width_all_plot = width - width_colorbar - width_left_yticks
        width_map = width_all_plot / (ncol + (ncol -1)*width_wspace_rate)
        height_map = width_map * rlat / rlon
        height_all_plot = height_map * (nrow + (nrow-1) * height_hspace_rate)
    ...
    for p in range(nplot):
        ...
        rect1 = [(width_left_yticks + pi * width_map * (1 + width_wspace_rate))/width,
                 (height_bottem_xticsk + (nrow -1- pj) * height_map * (1 + height_hspace_rate))/height,
                 width_map / width,
                 height_map / height]
        ax = plt.axes(rect1)
        if sub_title_list is None:
            try:
                ...
                dati_str = time_str[0:4] + "年" + time_str[4:6] + "月" + time_str[6:8] + "日" + time_str[8:10] + "时"
                title1 = data_name + " " + dati_str + str(sta_one_member.iloc[0,2]) + "H时效 "
            except:
                print("time or dtime or level 格式错误，请更改相应数据格式或直接指定title")
                title1= ""
        else:
            #title1 = title.replace("NNN",data_name)
            title1 = sub_title_list[p]
    ...
    left_low = (width_left_yticks + ncol * width_map * (1 + width_wspace_rate))/width
    print([left_low, height_bottem_xticsk / height,0.02, height_all_plot/height])
