...
class FS(object):
    ...
    def __init__(self, sorted_feats_by_feature_importances, max_features=100, early_stopping_steps=10, params=None,
                 cv=5, cv_seed=777):
    ...
    def fit(self, train_df, y, test_df=None):
        ...
        score = 0
        self.scores = []
        tqdm_ = tqdm(enumerate(self.sorted_feats))
        for i, feat in tqdm_:
            ...
            if self.clf.oof_score > score:
                ...
                step += 1
                if (step == self.early_stopping_steps or
                        len(self.selected_features_) == self.max_features):  # 累计n个特征未提升终止特征选择
                    break
