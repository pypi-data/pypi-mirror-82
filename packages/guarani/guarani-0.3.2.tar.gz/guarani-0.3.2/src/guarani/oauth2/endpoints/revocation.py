...
from guarani.oauth2.exceptions import InvalidClient, OAuth2Error, UnsupportedTokenType
...
class RevocationEndpoint(BaseEndpoint):
    ...
    async def __call__(self, request: OAuth2Request) -> OAuth2JSONResponse:
        ...
        try:
            ...
            await self.revoke_token(client, token, token_type_hint)

            return OAuth2JSONResponse()
        except InvalidClient as exc:
            return OAuth2JSONResponse(401, exc.headers, exc.dump())
        except OAuth2Error as exc:
            return OAuth2JSONResponse(400, exc.headers, exc.dump())
