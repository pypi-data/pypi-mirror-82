...
from guarani.oauth2.exceptions import FatalError, InvalidGrant, InvalidRequest
...
class AuthorizationCodeGrant(BaseGrant):
    ...
    async def authorize(self, data: dict, client: ClientMixin, user: Any) -> str:
        ...
        data = self.validate_authorization_request(data)

        self.validate_requested_scopes(data["scopes"], client, data.get("state"))

        code = await self.generate_authorization_code()

        await self.save_authorization_code(code, data, client, user)

        return self.create_authorization_response(
            data["redirect_uri"], {"code": code, "state": data.get("state")}
        )

    def validate_authorization_request(self, data: dict) -> dict:
        """
        ...
        if data.get("response_type") != "code":
            raise InvalidRequest(
                description='The response type MUST be "code".', state=data.get("state")
            )

        if not data.get("client_id") or not isinstance(data.get("client_id"), str):
            raise FatalError(
                description='Invalid parameter "client_id".', state=data.get("state")
            )
        ...
        if not data.get("code_challenge") or not isinstance(
            ...
            raise InvalidRequest(
                description="Invalid code challenge.", state=data.get("state")
            )
        ...
        if not data.get("redirect_uri") or not isinstance(
            ...
            raise FatalError(
                description='Invalid parameter "redirect_uri".', state=data.get("state")
            )

        if not data.get("scope") or not isinstance(data.get("scope"), str):
            raise InvalidRequest(
                description='Invalid parameter "scope".', state=data.get("state")
            )
    ...
    async def token(self, data: dict, client: ClientMixin) -> dict:
        ...
        try:
            ...
            async for response in self.run_hook("post_token", token, client, user):
    ...
    def create_authorization_response(self, url: str, data: dict) -> str:
        """
    ...
    async def save_authorization_code(
        self, code: str, data: dict, client: ClientMixin, user: Any
    ) -> None:
        """
