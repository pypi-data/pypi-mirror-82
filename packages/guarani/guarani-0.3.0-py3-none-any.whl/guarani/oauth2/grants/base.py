...
from guarani.oauth2.exceptions import InvalidScope
...
class BaseGrant:
    ...
    async def run_hook(self, name: str, /, *args: Any) -> Any:
        for extension in self.extensions:
            if hook := getattr(extension, name):
                yield await hook(*args)
    ...
    async def authorize(self, data: dict, client: ClientMixin, user: Any) -> str:
        """
