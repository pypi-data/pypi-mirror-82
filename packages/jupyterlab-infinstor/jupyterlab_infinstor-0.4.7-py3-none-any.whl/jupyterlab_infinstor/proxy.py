...
class ProxyHandler(WebSocketHandlerMixin, IPythonHandler):
    ...
    async def proxy(self, host, port, proxied_path):
        ...
        if (proxied_path == '/' or proxied_path.startswith('/static-files')):
            ...
            host = new_host
            rest_call = True
        elif (proxied_path.startswith('/model-versions/get-artifact')):
            new_host = 'mlflow.' + builtins.service + '.com'
            new_proxied_path = '/Prod/2.0/mlflow/model-versions/get-artifact' + proxied_path[len('/model-versions/get-artifact'):]
            print('Proxying REST. http://' + str(host) + ':' + str(port) + str(proxied_path) + ' -> https://' + str(host) + str(new_proxied_path));
            proxied_path = new_proxied_path
            host = new_host
            rest_call = True
