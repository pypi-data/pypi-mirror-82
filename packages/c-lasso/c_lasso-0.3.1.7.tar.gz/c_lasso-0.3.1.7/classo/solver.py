...
class solution_PATH:
    ...
    def __repr__(self):
        ...
        if len(self.BETAS[0])>100: 
            top = np.argpartition(np.mean(abs(self.BETAS),axis=0), -100)[-100:]
        else : 
...
class solution_CV:
    ...
    def __repr__(self):
        ...
        string = "\n CROSS VALIDATION : "
        if len(self.refit)>100: 
            top = np.argpartition(abs(self.refit), -100)[-100:]
        else : 
            top = np.arange(len(self.refit))
...
class solution_StabSel:
    ...
    def __repr__(self):
        ...
        if len(self.distribution)>100: 
            top = np.argpartition(self.distribution, -100)[-100:]
        else : 
            top = np.arange(len(self.distribution))
...
class solution_LAMfixed:
    ...
    def __repr__(self):
        ...
        if len(self.beta)>100: 
            top = np.argpartition(abs(self.beta), -100)[-100:]
        else : 
            top = np.arange(len(self.beta))
