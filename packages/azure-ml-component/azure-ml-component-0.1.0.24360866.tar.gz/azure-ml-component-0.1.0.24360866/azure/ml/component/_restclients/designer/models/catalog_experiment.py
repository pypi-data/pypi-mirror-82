# coding=utf-8
# --------------------------------------------------------------------------
# Code generated by Microsoft (R) AutoRest Code Generator.
# Changes may cause incorrect behavior and will be lost if the code is
# regenerated.
# --------------------------------------------------------------------------

from msrest.serialization import Model


class CatalogExperiment(Model):
    """CatalogExperiment.

    :param id:
    :type id: str
    :param author:
    :type author: ~designer.models.Person
    :param name:
    :type name: str
    :param summary:
    :type summary: str
    :param hidden:
    :type hidden: bool
    :param owner:
    :type owner: ~designer.models.Owner
    :param tags:
    :type tags: list[str]
    :param image_url:
    :type image_url: str
    :param content:
    :type content: ~designer.models.Storage
    :param algorithms:
    :type algorithms: list[str]
    :param slugs:
    :type slugs: list[str]
    :param etag:
    :type etag: str
    """

    _attribute_map = {
        'id': {'key': 'id', 'type': 'str'},
        'author': {'key': 'author', 'type': 'Person'},
        'name': {'key': 'name', 'type': 'str'},
        'summary': {'key': 'summary', 'type': 'str'},
        'hidden': {'key': 'hidden', 'type': 'bool'},
        'owner': {'key': 'owner', 'type': 'Owner'},
        'tags': {'key': 'tags', 'type': '[str]'},
        'image_url': {'key': 'image_url', 'type': 'str'},
        'content': {'key': 'content', 'type': 'Storage'},
        'algorithms': {'key': 'algorithms', 'type': '[str]'},
        'slugs': {'key': 'slugs', 'type': '[str]'},
        'etag': {'key': 'etag', 'type': 'str'},
    }

    def __init__(self, **kwargs):
        super(CatalogExperiment, self).__init__(**kwargs)
        self.id = kwargs.get('id', None)
        self.author = kwargs.get('author', None)
        self.name = kwargs.get('name', None)
        self.summary = kwargs.get('summary', None)
        self.hidden = kwargs.get('hidden', None)
        self.owner = kwargs.get('owner', None)
        self.tags = kwargs.get('tags', None)
        self.image_url = kwargs.get('image_url', None)
        self.content = kwargs.get('content', None)
        self.algorithms = kwargs.get('algorithms', None)
        self.slugs = kwargs.get('slugs', None)
        self.etag = kwargs.get('etag', None)
