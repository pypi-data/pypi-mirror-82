...
from .core._run_settings_definition import RunSettingParam
from ._run_settings import _RunSettings
...
class ModuleValidator(object):
    ...
    @staticmethod
    def validate_runsettings(runsettings: _RunSettings, process_error):
        ...
        reserved_param_names = [
            '_target', 'configure', '_workspace', '_params_spec', '__doc__', '_owner']
    ...
    def validate_k8srunsettings(k8srunsettings, process_error):
        ...
        for section_name in k8srunsettings._params_spec:
            ...
            for param_name in section_settings._params_spec:
                ...
                ModuleValidator._validate_runsetting_parameter(f"k8srunsettings.{section_name}.{param_name}",
                                                               param_value,
                                                               param_spec,
                                                               process_error)
            # Raise warnings for parameters not in spec
        ...
        if spec.parameter_type == RunSettingParameterType.json_string and not isinstance(param_value, str):
            ...
            if dumped_json_string:
                ...
                if json_schema_type is not None:
                    ...
                    if json_schema_type == JsonSchemaType.object:
                        if not isinstance(param_value, dict):
                            process_error(ValueError(f"Parameter type mismatched '{param_hint_name}', expected type: "
                                                     f"'dict', actually '{type(param_value).__name__}'."),
                    ...
                    # List json type validation
                        ...
                        if not (isinstance(param_value, list) or isinstance(param_value, tuple)):
                            process_error(ValueError(f"Parameter type mismatched '{param_hint_name}', expected type: "
                                                     f"'list' or 'tuple', actually '{type(param_value).__name__}'."),
                                          ValidationError.INVALID_RUNSETTING_PARAMETER)
                ...
                if json_schema_type is not None:
                    ...
                    process_error(ValueError(f"Parameter '{param_hint_name}' with type '{type(param_value).__name__}' "
                                             f"is invalid, which is not JSON serializable, expected type: "
                                             f"{expected_type} object or JSON string"),
                                  ValidationError.INVALID_RUNSETTING_PARAMETER)
                else:
                    # json_schema type is not given
                    process_error(ValueError(f"Parameter '{param_hint_name}' with type '{type(param_value).__name__}' "
                                             f"is invalid, which is not JSON serializable, expected type: "
                                             f"serializable JSON object or JSON string"),
                                  ValidationError.INVALID_RUNSETTING_PARAMETER)

        if not isinstance(param_value, spec.parameter_type_in_py):
            process_error(ValueError(f"Parameter type mismatched '{param_hint_name}', expected type: "
                                     f"'{spec.parameter_type_in_py.__name__}', "
                                     f"actually '{type(param_value).__name__}'."),
                          ValidationError.INVALID_RUNSETTING_PARAMETER)
