...
from azure.ml.component._module_run_helper import _download_snapshot, _extract_zip, \
    _mock_parallel_driver_file, MOCK_PARALLEL_DRIVER, translate_parallel_command
from azure.ml.component._restclients.service_caller import DesignerServiceCaller
from azure.ml.component.debug._constants import VSCODE_DIR, INPUT_DIR, LAUNCH_CONFIG, CONTAINER_DIR, \
...
from azure.ml.component.debug._image import ImageBase
...
class DebugOnlineStepRunHelper(DebugStepRunHelper):
    ...
    def prepare_dev_container(workspace, step, dry_run=False) -> ImageBase:
        ...
        if dry_run:
            ...
            image_name = step_run_image.get_module_image()
    ...
    @staticmethod
    def download_snapshot(service_caller: DesignerServiceCaller, step_run: Run, module_type: str, dry_run=False):
        if dry_run:
            return True
        ...
        try:
            ...
            if module_type and module_type.lower() == 'parallel':
    ...
    @staticmethod
    def prepare_arguments(step_name, details, port_arg_map, module_type):
        if 'runDefinition' not in details:
            raise RuntimeError('Failed to get runDefinition from step run details, '
                               'please make sure this step run has started successfully.')
