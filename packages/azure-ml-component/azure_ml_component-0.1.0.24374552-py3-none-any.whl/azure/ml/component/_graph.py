...
from .component import Component, _InputBuilder, _OutputBuilder
    ...
    def get_dependencies(node, visted):
        ...
        for input in node.inputs.values():
            if isinstance(input, _InputBuilder):
                ...
                elif isinstance(dset, Component) or isinstance(dset, PipelineComponent):
...
class _GraphEntityBuilder(object):
    ...
    def build_graph_entity(self, is_local_run=False):
        ...
        :param remove_empty_nodes: whether to remove data node that refer to parameter without value
        :type remove_empty_nodes: bool
    ...
    def _produce_module_runsettings(self, module: Component, module_node: GraphModuleNode):
        ...
        module_node_run_setting.step_type = module._module_dto.module_entity.step_type
    ...
    def _update_module_node_params(self, graph_entity: GraphDraftEntity, module_node: GraphModuleNode,
                                   module: Component, inputs_map, outputs_map, pipeline_parameters):
    ...
    def _update_data_path_parameter_list(self, graph_entity: GraphDraftEntity, pipeline_parameters):
        ...
        for name, pipeline_parameter in self._data_path_parameter_input.items():
            ...
            if isinstance(dset, _GlobalDataset):
                dataset_def = self._get_dataset_def_from_dataset(dset)
