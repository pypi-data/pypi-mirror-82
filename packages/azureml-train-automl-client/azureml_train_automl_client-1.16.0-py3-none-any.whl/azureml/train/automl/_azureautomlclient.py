...
class AzureAutoMLClient:
    ...
    def _get_task(self, previous_fit_outputs, is_sparse=None):
        ...
        """
        return self._get_tasks(previous_fit_outputs, 1, is_sparse)[0]

    def _get_tasks(self, previous_fit_outputs, max_batch_size, is_sparse):
        """
        ...
        with logging_utilities.log_activity(logger=logger, activity_name=TelemetryConstants.GET_PIPELINE_NAME):
            try:
                ...
                return self._jasmine_client.get_next_task_batch(
                    self.parent_run_id,
                    max_batch_size,
                    local_run_get_next_task_batch_input).iteration_tasks
            except (AzureMLException, AzureMLServiceException):
                raise
