...
class Window1(BaseWindow):
    def CreateWidgets(self):
        self.button = Widgets.Button(master=self, Text="button 1").SetCommand(self.exit).Place(relx=0.0, rely=0.0, relheight=1.0, relwidth=1.0).SetImage(data=PhotoData['exit'])


class Window2(BaseWindow):
    def CreateWidgets(self):
        self.button = Widgets.Button(master=self, Text="button 2").SetCommand(self.exit).Place(relx=0.0, rely=0.0, relheight=1.0, relwidth=1.0).SetImage(data=PhotoData['exit'])


class Window3(BaseWindow):
    nested: Window2
    def CreateWidgets(self):
        self.button = Widgets.Button(master=self, Text="button 3").SetCommand(self.exit).Place(relx=0.0, rely=0.0, relheight=1.0, relwidth=0.5).SetImage(data=PhotoData['exit'])
