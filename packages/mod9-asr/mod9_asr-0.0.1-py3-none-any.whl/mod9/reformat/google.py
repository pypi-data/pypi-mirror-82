...
class GoogleConfigurationSettingsAndMappings:
    """
    Hold dict's used to translate keys and values from Google to Mod9 forms.

    Internally use camelCase for google_allowed_keys.
    We chose to use camelCase internally for compatibility with
    Google's protobuf json_format funtions
    (which are called by to_json() and from_json()).
    We need to make sure we use the proper casing, both internally
    and for output, or can run into non-obvious bugs.
    These bugs can be non-obvious because many of the primary attributes
    are one-word attributes (like `.transcript`) and so functionality will
    only break if it is dependent on multi-word attributes.

    Input from REST API come in camelCase.
    Input from Python SDK come in snake_case, are converted to camelCase using
    either from_json() (in the case of Google protobuf object input), or
    utils.recursively_convert_dict_keys_case() and utils.snake_case_to_camel_case()
    (in the case of dict input).
    Output from REST API use camelCase.
    Output from Python SDK converted to snake_case in to_json().
    """
    def __init__(self):
        """Define dict's used to translate keys and values from Google to Mod9 forms."""
        ...
        self.mod9_allowed_keys = (
            ...
            'transcript-alternatives-max',
            'phrase-alternatives-max',   # only in Mod9
        )
...
def input_to_mod9(google_input_settings, module):
    """Wrapper method to take Google inputs of various types and return Mod9-compatible inputs."""
    ...
    if 'format' in mod9_config_settings and mod9_config_settings['format'] is None:
        ...
        wav_encoding = utils.parse_wav_encoding(mod9_audio_settings)
        if wav_encoding:
...
def google_config_settings_to_mod9(google_config_settings):
    """Map from Google-style key:value inputs to Mod9 ASR TCP server-style key:value inputs."""
    ...
    # `to_json()` populates absent attributes of the config with falsy values; remove falsy values.
    ...
    mod9_config_settings['transcript-alternatives'] = True
    n_best_N = mod9_config_settings.get('transcript-alternatives-max', 1)
    # Google sets n_best_N: 0 -> 1.
    mod9_config_settings['transcript-alternatives-max'] = n_best_N if n_best_N > 0 else 1

    # This option only applies with speech_mod9.
    if mod9_config_settings.get('phrase-alternatives-max'):
        mod9_config_settings['phrase-alternatives'] = True
...
def google_audio_settings_to_mod9(google_audio_settings):
    """Map from Google-style audio input to Mod9 TCP server-style audio input."""
...
def result_from_mod9(mod9_results):
    ...
    for mod9_result in mod9_results:
        ...
        if not mod9_result['final']:
            google_result['stability'] = 0.5  # rather arbitrary
