...
from parallelsdk.scheduling_toolbox import scheduling_problem
from parallelsdk.scheduling_toolbox import employees_scheduling_problem
from parallelsdk.scheduling_toolbox import job_shop_problem
...
class OptimizerSchedulingModel(parallel_model.ParallelModel):
    ...
    def __init__(self, name, model_type):
        ...
        elif self.model_type is scheduling_problem.SchedulingModelType.JOB_SHOP:
            self.scheduling_model = job_shop_problem.JobShopProblem(self.model_name)
        else:
            err_msg = "OptimizerSchedulingModel - unrecognized scheduling model"
            logging.error(err_msg)
            raise Exception(err_msg)
    ...
    def on_message_impl(self, optilab_reply_message):
        if optilab_reply_message.details.Is(scheduling_model_pb2.SchedulingSolutionProto.DESCRIPTOR):
            ...
            err_msg = "OptimizerSchedulingModel - received an unrecognized back-end message"
            logging.error(err_msg)
            print(err_msg)
