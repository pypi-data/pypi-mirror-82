...
class AutoBagFixer(Watcher):
    def __init__(self, cfg):
        ...
        self.bag_channel = Channel()
        self.file_channel = Channel()
        self.fixer_thread = threading.Thread(target=self.thread_fixer)
        self.move_thread = threading.Thread(target=self.thread_mover)
    ...
    def task_file_moved(self, event):
        ...
        if '.bag' in str(file_out):
            self.bag_channel.put([file_in, file_out])
        else:
            self.file_channel.put([file_in, file_out])

    def thread_fixer(self):
        while not self.bag_channel.get_finish():
            bag = self.bag_channel.get()
            self.log.debug(f'Bag to fix {self.cyan(bag[1])}')
            bf = BagFixer(bag[0], bag[1])
            bf.start()
            bf.join()

    def thread_mover(self):
        while not self.file_channel.get_finish():
            file = self.file_channel.get()
            self.log.debug(f'File moved: {self.cyan(file[1])}')
            move(file[0], file[1])

    def __del__(self):
        self.bag_channel.set_finish()
        self.file_channel.set_finish()
