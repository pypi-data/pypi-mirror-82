...
class loadprojectini:
    def __init__(self,configini):
        ...
        self.label_settingsini = FileSelect(label_deeplabstream, 'Select DLS settings.ini',title='Select settings.ini file inside of DeepLabStream folder')
    ...
    def DLSsettings(self):
        ...
        config = ConfigParser()
        configFile = str(self.label_settingsini.file_path)
        config.read(configFile)
        ...
        self.e_model = FileSelect(label_dlc,'Model',title='Select DeepLabCut tracking model',lblwidth='15')
        #classification
        ...
        self.e_model.filePath.set(model)
        #classification
    ...
    def saveDLSsettings(self):
        ...
        config.set('Streaming', 'IPWEBCAM', str(self.e_reso.entry_get))

        config.set('DeepLabCut', 'DLC_PATH', str(self.e_dlcpath.folder_path))
        config.set('DeepLabCut', 'MODEL', str(self.e_model.file_path))
        ...
        config.set('DeepLabStream','dls_path',str(self.e_dls_path.folderPath))

        with open(self.label_settingsini.file_path, 'w') as configfile:
            config.write(configfile)
