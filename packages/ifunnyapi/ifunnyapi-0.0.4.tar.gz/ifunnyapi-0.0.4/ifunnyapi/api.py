...
class _iFunnyBaseAPI:
    ...
    def _get_paging_items(self, path: str, key: str,
        ...
        batch = self._get(f"{path}?limit={ilim}")
        ...
        if lnone:
            while has_next(batch := self._get(nexturl)):
            ...
            for _ in range(val):
                ...
                items.extend(get_items(batch))
                nexturl = f"{path}?limit=100&next={get_next(batch)}"
            batch = self._get(f"{path}?limit={rem}")
            items.extend(get_items(batch))
            return items
    ...
    def user_features(self, *, user_id, limit: int = None) -> List[dict]:
        return self._get_paging_items(
            f"/timelines/users/{user_id}/featured?limit=30",
            "content",
            limit
        )
...
class iFunnyAPI(_iFunnyBaseAPI):
    ...
    @classmethod
    def _req_auth(cls, btoken: str, email: str, password: str):
        r = requests.post(
    ...
    def from_creds(cls, email: str, password: str, primer: int = 10):
        ...
        if "error" in r:
            raise APIError(r["error_description"])
        token = r["access_token"]
        return cls(token)

    @classmethod
    def generate_account(cls, email: str, nick: str, password: str,
