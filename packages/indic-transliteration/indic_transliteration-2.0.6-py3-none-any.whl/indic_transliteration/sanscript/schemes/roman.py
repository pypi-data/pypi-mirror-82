...
class ItransScheme(RomanScheme):
    ...
    def fix_lazy_anusvaara(self, data_in, omit_sam=False, omit_yrl=False, ignore_padaanta=False):
        ...
        if omit_sam:
            ...
            prefix = r"\w"
        data_out = regex.sub('(%s)M( *)([kgx])' % (prefix), r'\1~N\2\3',   data_out)
        data_out = regex.sub('(%s)M( *)([cCj])' % (prefix), r'\1~n\2\3', data_out)
        data_out = regex.sub('(%s)M( *)([tdn])' % (prefix), r'\1n\2\3', data_out)
        data_out = regex.sub('(%s)M( *)([TDN])' % (prefix), r'\1N\2\3', data_out)
        data_out = regex.sub('(%s)M( *)([pb])' % (prefix), r'\1m\2\3', data_out)
        if not omit_yrl:
            data_out = regex.sub('(%s)M( *)([yvl])' % (prefix), r'\1\2.N\2\3', data_out)
        return data_out
