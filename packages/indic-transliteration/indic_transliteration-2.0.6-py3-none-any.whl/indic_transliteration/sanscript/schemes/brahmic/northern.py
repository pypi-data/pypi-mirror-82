...
class DevanagariScheme(BrahmicScheme):
    ...
    def fix_lazy_anusvaara(self, data_in, omit_sam=False, omit_yrl=False, ignore_padaanta=False):
        ...
        if omit_sam:
            ...
            prefix = r"."
        data_out = regex.sub('(%s)ं( *)([क-ङ])' % (prefix), r'\1ङ्\2\3',   data_out)
        data_out = regex.sub('(%s)ं( *)([च-ञ])' % (prefix), r'\1ञ्\2\3',   data_out)
        data_out = regex.sub('(%s)ं( *)([त-न])' % (prefix), r'\1न्\2\3',   data_out)
        data_out = regex.sub('(%s)ं( *)([ट-ण])' % (prefix), r'\1ण्\2\3',   data_out)
        data_out = regex.sub('(%s)ं( *)([प-म])' % (prefix), r'\1म्\2\3',   data_out)
        if not omit_yrl:
            data_out = regex.sub('(%s)ं( *)([यलव])' % (prefix), r'\1\2्ँ\2\3',   data_out)
        return data_out
