...
class ItransScheme(RomanScheme):
    ...
    def fix_lazy_anusvaara(self, data_in, omit_sam=False, omit_yrl=False, ignore_padaanta=False):
        ...
        if omit_sam:
            ...
            prefix = r"\w"
        data_out = regex.sub(r'(%s)M( *)([kgx])', r'\1~N\2\3' % (prefix),   data_out)
        data_out = regex.sub(r'(%s)M( *)([cCj])', r'\1~n\2\3' % (prefix),   data_out)
        data_out = regex.sub(r'(%s)M( *)([tdn])', r'\1n\2\3' % (prefix),   data_out)
        data_out = regex.sub(r'(%s)M( *)([TDN])', r'\1N\2\3' % (prefix),   data_out)
        data_out = regex.sub(r'(%s)M( *)([pb])', r'\1m\2\3' % (prefix),   data_out)
        if not omit_yrl:
            data_out = regex.sub(r'(%s)M( *)([yvl])', r'\1\2.N\2\3' % (prefix),   data_out)
        return data_out
