...
def lambdaSingleQuery(event, _):
  ...
  result = singleQueryResult.inventory if singleQueryResult else f"missing product {body}"
...
def lambdaAllQuery (events, _):
  url = Database.allQuery(bucket = INVENTORY_BUCKET_NAME, user=None, pw=None)
  print(url[:20])
