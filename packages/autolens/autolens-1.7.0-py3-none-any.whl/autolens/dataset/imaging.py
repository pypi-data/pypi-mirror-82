...
class SimulatorImaging(imaging.SimulatorImaging):
    ...
    def from_tracer_and_grid(self, tracer, grid, name=None):
        ...
        if self.psf is not None:
            ...
            simulator.exposure_time_map = self.exposure_time_map.padded_before_convolution_from(
                kernel_shape=self.psf.shape_2d
            )

            if self.background_sky_map is not None:

                simulator.background_sky_map = self.background_sky_map.padded_before_convolution_from(
                    kernel_shape=self.psf.shape_2d
                )
