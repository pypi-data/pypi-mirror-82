	...
	* speed vs rate plots
	...
	def makeHDPlot(self, cluster, ax, **kwargs):
		...
		if rmap[0].ndim == 1:
			...
			# See if we should add the mean resultant vector (mrvspk_pos_idx
...
class FieldCalcs:
	...
	def limit_to_one(self, A, prc=50, min_dist=5):
		...
		field_labels = skimage.segmentation.watershed(image=-Ac,
											  markers=peak_labels)
	...
	def getGridFieldMeasures(self, A, maxima='centroid', field_extent_method=2, allProps=True, **kwargs):
		...
		if field_extent_method == 1:
			...
			fieldsLabel = skimage.segmentation.watershed(image=-A_tmp, markers=peaksLabel)
