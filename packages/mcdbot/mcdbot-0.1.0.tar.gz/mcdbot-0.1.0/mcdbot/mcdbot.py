...
class Mcdbot(discord.Client):
    def __init__(self, config: Config = global_config):
        ...
        self.main_guild = self.get_guild(global_config.main_guild_id)
    ...
    def run(self):
        ...
        async def runner():
            nonlocal self
            try:
                await self.redis.start(), self.rcon.start(), self.mojang_api.start()
                await self.start()
            finally:
                await self.rcon.stop(), self.redis.stop(), self.mojang_api.stop()
    ...
    def on_error(self, event_method, *args, **kwargs):
        ...
        try:
            ...
            await main_channel.channel.send(
