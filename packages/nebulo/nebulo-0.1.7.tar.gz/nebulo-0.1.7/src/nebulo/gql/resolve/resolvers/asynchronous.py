...
def jwt_claims_to_statement(jwt_claims: typing.Dict[str, typing.Any]):
    """Set 'jwt.claims.<key>' for each claim in claims dict"""
    ...
    if role_key in jwt_claims:
        claims.append(
            func.set_config(
                ...
                func.cast(str(claims[role_key]), Text()),
                True,
            )
        )
...
async def async_resolver(_, info: ResolveInfo, **kwargs) -> typing.Any:
    ...
    async with database.transaction():
        ...
        if isinstance(tree.return_type, FunctionPayloadType):
            ...
            function_args = [
                val
                for key, val in tree.args["input"].items()
            ...
            if isinstance(sql_function.return_sqla_type, TableProtocol):
                ...
                stmt = select(
                    [
                        to_node_id_sql(return_sqla_model, core_table.alias()).label(
                            "nodeId"
                        )
                    ]
                ...
                if query_tree is not None:
                    ...
                    coro_rvf_result: str = (await database.fetch_one(query=query))[
                        "json"
                    ]
            ...
            result = {
                tree.alias: {**stmt_result, **{mutation_id_alias: maybe_mutation_id}}
            }
            ...
            node_id_alias = next(
                iter([x.alias for x in tree.fields if x.name == "nodeId"]), "nodeId"
            )
