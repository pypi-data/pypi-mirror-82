...
def transform_triple_colons(app, docname, source):
    """Transform ::: when read
    ...
    #print(docname)
    #print(source)
    if not app.config.sphinx_lesson_transform_triple_colons:
        return
    LOG.debug('transform_triple_colons %s', docname)
    content = source[0]
    LOG.debug(content)
...
def setup(app):
    ...
    app.add_config_value('sphinx_lesson_transform_triple_colons', True, 'env')
    ...
    app.connect('source-read', transform_triple_colons)
