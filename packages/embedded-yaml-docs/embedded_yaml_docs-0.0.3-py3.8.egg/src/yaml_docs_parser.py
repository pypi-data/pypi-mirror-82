...
class YAMLDocsParser():
    ...
    def __get_token_from_comment(
        self,
        token: any,
        exclude_comments: list,
        remove_symbols: list
        ...
        # Remove symbols from the comment block.
        if comment_block:
            comment_block = [ comment_block.replace(sym, '') for sym in remove_symbols ]
    ...
    def extract_docs(
            ...
            for key in section.keys():
                ...
                if isinstance(val, CommentedSeq):
                    ...
                    if val.ca.items and '0' in val.ca.items:
                        for comment in self.__get_token_from_comment(val.ca.items[0], exclude_comments, remove_symbols):
                ...
                if section.ca and section.ca.items:
                    ...
                    if key in section.ca.items:
                        for comment in self.__get_token_from_comment(section.ca.items[key], exclude_comments, remove_symbols):
                            # Check if we are currently at the root, or the section is a
                            # list of YAML objects (CommentedSeq
                            if parent is None or not isinstance(parent, CommentedSeq):
        ...
        # Extract the comments for a list section.
            ...
            for index, item in enumerate(section):
                ...
                if index in section.ca.items:
                    for comment in self.__get_token_from_comment(section.ca.items[index], exclude_comments, remove_symbols):
        ...
        if parent is None and isinstance(section, dict):
            if section.ca.comment is not None:
                for comment in self.__get_token_from_comment(section.ca.comment, exclude_comments, remove_symbols):
                    section[property_name] = comment