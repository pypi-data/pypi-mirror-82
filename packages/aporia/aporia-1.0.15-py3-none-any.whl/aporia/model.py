...
from aporia.api.log_predict_label import is_valid_predict_label_input, log_predict_label_batch
...
class BaseModel:
    ...
    def log_prediction_label(
        self,
        labels: Union[float, List[float]],
        user_id: Optional[PredictionIdentifier] = None,
        aporia_id: Optional[int] = None,
    ):
        """Logs a single prediction labels.

        Use this function to report the label (ground truth) of a previously reported prediction.
        The ground truth will be used by Aporia to calculate model's performance. Either user_id or
        aporia_id must be reported, but no both.

        Args:
            labels (Union[float, List[float]]): Prediction labels
            ...
            aporia_id (int, optional): An unique identifier of the prediction which was generated
                by Aporia during the prediction report. Defaults to None.
        """
        try:
            self.log_prediction_label_batch(
                user_ids=None if user_id is None else [user_id],
                aporia_ids=None if aporia_id is None else [aporia_id],
                labels=[convert_to_list(convert_scalar_to_iterable(labels))],
            )
        except Exception as err:
            handle_error(
                message="Logging prediction label failed, error: {}".format(str(err)),
                add_trace=self._debug,
                raise_exception=False,
                original_exception=err,
                log_level=logging.ERROR,
    ...
    def log_prediction_label_batch(
        self,
        labels: List[List[float]],
        user_ids: Optional[List[PredictionIdentifier]] = None,
        aporia_ids: Optional[List[int]] = None,
    ):
        """Logs multiple predictions labels.

            Use this function to report the label (ground truth) of a previously reported predictions.
            The ground truth will be used by Aporia to calculate model's performance. Either user_ids or
            aporia_ids must be reported, but no both.
        ...
        if not self._model_version_exists:
            logger.warning(
                "Logging prediction label failed: features for this model version were not reported."
            )
            return

        logger.debug("Logging {} predictions labels".format(len(labels)))
        try:
            labels = convert_list_members_to_lists(convert_to_list(labels))

            if not is_valid_predict_label_input(
                user_ids=user_ids,
                aporia_ids=aporia_ids,
                labels=labels,
            ):
                logger.warning("Logging prediction label failed: Invalid input.")
                return

            self._event_loop.run_coroutine(
                coro=self._log_predict_label_batch(
                    user_ids=user_ids,
                    aporia_ids=aporia_ids,
                    labels=labels,
                ),
            ...
            handle_error(
                message="Logging prediction label batch failed, error: {}".format(str(err)),
                add_trace=self._debug,
                raise_exception=False,
                original_exception=err,
                log_level=logging.ERROR,
    ...
    async def _log_predict_label_batch(
        self,
        labels: List[List[float]],
        user_ids: Optional[List[PredictionIdentifier]] = None,
        aporia_ids: Optional[List[int]] = None,
