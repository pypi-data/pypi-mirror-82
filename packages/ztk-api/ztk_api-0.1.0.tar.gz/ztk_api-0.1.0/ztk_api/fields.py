...
cid_field = Field(None, title='一级商品分类', description='''一级商品分类
值为空：全部商品
1：女装
2：母婴
3：美妆
...
14：家装家纺
''')
sort_fields = Field('new', title='商品排序方式', description='''
new：按照综合排序\n
...
tao_id_field = Field(..., title='淘宝的商品 id', description='都是同一个意思: item_id/num_iid/tao_id')
...
date_time_yongjin_field = Field(..., title='数据更新时间', description='例如: 2020/06/25 20:35:46')
...
small_images_field = Field(..., title='商品组图/商品小图列表', description='商品组图, 为多个 url, 使用 | 分割')
...
creditLevel_field = Field(..., title='店铺等级', description='''1-20:
一星 二星 三星 四星 五星
一钻 二钻 三钻 四钻 五钻
一皇冠 二皇冠 三皇冠 四皇冠 五皇冠
一金冠 二金冠 三金冠 四金冠 五金冠
...
tag_field = Field(..., title='朋友圈文案', description='例如: 赶紧拯救你厨房的油腻吧😂植护🌿加厚厨房抽纸高密度纤维纸张，吸油💧很不错5包装|￥14.8元💰✔厨房小帮手，用处多多')
presale_discount_fee_text_field = Field(None, title='预售商品-商品优惠信息', description='例如: 付定金立减20元')
...
material_lib_type_field = Field(..., title='商品库类型',
...
kuadian_promotion_info_field = Field(..., title='跨店满减信息', description='例如: ["每100减20","每200减50"]')
