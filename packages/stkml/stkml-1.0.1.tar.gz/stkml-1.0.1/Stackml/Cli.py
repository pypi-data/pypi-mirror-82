...
from Stackml.Diagram.Map.RegionNotFound import RegionNotFound
from Stackml.Drawio import DrawIO
from Stackml.Stackml import Stackml, StackmlSyntaxicErrorList, ModelException, WarningList
from Stackml.Diagram import  StackmlDiagram
from Stackml import MODULE_DIR, __version__
...
def diagram(ctx: click.Context, type: str, icons: str, outputfile: str) -> None:
    ...
    if which('dot'):
        ...
        diagram_ = StackmlDiagram(icons=icons)
        try:
...
def create_stackml(stackml_folder):
    ...
    stackml = Stackml(stackml_folder)
