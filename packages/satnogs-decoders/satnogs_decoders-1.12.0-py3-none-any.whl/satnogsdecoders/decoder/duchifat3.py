...
class Duchifat3(KaitaiStruct):
    ...
    class TelemetryDumpsT(KaitaiStruct):
        ...
        def _read(self):
            ...
            if _on == 50:
                self.sub_service = self._root.EpsTlmT(self._io, self, self._root)
            elif _on == 51:
                self.sub_service = self._root.CommTlmT(self._io, self, self._root)
            elif _on == 53:
                self.sub_service = self._root.SolarPanelsTlmT(self._io, self, self._root)
            elif _on == 73:
                self.sub_service = self._root.FileSystemSpaceAT(self._io, self, self._root)
    ...
    class GeneralT(KaitaiStruct):
        ...
        def _read(self):
            _on = self._parent.sub_service_type
            if _on == 49:
                self.sub_service = self._root.EventLogT(self._io, self, self._root)
            elif _on == 50:
                self.sub_service = self._root.ErrorLogT(self._io, self, self._root)
    ...
    class GlobalParametersT(KaitaiStruct):
        ...
        def _read(self):
            _on = self._parent.sub_service_type
            if _on == 25:
                self.sub_service = self._root.BeaconT(self._io, self, self._root)
    ...
    class Ax25InfoData(KaitaiStruct):
        ...
        def _read(self):
            ...
            if _on == 3:
                self.service = self._root.GlobalParametersT(self._io, self, self._root)
            elif _on == 13:
                self.service = self._root.GeneralT(self._io, self, self._root)
            elif _on == 88:
                self.service = self._root.TelemetryDumpsT(self._io, self, self._root)
