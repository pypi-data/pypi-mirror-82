...
class Meznsat(KaitaiStruct):
    ...
    class UiFrame(KaitaiStruct):
        ...
        def _read(self):
            ...
            if _on == u"A68MZ ":
                ...
                self.ax25_info = self._root.MeznsatPayload(io, self, self._root)
            else:
    ...
    class MeznsatPayload(KaitaiStruct):
        ...
        def _read(self):
            _on = self._io.size()
            if _on == 78:
                self.beacon_type = self._root.MeznsatTlm(self._io, self, self._root)
            else:
                self.beacon_type = self._root.MeznsatGenericPacket(self._io, self, self._root)
