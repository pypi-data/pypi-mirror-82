...
class Chomptt(KaitaiStruct):
    ...
    class UiFrame(KaitaiStruct):
        ...
        def _read(self):
            ...
            if _on == 69:
                ...
                self.ax25_info = self._root.Optib85T(io, self, self._root)
            elif _on == 128:
                self._raw_ax25_info = self._io.read_bytes_full()
                io = KaitaiStream(BytesIO(self._raw_ax25_info))
                self.ax25_info = self._root.Chomptb85T(io, self, self._root)
            else:
    ...
    class Optib85T(KaitaiStruct):
        ...
        def _read(self):
            ...
            io = KaitaiStream(BytesIO(self._raw_optib85))
            self.optib85 = self._root.OptiT(io, self, self._root)
    ...
    class Chomptb85T(KaitaiStruct):
        ...
        def _read(self):
            ...
            io = KaitaiStream(BytesIO(self._raw_chomptb85))
            self.chomptb85 = self._root.ChomptT(io, self, self._root)
            self.eod = self._io.read_bytes(2)
