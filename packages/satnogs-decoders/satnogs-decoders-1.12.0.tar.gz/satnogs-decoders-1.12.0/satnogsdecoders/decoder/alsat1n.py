...
class Alsat1n(KaitaiStruct):
    ...
    def _read(self):
        ...
        _on = self.callsign
        if _on == u"AL1N":
            self.alsat1n_payload = self._root.Alsat1nPayloadT(self._io, self, self._root)
    ...
    class DownloadData(KaitaiStruct):
        ...
        def _read(self):
            ...
            io = KaitaiStream(BytesIO(self._raw_bin_file_data))
            self.bin_file_data = self._root.EncodedDataT(io, self, self._root)
    ...
    class SscChannelT(KaitaiStruct):
        ...
        def _read(self):
            ...
            if _on == 5:
                self.payload_type = self._root.Obc(self._io, self, self._root)
            elif _on == 19:
                self.payload_type = self._root.File(self._io, self, self._root)
            elif _on == 46:
                self.payload_type = self._root.Eps(self._io, self, self._root)
    ...
    class Eps(KaitaiStruct):
        ...
        def _read(self):
            ...
            if _on == 244:
                self.msg_type = self._root.EpsPlatform(self._io, self, self._root)
            elif _on == 161:
                self.msg_type = self._root.P7Psu(self._io, self, self._root)
            elif _on == 63:
                self.msg_type = self._root.EpsPsuCurrent5v(self._io, self, self._root)
            elif _on == 166:
                self.msg_type = self._root.P8Psu(self._io, self, self._root)
            elif _on == 65:
                self.msg_type = self._root.EpsPsuCurrent3v3(self._io, self, self._root)
            elif _on == 122:
                self.msg_type = self._root.PdmPlatform(self._io, self, self._root)
            elif _on == 240:
                self.msg_type = self._root.EpsSys(self._io, self, self._root)
            elif _on == 175:
                self.msg_type = self._root.PdmPayload(self._io, self, self._root)
    ...
    class Alsat1nPayloadT(KaitaiStruct):
        ...
        def _read(self):
            self.strx_header = self._root.StrxHeaderT(self._io, self, self._root)
            self.ssc_header = self._root.SscHeaderT(self._io, self, self._root)
            self.ssc_channel = self._root.SscChannelT(self._io, self, self._root)
    ...
    class File(KaitaiStruct):
        ...
        def _read(self):
            _on = self._parent._parent.ssc_header.msg_id
            if _on == 21:
                self.msg_type = self._root.DownloadData(self._io, self, self._root)
            elif _on == 30:
                self.msg_type = self._root.FileCount(self._io, self, self._root)
    ...
    class Obc(KaitaiStruct):
        ...
        def _read(self):
            _on = self._parent._parent.ssc_header.msg_id
            if _on == 1:
                self.msg_type = self._root.ObcHealth(self._io, self, self._root)
            elif _on == 32:
                self.msg_type = self._root.I2cTraffic(self._io, self, self._root)
            elif _on == 127:
                self.msg_type = self._root.WatchdogTimeRemaining(self._io, self, self._root)
            elif _on == 176:
                self.msg_type = self._root.AmradMsg(self._io, self, self._root)
