    ...
    class UiFrame(KaitaiStruct):
        ...
        def _read(self):
            ...
            if _on == u"NA1SS ":
                ...
                self.info = self._root.AprsMicET(io, self, self._root)
            else:
                self._raw_info = self._io.read_bytes_full()
                io = KaitaiStream(BytesIO(self._raw_info))
                self.info = self._root.AprsT(io, self, self._root)
    ...
    class AprsMicET(KaitaiStruct):
        ...
        def _read(self):
            ...
            if _on == u"]":
                ...
                io = KaitaiStream(BytesIO(self._raw_tlm_data))
                self.tlm_data = self._root.KenwoodTmd700T(io, self, self._root)
            else:
