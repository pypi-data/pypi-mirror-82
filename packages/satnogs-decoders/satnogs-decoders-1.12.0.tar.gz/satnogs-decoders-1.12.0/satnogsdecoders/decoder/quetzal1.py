...
class Quetzal1(KaitaiStruct):
    ...
    class BeaconT(KaitaiStruct):
        ...
        def _read(self):
            self.cubesat_identifier_magic = self._io.ensure_fixed_contents(b"\x51\x55\x45\x54\x5A\x41\x4C\x31")
            self.cdhs = self._root.CdhsT(self._io, self, self._root)
            self.eps = self._root.EpsT(self._io, self, self._root)
            self.adcs = self._root.AdcsT(self._io, self, self._root)
            self.comm = self._root.CommT(self._io, self, self._root)
            self.payload = self._root.PayloadT(self._io, self, self._root)
            self.ram_params = self._root.RamParamsT(self._io, self, self._root)
            self.uvg_msg = self._root.UvgMsgT(self._io, self, self._root)
    ...
    class Ax100T(KaitaiStruct):
        ...
        def _read(self):
            ...
            if _on == 0:
                self.csp_node_port = self._root.Ax100ControlPortT(self._io, self, self._root)
    ...
    class ObcT(KaitaiStruct):
        ...
        def _read(self):
            self.csp_node_port = self._root.BeaconT(self._io, self, self._root)
    ...
    class Ax25InfoData(KaitaiStruct):
        ...
        def _read(self):
            self.csp_header = self._root.CspHeaderT(self._io, self, self._root)
            _on = self.csp_header.source
            if _on == 1:
                self.csp_node = self._root.ObcT(self._io, self, self._root)
            elif _on == 5:
                self.csp_node = self._root.Ax100T(self._io, self, self._root)
