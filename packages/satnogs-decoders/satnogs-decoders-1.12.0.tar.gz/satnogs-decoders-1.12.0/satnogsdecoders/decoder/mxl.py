...
class Mxl(KaitaiStruct):
    ...
    class UiFrame(KaitaiStruct):
        ...
        def _read(self):
            ...
            if _on == u"KF6RFX":
                ...
                self.ax25_info = self._root.RapT(io, self, self._root)
            elif _on == u"NOCALL":
                self._raw_ax25_info = self._io.read_bytes_full()
                io = KaitaiStream(BytesIO(self._raw_ax25_info))
                self.ax25_info = self._root.RapT(io, self, self._root)
    ...
    class GrifexBeaconT(KaitaiStruct):
        ...
        def _read(self):
            ...
            if _on == 261:
                self.grifex_beacon = self._root.GrifexT(self._io, self, self._root)
    ...
    class Mcubed2BeaconT(KaitaiStruct):
        ...
        def _read(self):
            _on = self._root.framelength
            if _on == 257:
                self.mcubed2_beacon = self._root.Mcubed2T(self._io, self, self._root)
    ...
    class TbexBeaconT(KaitaiStruct):
        ...
        def _read(self):
            _on = self._root.framelength
            if _on == 219:
                self.tbex_beacon = self._root.TbexBeacon2T(self._io, self, self._root)
            elif _on == 255:
                self.tbex_beacon = self._root.TbexBeacon1T(self._io, self, self._root)
    ...
    class RapT(KaitaiStruct):
        ...
        def _read(self):
            ...
            io = KaitaiStream(BytesIO(self._raw_rap_header))
            self.rap_header = self._root.RapHeaderT(io, self, self._root)
            _on = self.rap_header.secondary_id
            if _on == 50:
                self.payload = self._root.Mcubed2BeaconT(self._io, self, self._root)
            elif _on == 66:
                self.payload = self._root.GrifexBeaconT(self._io, self, self._root)
            elif _on == 82:
