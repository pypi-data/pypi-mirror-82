...
class Opssat1(KaitaiStruct):
    ...
    class UiFrame(KaitaiStruct):
        ...
        def _read(self):
            ...
            if _on == u"DP0OPS":
                self.info = self._root.OpssatPayloadIdentify(self._io, self, self._root)
    ...
    class OpssatPayloadIdentify(KaitaiStruct):
        ...
        def _read(self):
            _on = self._root.id_magic
            if _on == 904908480:
                self.pld_id = self._root.OpssatPayloadDscrm(self._io, self, self._root)
            elif _on == 12625866:
                self.pld_id = self._root.OpssatPayloadPreprocessed(self._io, self, self._root)
            elif _on == 3399991296:
                self.pld_id = self._root.OpssatPayloadPreprocessed(self._io, self, self._root)
