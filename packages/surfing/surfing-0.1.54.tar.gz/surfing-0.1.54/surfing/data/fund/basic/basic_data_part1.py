...
class BasicDataPart1:
    ...
    def fund_ipo_status(self):
        fund_raw_ipo_status = self._raw_data_api.get_em_fund_ipo_status()
        fund_size = self._basic_data_api.get_fund_size_all_data()
        fund_size = fund_size.pivot_table(index='datetime', columns='fund_id',values='size')
        d1 = fund_raw_ipo_status.end_date.unique().tolist()
        d2 = fund_size.index.unique().tolist()
        date_list = sorted(list(set(d1).union(d2)))
        fund_size = fund_size.reindex(date_list).fillna(method='ffill')
        fund_size = fund_size.stack().reset_index().rename(columns={'datetime':'end_date',0:'size'}).set_index(['fund_id','end_date'])
        fund_raw_ipo_status['fund_id'] = fund_raw_ipo_status.apply(
                lambda x : self._data_helper._get_fund_id_from_order_book_id(x.em_id.split('.')[0], x.end_date), axis=1
        )
        fund_raw_ipo_status = fund_raw_ipo_status.drop(['em_id','_update_time'], axis=1)
        fund_raw_ipo_status = fund_raw_ipo_status.dropna(subset=['fund_id'])
        fund_raw_ipo_status = fund_raw_ipo_status.set_index(['fund_id','end_date'])
        fund_raw_ipo_status = fund_raw_ipo_status.join(fund_size[['size']]).reset_index()
        fund_raw_ipo_status = fund_raw_ipo_status.dropna(subset=['size'])
        fund_raw_ipo_status.loc[:,'ipo_allocation_weight'] = 10000 * fund_raw_ipo_status['ipo_allocation_amount'] / fund_raw_ipo_status['size']
        self._data_helper._upload_basic(fund_raw_ipo_status, FundIPOStats.__table__.name)
