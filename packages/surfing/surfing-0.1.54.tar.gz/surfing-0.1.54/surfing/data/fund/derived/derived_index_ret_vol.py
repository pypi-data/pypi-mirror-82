...
class IndexIndicatorProcessor:
    ...
    def _vol_history(self):
        ...
        for name, value in self._DIM_TYPE.items():
            ...
            for index, row in self.index_price_vol_part.iterrows():
                ...
                if bd_list.empty:
                    continue
                temp_vol.append(self.index_price_vol_part.loc[bd_list.array[-1]: index, :].std(ddof=1).rename(row.name))
    ...
    def _update_vol(self):
        ...
        for name, value in self._DIM_TYPE.items():
            ...
            if bd_list.empty:
                continue
            temp_vol = self.index_price_vol_part.loc[bd_list.array[-1]: last_date, :].std(ddof=1).rename(name + 'vol')
