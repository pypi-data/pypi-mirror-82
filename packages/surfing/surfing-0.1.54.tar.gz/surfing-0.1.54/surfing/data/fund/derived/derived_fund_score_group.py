...
class FundScoreGroupProcess:
    ...
    def _calc_score(self, x: pd.DataFrame, by_key: str) -> pd.DataFrame:
        ...
        def _calc_fund_manager_score(fund_x: pd.Series) -> Optional[float]:
            try:
                wind_class_2 = self._fund_info.at[fund_x.name, 'wind_class_2']
                our_type: str = self._wind_class_to_type[wind_class_2]
                managers = self._fund_manager_info.loc[fund_x.name, 'mng_id']
                return self._fund_manager_score.loc[(managers, our_type), 'score'].mean()
            except KeyError:
                pass
        ...
        # 第六个维度 基金经理评分
        result3 = pd.DataFrame(index=fin_data.index)
        result3['team_score'] = fin_data.apply(_calc_fund_manager_score, axis=1)

        # 合并所有的评分并计算总分
        total = result1.join(result2, how='outer').join(result3, how='outer')
