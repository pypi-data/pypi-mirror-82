...
def grow(ai_segment, i_step=1):
    ...
    input:
      ...
      i_step: integer which specifies how many pixels the basin
        ...
        function can't handle shrinking.
...
def membrane_px(ai_segment, dai_value, i_step=1):
    '''
    input:
      ...
      i_step: number of pixel the cell border, which is 2 pixel,
        in both direction is growen, to cover the membrane segement.
        default is 1.

    output:
      df_membrane_px: dictionary of pandas datafarame whith cell_id,
    ...
    description:
        function extracts protein mebran expression values,
    ...
    for i, i_cell in enumerate(sorted(ei_cell)):
        ...
        ai_membrane = grow(ai_border, i_step=i_step)
...
def membrane2_px(ai_segment, dai_value, i_step=1):
    '''
    input:
      ai_segment: numpy array representing a cells basin tiff file.
        it is assumed that basin borders are represented by 0 values,
    ...
    output:
      df_membrane_px: dictionary of pandas datafarame whith cell_id,
        image absolute xy coordinate and
    ...
    tai_membrane_yx = np.where(ai_segment_border != 0)
    ai_membrane_y = tai_membrane_yx[0]
    ai_membrane_x = tai_membrane_yx[1]
    ai_membrane_id = ai_segment_border[ai_segment_border != 0]
    ...
    for s_sensor in ls_sensor:
        ai_value = dai_value[s_sensor]
        li_membrane_value = []
        ai_membrane_value = ai_value[ai_segment_border != 0]
        dai_membrane_value.update({s_sensor : ai_membrane_value})

    s_mebrane_px = 'px\tcell\ty\tx\t' + '\t'.join(ls_sensor) + '\n'
    ls_membrane_px = [s_mebrane_px]  # header
    for i in range(len(ai_membrane_id)):
        print(f'process cell: {i}')
        s_membrane_px = f'{i}\t{ai_membrane_id[i]}\t{ai_membrane_y[i]}\t{ai_membrane_x[i]}'

        for s_sensor in ls_sensor:
            s_membrane_px = f'{s_membrane_px}\t{dai_membrane_value[s_sensor][i]}'

        s_membrane_px = s_membrane_px + '\n'
        ls_membrane_px.append(s_membrane_px)

    with open(f'calx1_membrane_px.tsv', 'w') as f:
        f.writelines(ls_membrane_px)


    # write membrane values
    for i_y, i_x, o_slide in [
            (+1, +0, slide_up), 
            (+1, +1, slide_upleft), 
            (+0, +1, slide_left), 
            (-1, +1, slide_downleft), 
        ...
        #for s_sensor in ls_sensor:
            ...
            #li_membrane_value = []
            #ai_value = dai_value[s_sensor].copy()
            #print('\n',ai_value)
            #for n in range(i_step):
            #    ai_value = o_slide(ai_value)
            #    print(ai_value)
            #    ai_membrane_value = ai_value[ai_segment_border != 0]
            #    print(ai_membrane_value)
            #    li_membrane_value.extend(list(ai_membrane_value))
            #dli_membrane_value.update({s_sensor : li_membrane_value})


        #print(dli_membrane_value)
        ls_membrane_px = []
        for m in range(i_step):
            for i_cell in range(len(ai_membrane_id)):
                print(f'process cell: {i_cell}')
                s_membrane_px = f'{i_cell}\t{ai_membrane_id[i_cell]}\t{ai_membrane_y[i_cell]+i_y*(m+1)}\t{ai_membrane_x[i_cell]+i_x*(m+1)}'
                ...
                for s_sensor in ls_sensor:
                    ...
                    print(ai_membrane_value)
                    #li_membrane_value.extend(list(ai_membrane_value))
                    #li_membrane_value = list(ai_membrane_value)

                # bue: the value number changes with each step!
                #s_membrane_px = f'{s_membrane_px}\t{dli_membrane_value[s_sensor][n]}'
                s_membrane_pxv = f'{s_membrane_px}\t{ai_membrane_value[i_cell]}\n'
                ls_membrane_px.append(s_membrane_pxv)

        with open(f'calx2_membrane_px.tsv', 'a') as f:
            f.writelines(ls_membrane_px)

    # load as pandas dataframe
    # bue: i can try to directely generate the dataframe form a list of lists,
    # but I think this is slower. also writing to file and loading as dataframe will need less ram!
    '''
    df_membrane_px = pd.read_csv(f'calx2_membrane_px.tsv', sep='\t', index_col=0)
    df_membrane_px.drop_duplicates(inplace=True)
    df_membrane_px = df_membrane_px.loc[(df_membrane_px.y > -1) & (df_membrane_px.x > -1),:]
    df_membrane_px = df_membrane_px.loc[
        (df_membrane_px.y < ai_segment.shape[0]) & (df_membrane_px.x < ai_segment.shape[1]),
        :
    ]
    df_membrane_px.index.name = f'membrane_{2*i_step + 1}px'
    df_membrane_px.to_csv(f'calx3_membrane_px.tsv', sep='\t')
    '''
...
def dfmembranepx_stats(df_membrane_px, di_threshold_raw=None):
    '''
    input:
      df_membrane_px: membrane_px output wich is a
        pandas datafarame whit atleast  cell_id,
        image absolute xy coordinate and
        membrane pixel related expression values for all proteins.

      di_threshold_raw: dictionary of raw intensity threshhold values
    ...
    description:
        function calculates statistical key numbers for mebrane protein
        expression values, for each protein submited to the function.
    '''
    # handle input
    ...
    for i, i_cell in enumerate(ei_cell):
        ...
        for s_sensor in es_sensor:
            ...
            if not (di_threshold_raw is None):
                ...
                r_fractpos = ab_membrane.sum() / ab_membrane.shape[0]
                lr_membrane.append(r_fractpos)
