...
def _log_no_change(subcmd, version_pattern, old_version):
    ...
    if is_semver:
        logger.warning(
            'calver {0} [--major/--minor/--patch] required for use with SemVer.'
            .format(subcmd))
    else:
...
@click.option('--tag', default=None, metavar='<NAME>', help=
    'Override release tag of current_version. Valid options are: {0}.'.
    format(', '.join(VALID_RELEASE_TAG_VALUES)))
@click.option('--tag-num', is_flag=True, default=False, help=
    'Increment release tag number (rc1, rc2, rc3..).')
...
def test(old_version, pattern, verbose=0, tag=None, major=False, minor=
    False, patch=False, tag_num=False, pin_date=False, date=None):
    ...
    new_version = incr_dispatch(old_version, raw_pattern=raw_pattern, major
...
def incr_dispatch(old_version, raw_pattern, **kwargs):
    ...
    if new_version is None:
        return None
    elif pkg_resources.parse_version(new_version
...
@click.option('-p', '--patch', is_flag=True, default=False, help=
    'Increment patch component.')
@click.option('-t', '--tag', default=None, metavar='<NAME>', help=
    'Override release tag of current_version. Valid options are: {0}.'.
    format(', '.join(VALID_RELEASE_TAG_VALUES)))
@click.option('--tag-num', is_flag=True, default=False, help=
    'Increment release tag number (rc1, rc2, rc3..).')
...
@click.option('--date', default=None, metavar='<ISODATE>', help=
    'Set explicit date in format YYYY-0M-0D (e.g. {0}).'.format(_current_date))
def update(verbose=0, dry=False, allow_dirty=False, fetch=True, major=False,
