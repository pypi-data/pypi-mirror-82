...
class AsyncImageBuilder(object):
    ...
    def __init__(self, config, logger=None):
        ...
        self._logger = logger or Logger.with_default_handlers()
    ...
    async def run(self, keep_context=True, tag=None, no_cache=True, dry_run=False):
        """
        Arguments
        ---------
            config (dict): Build configuration
        ...
        # create context obj
        self.context = AsyncContext(keep=keep_context, logger=self._logger)
        ...
        async with self.context:
            ...
            # Start docker build
            if not dry_run:
                await self._logger.info('Building image')
                await self._build_image(no_cache=no_cache)
                await self._logger.info("Built image '%s' successfully" 
                                    % tag if tag else self.image.id)

        return self.image
        
    async def _populate_context(self):

        # Dump config to yaml file in context
        await self._logger.info('Saving config to context folder')
        ...
        if 'proxy' in self.config:
            await self._process_proxy(self.config['proxy'])

        # generate pip.conf in context
        if 'pip-config' in self.config:
            await self._process_pip_config(self.config['pip-config'])

        if 'snapshot' in self.config:
            await self._process_snapshot(self.config['snapshot'])
        
        if 'repositories' in self.config:
            await self._process_repositories(self.config['repositories'])
        
        if 'files' in self.config:
            await self._process_files(self.config['files'])

        if 'requirements' in self.config:
            await self._discover_requirements_txt(self.config['requirements'])
        ...
        if 'packages' in self.config:
            await self._write_requirements_file(self.config['packages'])

        # job discovery     
        if 'jobfiles' in self.config:
            await self._discover_jobs(self.config['jobfiles'])   

        # Write formatted Dockerfile in context
        await self._logger.info('Writing formatted Dockerfile')
        self.context.write_file(INSTALLATION/'Dockerfile', 
                                self.image.manifest())

    async def _process_snapshot(self, snapshot_file):
        ...
        await self._logger.info('Copying %s to context' % snapshot_file)
        ...
        if 'repositories' in snapshot:
            await self._process_repositories(snapshot['repositories'])

        if 'packages' in snapshot:
            await self._write_requirements_file(snapshot['packages'])

    async def _process_proxy(self, proxy_config):
        await self._logger.info('Setting proxy environment variables')
    ...
    async def _process_files(self, files):
        await self._logger.info('Adding files to workspace')
        for from_path in files:
            ...
            if not url_parts.scheme:
                ...
                await self._logger.info('Copying with scp %s' % from_path)
                scp(host=host,
                ...
                await self._logger.info('Retreiving from ftp %s' % from_path)
                ftp_retrieve(host=host, from_path=url_parts.path,
                             to_path=to_path, port=port,
                             secure=url_parts.scheme == 'ftps')

    async def _process_repositories(self, repositories):
        # Clone all git repositories and checkout a specific commit
        # if one is given
        await self._logger.info('Cloning git repositories')

        for name, vals in repositories.items():
            await self._logger.info('Cloning repo %s' % vals['url'])
    ...
    async def _write_requirements_file(self, packages):
        # Generate python requirements file
        self._req_counter += 1
        filename = '%s-%s' % (self._req_counter, REQUIREMENTS_FILE)

        await self._logger.info('Writing %s' % filename)
    ...
    async def _discover_requirements_txt(self, config):
    ...
    async def _process_pip_config(self, config):
        # pip config for setting things like pypi server.
        await self._logger.info('Writing %s file' % PIP_CONF_FILE)
    ...
    async def _discover_jobs(self, jobfiles):
        await self._logger.info('Discovering Jobfiles')
        ...
        await self._logger.info('Number of discovered job files: %s' 
                          % len(rel_job_paths))
        await self._logger.info('List of job files written to: %s' % jobfiles)

    async def _build_image(self, no_cache=False):
        # copy entrypoint to the context
        await self._logger.info('Copying entrypoint to context')
        ...
        try:
            ...
            for line in await asyncio.wait_for(future, timeout=7200, loop=loop):

                # Log stream from build
                if 'stream' in line:
                    contents = line['stream'].rstrip()
                    if contents:
                        await self._logger.debug(contents)
