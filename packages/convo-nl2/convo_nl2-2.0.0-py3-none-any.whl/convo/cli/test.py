...
def run_core_test(args: argparse.Namespace) -> None:
    """Run core tests."""
    from convo.test import test_core_models_in_directory, test_core, test_core_models

    stories = convo.cli.utils.get_validated_path(
        args.stories, "stories", DEFAULT_DATA_PATH
    )
    if args.e2e:
        stories = convo.shared.data.get_test_directory(stories)
    else:
        stories = convo.shared.data.get_core_directory(stories)
    ...
    if isinstance(args.model, str):
        model_path = convo.cli.utils.get_validated_path(
...
def run_nlu_test(args: argparse.Namespace) -> None:
    """Run NLU tests."""
    from convo.test import compare_nlu_models, perform_nlu_cross_validation, test_nlu

    nlu_data = convo.cli.utils.get_validated_path(args.nlu, "nlu", DEFAULT_DATA_PATH)
    nlu_data = convo.shared.data.get_nlu_directory(nlu_data)
    output = args.out or DEFAULT_RESULTS_PATH
    args.errors = not args.no_errors
    ...
    if args.config is not None and len(args.config) == 1:
        ...
        if os.path.isdir(args.config):
            args.config = convo.shared.utils.io.list_files(args.config)
    ...
    if isinstance(args.config, list):
        ...
        for file in args.config:
            try:
                validation_utils.validate_yaml_schema(
                    convo.shared.utils.io.read_file(file), CONFIG_SCHEMA_FILE,
                )
        ...
        config = convo.cli.utils.get_validated_path(
