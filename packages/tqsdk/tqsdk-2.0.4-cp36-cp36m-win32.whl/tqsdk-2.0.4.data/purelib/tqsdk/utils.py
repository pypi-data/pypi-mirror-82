...
def _query_for_quote(symbol):
    ...
    query = """
        ...
        ... on basic{ class trading_time{day night} trading_day instrument_id price_tick price_decs exchange_id english_name}
...
def _symbols_to_quotes(symbols):
    ...
    quotes = {}
    for k in result:
        for quote in result[k]:
            if quote.get("leg1", None):
                quote["leg1_symbol"] = quote.get("leg1")["instrument_id"]
                del quote["leg1"]
            if quote.get("leg2", None):
                quote["leg2_symbol"] = quote.get("leg2")["instrument_id"]
                del quote["leg2"]
            ...
            if quote.get("expire_datetime"):
                e_dt = quote["expire_datetime"] / 1e9
                quote["expire_datetime"] = e_dt
                quote["delivery_year"] = datetime.fromtimestamp(e_dt).year
                quote["delivery_month"] = datetime.fromtimestamp(e_dt).month
            if quote.get("underlying"):  # 展开标的合约
                for edge in quote["underlying"]["edges"]:
                    ...
                    if underlying_quote.get("expire_datetime"):
                        e_dt = underlying_quote["expire_datetime"] / 1e9
                        underlying_quote["expire_datetime"] = e_dt
                        underlying_quote["delivery_year"] = datetime.fromtimestamp(e_dt).year
                        underlying_quote["delivery_month"] = datetime.fromtimestamp(e_dt).month
