...
class ChemModule(QtWidgets.QDialog):
    ...
    """
    Floating filter reactor module. Simple cartridge where a reaction
    could happen
    """
    ...
    def __init__(self, parent=None, cad_obj: Union[None, ff_rea] = None):
    ...
    def bottomTypeChanged(self):
        ...
        if bottom_type in ff_rea.BOTTOMS_PIPE:
    ...
    def topTypeChanged(self):
        ...
        if top_type in f_rea.THREADED_TOPS:
            ...
            self.spin_r_int.setEnabled(False)
    ...
    def deleteIO(self):
        ...
        self.l.debug(f"Floating Filter reactor, deleting I/O {name}")
    ...
    def openIODialog(self, item: QtWidgets.QTableWidgetItem):
        ...
        self.l.debug("Floating Filter Reactor, opening I/O dialog box")
    ...
    def restoreParams(self):
        ...
        self.params["r_int"] = self.cad_obj.r_int
        ...
        self.spin_r_int.setValue(self.params["r_int"])
        ...
        self.l.debug(f"Floating filter reactor, restored reactor: {self.params}")
    ...
    def readForm(self):
        ...
        self.params["r_int"] = self.spin_r_int.value()
        ...
        self.l.debug(f"Floating filter reactor, read form {self.params}")

    def addInput(self, params: dict):

        """The AddIODialogFFR class will call this method"""

        self.l.debug(f"Floating filter reactor, adding input: {params}")
    ...
    def addOutput(self, params: dict):
        ...
        self.l.debug(f"Floating filter reactor, adding output: {params}")
        ...
        if name not in self.dict_outputs:
            self._addNewRow(name, params["type_io"])
            self.l.debug(f"module_floating_filter_reactor: addOutput {params}")
        else:
    ...
    def addTopInlet(self, params: dict):
        ...
        self.l.debug(f"Floating Filter Reactor, adding top inlet: {params}")
        ...
        if name not in self.dict_top_inlets:
            self._addNewRow(name, params["type_io"])
            self.l.debug(f"module_floating_filter_reactor: top inlet {params}")
        else:
    ...
    def buildInputs(self, cad_obj):
        ...
        self.l.debug("Floating filter reactor, building inputs")
    ...
    def buildOutputs(self, cad_obj):
        ...
        self.l.debug("Floating filter reactor, building outputs")
    ...
    def _buildNewModule(self) -> bool:
        ...
        self.l.debug("Floating filter reactor, _buildNewModule")

        try:
            reactor = self._buildCADObject()
        except Exception as e:
            mes = f"Impossible to create floating filter reactor: {e}"
    ...
    def _updateModule(self) -> bool:
        ...
        self.l.debug("Floating fitler reactor, _updateModule")
        ...
        if (
            ...
            self.cad_obj.r_int = self.params["r_int"]
        else:
            self.cad_obj.r_constrained = False

        self.cad_obj.volume_bottom = self.params["volume_bottom"]
        self.cad_obj.volume_top = self.params["volume_top"]
        ...
        try:
            ...
            mes = f"Impossible to refresh floating filter reactor: {e}"
            self.l.error(mes, exc_info=True)
            QtWidgets.QMessageBox.critical(
    ...
    def _buildCADObject(self) -> ff_rea:
        ...
        self.l.debug("Floating filter reactor, buildModule wo cad_obj")
        ...
        # Try to set the internal radius only if top is not threaded
    ...
    def initUI(self):
        ...
        if self.cad_obj is None:
            self.setWindowTitle(f"Add a filter reactor module")
        else:
            self.setWindowTitle(f"Modify filter reactor module")
    ...
    def _buildAdvancedSettings(self) -> QtWidgets.QWidget:
        ...
        self.combo_align_top_strategy.addItems(ff_rea.ALIGN_TOP_STRATEGIES)

        # How to expand the module if expand is chosen to align the top
        self.combo_expand_strategy = QtWidgets.QComboBox(self)
        self.combo_expand_strategy.addItems(ff_rea.EXPAND_STRATEGIES)

        # Combo box for align filter strategy
        self.combo_align_filter_strategy = QtWidgets.QComboBox(self)
        self.combo_align_filter_strategy.addItems(ff_rea.ALIGN_FILTER_STRATEGIES)

        # Add widgets to form layout
        fbox.addRow("Reactor internal radius: ", self.spin_r_int)
