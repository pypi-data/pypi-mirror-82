...
from grid.cli.grid_train import (_check_run_name_is_valid,
                                 _check_is_python_script,
                                 _check_description_isnt_too_long)
...
class TrainCallbacksTestCase(unittest.TestCase):
    ...
    def test_python_script_callback(self):
        """
        _check_is_python_script() raises exception if file isn't Python
        script.
        """
        f = 'foo.txt'
        with self.assertRaises(click.BadParameter):
            _check_is_python_script(None, None, f)
