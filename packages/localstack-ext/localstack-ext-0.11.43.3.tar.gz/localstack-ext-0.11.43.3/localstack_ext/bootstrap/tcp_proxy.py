...
class AttrDict(AKVLn):
 def __init__(self,*args,**kwargs):
  AKVLG(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and AKVLh(socket,'IPV6_V6ONLY'):
...
class Half(AKVLW):
 def __init__(self,opt,sock,addr,AKVLN):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.AKVLN=AKVLN
  self.name='peer' if self.AKVLN else 'client'
  self.queue=[]
  self.dest=AKVLd
  self.err=AKVLd
  self.ready=AKVLu
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':AKVLu,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':AKVLu,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':AKVLu,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':AKVLu,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
