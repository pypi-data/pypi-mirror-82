...
class MetricsPusher:
    ...
    def set_metrics_pod_ips(self):
        ...
        :return: None
        """
        socket_info_list = socket.getaddrinfo(self.metrics_service_name,
                                              self.metrics_service_port)
    ...
    def push_metrics(self, registry):
        for gateway in self.metrics_pod_ips:
            try:
                ...
                LOGGER.debug(f'Job {self.job}: Metric pushed to Batch-Monitoring successfully')
