...
class ParseHtml:
    ...
    def get_tags(
        self, tag_name: str = None, tag_text: str = None, label_text: str = None,
    ) -> None:
        try:
    ...
    def get_library_prep_parameters(self, label: str = None, key: str = None,) -> None:

        for tag in self.soup.find_all("label"):
            if tag.text.strip() == label:
                parameters = pd.read_html(repr(tag.find_next("table")), index_col=0)[0]
    ...
    def get_tables(self) -> None:
        ...
        self.get_library_prep_parameters(
            label="Hashtag parameters", key="hashtag_parameters",
        )

        # collect TCR prep parameters
        self.get_library_prep_parameters(
            label="TCR parameters", key="TCR_parameters",
        )
