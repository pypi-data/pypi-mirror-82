        ...
        for i in range(len(All_pathways)):
                    ...
                    if str(All_pathways_RLS_reactant[i]) == str(pathway_t[j]) and str(All_pathways_RLS_product[i]) == str(pathway_t[j+1]):
                        if (pathway_len % 2 == 0) and j == (pathway_len/2 - 1):
                            if (arrow_type == 'single'):
                                ...
                                f.write('edge[dir="both",style="dashed",penwidth="' + str(linewidth_d) + '",labeldistance=5, labelangle=-40, arrowhead = lnormal, arrowtail = lnormal,color=' + str(color_scheme[i+1]) + ',fontcolor=' + str(color_scheme[i+1]) + ',weight=2,headlabel="               ' + str(pathway_fwd_r[j]) + str('*') + '",label = " ", taillabel="' + str(pathway_rev_r[j]) + str('*') +'             "];\n')
                            f.write('"' + str(pathway_t[j]) + '" -> "' + str(pathway_t[j+1]) + '"[constraint=false];\n')
                        else:
                            if (arrow_type == 'single'):    
                                f.write('edge[dir="forward",style="dashed",penwidth="' + str(linewidth) + '",color=' + str(color_scheme[i+1]) + ',fontcolor=' + str(color_scheme[i+1]) + ',weight=2,label="   ' + str(pathway_r[j]) + str('*') + '"];\n')
                            elif (arrow_type == 'double'):
                                f.write('edge[dir="both",style="dashed",penwidth="' + str(linewidth_d) + '",labeldistance=10, labelangle=-30, arrowhead = lnormal, arrowtail = lnormal,color=' + str(color_scheme[i+1]) + ',fontcolor=' + str(color_scheme[i+1]) + ',weight=2,headlabel="  ' + str(pathway_fwd_r[j]) + str('*') + '   ",label = " ", taillabel="     ' + str(pathway_rev_r[j]) + str('*') +'"];\n')
                            f.write('"' + str(pathway_t[j]) + '" -> "' + str(pathway_t[j+1]) + '"\n')
                    else:
                        if (pathway_len % 2 == 0) and j == (pathway_len/2 - 1):
                            if (arrow_type == 'single'):
                                ...
                                f.write('edge[dir="both",style="solid",penwidth="' + str(linewidth_d) + '",labeldistance=5, labelangle=-40, arrowhead = lnormal, arrowtail = lnormal, color=' + str(color_scheme[i+1]) + ',fontcolor=' + str(color_scheme[i+1]) + ',weight=2,headlabel="            ' + str(pathway_fwd_r[j]) + '",label = " ", taillabel="' + str(pathway_rev_r[j]) +'          "];\n')
                            f.write('"' + str(pathway_t[j]) + '" -> "' + str(pathway_t[j+1]) + '"[constraint=false];\n')
                        else:
                            if (arrow_type == 'single'):
                                f.write('edge[dir="forward",style="solid",penwidth="' + str(linewidth) + '",color=' + str(color_scheme[i+1]) + ',fontcolor=' + str(color_scheme[i+1]) + ',weight=2,label="   ' + str(pathway_r[j]) + '"];\n')
                            elif (arrow_type == 'double'):
                                f.write('edge[dir="both",style="solid",penwidth="' + str(linewidth_d) + '",labeldistance=10, labelangle=-30, arrowhead = lnormal, arrowtail = lnormal, color=' + str(color_scheme[i+1]) + ',fontcolor=' + str(color_scheme[i+1]) + ',weight=2,headlabel="    ' + str(pathway_fwd_r[j]) + '   ",label = " ", taillabel="    ' + str(pathway_rev_r[j]) +'"];\n')
                            f.write('"' + str(pathway_t[j]) + '" -> "' + str(pathway_t[j+1]) + '"\n')
                    ...
                    if str(All_pathways_RLS_reactant[i]) == str(pathway_t[j]) and str(All_pathways_RLS_product[i]) == str(pathway_t[j+1]):
                        if (arrow_type == 'single'):
                            f.write('edge[dir="back",style="dashed",penwidth="' + str(linewidth) + '",color=' + str(color_scheme[i+1]) + ',fontcolor=' + str(color_scheme[i+1]) + ',weight=2,label="   ' + str(pathway_r[j]) + str('*') + '"];\n')
                                ...
                                f.write('edge[dir="both",style="dashed",penwidth="' + str(linewidth_d) + '",labeldistance=10, labelangle=-30, arrowhead = lnormal, arrowtail = lnormal,color=' + str(color_scheme[i+1]) + ',fontcolor=' + str(color_scheme[i+1]) + ',weight=2,headlabel="  ' + str(pathway_rev_r[j]) + str('*') + '    ",label = " ", taillabel="     ' + str(pathway_fwd_r[j]) + str('*') +'"];\n')
                        ...
                        if (arrow_type == 'single'):
                            ...
                            f.write('edge[dir="both",style="solid",penwidth="' + str(linewidth_d) + '",labeldistance=10, labelangle=-30, arrowhead = lnormal, arrowtail = lnormal, color=' + str(color_scheme[i+1]) + ',fontcolor=' + str(color_scheme[i+1]) + ',weight=2,headlabel="    ' + str(pathway_rev_r[j]) + '    ",label = " ", taillabel="    ' + str(pathway_fwd_r[j]) +'"];\n')
                        f.write('"' + str(pathway_t[j+1]) + '" -> "' + str(pathway_t[j]) + '"\n')
            ...
            #Species_Processed[i] = Species_Processed[i].strip('\'')
                if (arrow_type == 'single'):    
                    ...
                    f.write('"' + str(spec_proc_temp[n]) + '"[width=5, height=5, margin=1,fixedsize=true, imagescale = true, shape=box,image="' + str(species_directory[int(spec_proc_temp[n])]) + '.png", label = "",URL="' + str("Species_pathways\\") + str(int(spec_proc_temp[n])) + '.svg"];\n')
