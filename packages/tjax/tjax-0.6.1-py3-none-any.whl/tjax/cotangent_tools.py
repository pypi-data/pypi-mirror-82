...
transform_cotangent = custom_vjp(transform_cotangent, static_argnums=0)
...
def _transform_cotangent_bwd(residuals: Callable[[X], X], x_bar: X) -> X:
    f = residuals
    return (f(x_bar),)


transform_cotangent.defvjp(_transform_cotangent_fwd, _transform_cotangent_bwd)


def mapped_transform_cotangent(f: Callable[[Array], Array], x: X) -> X:
    def f(x_bar: X) -> X:
        return tree_map(f, x_bar)
