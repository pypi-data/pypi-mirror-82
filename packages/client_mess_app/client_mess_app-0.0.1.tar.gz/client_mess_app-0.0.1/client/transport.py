""" Транспортная подсистема клиентского модуля """
import binascii
import hashlib
import hmac
import socket
...
from logs.configs_logs import config_client_log
...
class ClientTransport(threading.Thread, QObject):
    ...
    """
    # Сигналы новое сообщение, 205, потеря соединения
    ...
    def connection_init(self, port, ip):
        ...
        with socket_lock:
            ...
            try:
                message_out(self.transport, presence)
                ans = message_in(self.transport)
                ...
                if RESPONSE in ans:
                    if ans[RESPONSE] == 511:
                        ...
                        my_ans[DATA] = binascii.b2a_base64(
                            digest).decode('ascii')
                        message_out(self.transport, my_ans)
                        self.process_server_ans(message_in(self.transport))
    ...
    def contacts_list_update(self):
        ...
        with socket_lock:
            message_out(self.transport, req)
            ans = message_in(self.transport)
    ...
    def transport_shutdown(self):
        ...
        with socket_lock:
            try:
                message_out(self.transport, message)
            except OSError:
                pass
        client_logger.debug('Транспорт завершает работу.')
        time.sleep(0.5)

    def message_out(self, to, message):
        ...
        with socket_lock:
            message_out(self.transport, message_dict)
            self.process_server_ans(message_in(self.transport))
        ...
        while self.running:
            ...
            with socket_lock:
                try:
                    ...
                    message = message_in(self.transport)
                except OSError as err:
                    if err.errno:
                        client_logger.critical('Потеряно соединение с сервером.')
                        self.running = False
