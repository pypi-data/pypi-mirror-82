...
from savvihub.common.config import config as global_config
from savvihub import history


class Experiment:
    def __init__(self, experiment_id, client):
        self.experiment_id = experiment_id
        self.client = client
        self._history = None

    @classmethod
    def from_given_or_global_config(cls, config, client):
        if not config:
            config = global_config
        experiment = cls(config.experiment_id, client)
        return experiment
    ...
    def log(self, step, row=None):
        ...
        self.history.update(client, step, row)
