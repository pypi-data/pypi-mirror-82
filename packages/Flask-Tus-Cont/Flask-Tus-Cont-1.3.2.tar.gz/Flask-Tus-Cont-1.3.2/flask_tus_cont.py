...
class TusManager(object):
    ...
    def init_app(self, app,**kwargs):
        if (hasattr(app, 'config')):
            ...
            setattr(self, 'upload_folder', app.config.get('UPLOAD_FOLDER', 'uploads/'))
            ...
            setattr(self, 'tus_redis_url', app.config.get('REDIS_URL', 'redis://'))
    ...
    def redis_connect(self):
        if (hasattr(self, 'tus_redis') == False):
            return redis.Redis.from_url(self.tus_redis_url)
        return redis.Redis()
