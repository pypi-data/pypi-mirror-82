...
class NodeRunner:
    ...
    def generate_node_function(self, arguments):
        ...
        return f"""
            module.paths.push('{os.path.join(os.getcwd(), self.function_name, "node_modules")}');
    ...
    def get_handler_location(self):
        ...
        if self.function_type == "algorithm":
            return os.path.join(os.getcwd(), self.function_name, *handler_definition.split(".")[:-1]), function_name
        if self.function_type == "integration":
            root = self.function_config.get("function").get("root", "app")
            return os.path.join(os.getcwd(), root, *handler_definition.split(".")[:-1]), function_name
