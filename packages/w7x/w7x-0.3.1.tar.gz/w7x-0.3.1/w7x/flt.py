...
class ConnectionLength(Points3D):
    ...
    def create_test(cls):
        ...
        a = []
        a.append(
            C(length=899.895318339, x=1.0, y=0, z=0.974383343584, part=0, element=44301)
        )
        a.append(
            C(length=2826.55635289, x=0, y=1, z=0.95792010423, part=1, element=20291)
        )
        a.append(C(length=2.55635289, x=-80, y=1, z=1.9573, part=2, element=2))
        a.append(
            C(
        ...
        a.append(
            C(length=2826.55635289, x=0, y=-1, z=0.95792010423, part=4, element=20291)
        )
        obj = cls(a)
...
class ComponentLoad(object):
    ...
    def __init__(self, component_load, mm_ids=None):
        ...
        for cl in component_load:
            if cl.id not in self._component_id_dict:
                self._component_id_dict[cl.id] = {}
            for element in cl.elements:
                if element.id not in self._component_id_dict[cl.id]:
                    self._component_id_dict[cl.id][element.id] = element.events
                else:
                    self._component_id_dict[cl.id][element.id] += element.events
    ...
    def create_test(cls):
        ...
        a.append(
            C(
                ...
                events=3,
                elements=[
                    C(id=103638, events=1, area=0.00017),
                    C(id=123456, events=1, area=0.00017),
                    C(id=134679, events=1, area=0.00017),
                ],
            )
        )
        a.append(C(id=3, events=21, elements=[C(id=123, events=21, area=0.00017)]))
        a.append(C(id=0, events=41, elements=[C(id=123456, events=41, area=0.00017)]))
        ...
        **kwargs
        ...
        for iShift, additionalShift in enumerate(additiona_shifts):
            ...
            if len(resForwardClass) == nStart:
                logging.info("Length {} accepted!".format(len(resForwardClass)))
                ...
                logging.warning(
                    ...
                    "does not meet opts.nRevolutions({2})".format(
        ...
        connection_length = ConnectionLength(
            resForward.connection + resInvers.connection
        )
        connection_length.mm_ids = self.machine.mm_ids

        component_load = ComponentLoad(
            resForward.loads.components + resInvers.loads.components
        )
