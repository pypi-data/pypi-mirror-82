...
############################################################
def check_download_atac_seq_file(atac_seq_file):

    current_abs_path = os.path.dirname(os.path.abspath(__file__))
    #We need  filename not full path
    os.makedirs(os.path.join(current_abs_path,'lib','epigenomics'),exist_ok=True)
    lib_epigenomics_path = os.path.join(current_abs_path,'lib','epigenomics')

    if os.path.isabs(lib_epigenomics_path):
        os.chdir(lib_epigenomics_path)

        atac_seq_signal_file_path= os.path.join(lib_epigenomics_path,atac_seq_file)

        if not os.path.exists(atac_seq_signal_file_path):
            print('Does not exists: %s' %(atac_seq_signal_file_path))
            try:
                print('Downloading %s under %s' % (atac_seq_file, lib_epigenomics_path))
                ...
                cmd="bash -c '" + 'wget -r -l1 -c -nc --no-parent -nd ftp://alexandrovlab-ftp.ucsd.edu/pub/tools/SigProfilerTopography/lib/epigenomics/' + atac_seq_file + "'"
                # print(cmd)
                os.system(cmd)
            except:
        ...
        print('%s is not an absolute path.' %(lib_epigenomics_path))
...
def runAnalyses(genome,
                ...
                epigenomics_files=[DEFAULT_HISTONE_OCCUPANCY_FILE1,DEFAULT_HISTONE_OCCUPANCY_FILE2,DEFAULT_HISTONE_OCCUPANCY_FILE3,DEFAULT_HISTONE_OCCUPANCY_FILE4,DEFAULT_HISTONE_OCCUPANCY_FILE5,DEFAULT_HISTONE_OCCUPANCY_FILE6,DEFAULT_CTCF_OCCUPANCY_FILE,DEFAULT_ATAC_SEQ_OCCUPANCY_FILE],
    ...
    default_epigenomics_files = [DEFAULT_HISTONE_OCCUPANCY_FILE1,
                                 DEFAULT_HISTONE_OCCUPANCY_FILE2,
                                 DEFAULT_HISTONE_OCCUPANCY_FILE3,
                                 DEFAULT_HISTONE_OCCUPANCY_FILE4,
                                 DEFAULT_HISTONE_OCCUPANCY_FILE5,
                                 DEFAULT_HISTONE_OCCUPANCY_FILE6,
    ...
    if (set(epigenomics_files) == set(default_epigenomics_files)):
        ...
        epigenomics_files_memos = ['H3K27me3_Breast_Epithelium',
                                   'H3K36me3_Breast_Epithelium',
                                   'H3K9me3_Breast_Epithelium',
                                   'H3K27ac_Breast_Epithelium',
                                   'H3K4me1_Breast_Epithelium',
                                   'H3K4me3_Breast_Epithelium',
                                   'CTCF_Breast_Epithelium',
                                   'ATAC-Seq_Breast_Epithelium']
        ...
        #If default ATAC-Seq file is not downloaded yet download here
        check_download_atac_seq_file(DEFAULT_ATAC_SEQ_OCCUPANCY_FILE)
    ###############################################
