    ...
    if (!r.targets.swapped)
    {
        ...
        if (in_field(sub(ball_location, multiply(r.best_shot_vector, double_to_vector(best_shot_value))), 1))
        {
            ...
            if (slope > 0) {
                ...
                if (is_viable.forward || (is_viable.backward && slope > 0.5))
                    r.found = 1;
            }
        }
    }
...
struct ground_shot parse_slice_for_ground_shot_with_target(double time_remaining, double best_shot_value, double boost_accel, Vector ball_location, struct car me, Vector left_target, Vector right_target)
{
    ...
    if (!r.targets.swapped)
    {
        ...
        if (in_field(sub(ball_location, multiply(best_shot_vector, double_to_vector(best_shot_value))), 1) && find_slope(best_shot_vector, car_to_ball) > 0)
