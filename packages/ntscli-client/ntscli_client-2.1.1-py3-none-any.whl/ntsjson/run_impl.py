...
def run_impl(
    ...
    with stateful_session_mgr(**dict(configuration=configuration, **session_target.to_dict())) as session:
        ...
        should_remove_eyepatch_tests = should_remove_eyepatch_tests_decision(force_keep_eyepatch, rae, session)

        filter_testcases(batch, categories, session.plan_request, not should_remove_eyepatch_tests, names, names_re, tags)
...
def should_remove_eyepatch_tests_decision(force_keep_eyepatch: bool, rae: str, session: StatefulSession):
    """
    Should I remove eyepatch tests?

    :param force_keep_eyepatch: bool from the command line
    :param rae:
    ...
    try:
        ...
        logger.info("There was a problem getting the list of EyePatch connected devices, so the test plan will keep EyePatch tests")
