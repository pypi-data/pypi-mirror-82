...
#数据标准化
def data_scale(df, factor_cols, method = 'normal', capcol = 'S_DQ_MV'):
    if method == 'normal':
        df_proc = df.loc[:, factor_cols]
        df_proc = (df_proc - df_proc.mean()) / df_proc.std()
        df.loc[:, factor_cols] = df_proc
    elif method == 'cap':
        df = df.copy()
        cap_weight = df[capcol] / df[capcol].sum()
        for name in factor_cols:
            avg = (df[name] * cap_weight).sum()
            df[name] = (df[name] - avg) / df[name].std()
    return df
