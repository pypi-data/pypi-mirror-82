...
class Lims:
    ...
    def __init__(self, root):
        self.root = root
        self.directories = [os.path.join(root, directory) for directory in
    ...
    def tools_generate_used_techniques_from_raw(self, path):
        technique_lims = {}
    ...
    def write_ftir(self):
        ftir_out = self.frame.Name.apply(lambda x: f"{self.TODAY.strftime('%Y%m%d')}_{x}")
        ...
        for path in self.directories:
            for file in os.listdir(path):
                try:
                    ...
                    continue
                if tech := [d for d in re_extract(path, self.techniques - set('EDS'))]:
                    tech = tech[0]
                else:
                    continue
