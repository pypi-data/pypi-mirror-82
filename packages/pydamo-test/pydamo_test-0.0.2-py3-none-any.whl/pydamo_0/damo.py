...
class DM:
    def __init__(self):
        try:
            ...
            try:
                tt.tqdm_sleep('启用[方案1-regsvr32]自动注册中...')
                os.system('regsvr32 dm.dll /s')
                self.dm = Dispatch('dm.dmsoft')  # 调用大漠插件
                print('自动注册dm.dll - os.system方案')
            except:
                try:
                    tt.tqdm_sleep('启用[方案2-admin权限]自动注册中...')
                    subprocess.call(cmd='regist_dm_admin.bat', shell=True)
                    self.dm = Dispatch('dm.dmsoft')  # 调用大漠插件
                    print('自动注册dm.dll - 管理员权限方案')
                except:
    ...
    if (1):
        ...
        def Beep(self, duration=1000, f=1000):
