...
#        fut_transfers = pd.read_pickle(local_root_directory + transfers_pkl_file_name)
    ...
    funding_df = funding_df[funding_df['Date'] <= maxDate].copy()
    ...
    if not by_pass_month:
        filtered_binance_df=filter_me_month(filtered_binance_df.copy(),me_month)
    ...
    finalFilteredWallet['Date'] = finalFilteredWallet['Date'].dt.strftime('%m/%d/%Y')
    final_converted_history_balance['Date'] = final_converted_history_balance['Date'].dt.strftime('%m/%d/%Y')

    filtered_binance_df['Date'] = filtered_binance_df['Date'].dt.strftime('%m/%d/%Y')

    filtered_binance_df['Value'] = filtered_binance_df['Date']
    filtered_binance_df['Settlement'] = filtered_binance_df['Date']

    filtered_binance_df['Depo'] = 'Binance'
    filtered_binance_df['Bank'] = 'Binance'

    filtered_binance_df['Pmt Cur'] = 'USDT'
    filtered_binance_df['Source'] = 'Manual'

    funding_df['Date'] = funding_df['Date'].dt.strftime('%m/%d/%Y')

    funding_df['Value'] = funding_df['Date']
    funding_df['Settlement'] = funding_df['Date']

    funding_df['Depo'] = 'Binance'
    funding_df['Bank'] = 'Binance'

    funding_df['Pmt Cur'] = 'USDT'
    funding_df['Source'] = 'Manual'

    return filtered_binance_df, finalFilteredWallet, final_converted_history_balance, actualPositions, funding_df
