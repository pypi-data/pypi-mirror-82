...
class rfic_models:
    ...
    def fcf_dcf_using_wacc(self, RFR, mkt_return, SR_g, LR_g, shock):
        ...
        # parser = Financials(subjectCompany)
        if shock:
            output = self.dcf_n1Shock(fcf(parser.cash_flow_statement), aapl_wacc, shock, short_term_growth=SR_g,
                                 long_term_growth=LR_g)
        else:
            output = self.dcf(fcf(parser.cash_flow_statement), aapl_wacc, short_term_growth=SR_g, long_term_growth=LR_g)
    ...
    def dcf_g_range_simple(self, RFR, mkt_return, SR_g_scenarios, LR_g):
        ...
        for i in range(len(SR_g_scenarios)):
            ...
            records_dcf_1.append(self.fcf_dcf_using_wacc(RFR, mkt_return, SR_g, LR_g, False))
            # print(records_dcf_1)
    ...
    def dcf_g_range_shock(self, RFR, mkt_return, SR_g_scenarios, LR_g, revenue_growth_decline_rate_shock):
        ...
        for i in range(len(SR_g_scenarios)):
            time.sleep(2)
            SR_g = SR_g_scenarios[i]
            records_dcf_1.append(self.fcf_dcf_using_wacc(RFR, mkt_return, SR_g, LR_g, revenue_growth_decline_rate_shock))
            # print(records_dcf_1)
            print("Done computing ", (i + 1), ' DCF out of ', len(SR_g_scenarios))
    ...
    def peer_dcf_comparison(self, RFR, mkt_return, LR_g, SR_g,shock):
        ...
        for i in tickerList:
            ...
            try:
                # time.sleep(1)
                records_dcf_2.append(self.fcf_dcf_using_wacc(RFR, mkt_return, SR_g, LR_g, shock))
                tList.append(i)
            # print(records_dcf_1)
