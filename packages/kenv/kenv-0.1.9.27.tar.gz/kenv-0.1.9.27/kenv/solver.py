...
class Equations:
    ...
    def particle_prime(self,
        ...
        P = 2*self.beam.current / (alfven_current * (g*beta)**3)
        #if (x*x + y*y > envelope_x(z)**2 + envelope_y(z)**2) and (x != 0.0 and y != 0):
        #    dxdz = xp
        ...
        if (x*x + y*y > envelope_x(z)**2 + envelope_y(z)**2) and (x != 0.0 and y != 0):
            dxdz = xp
            dxpdz = 2*P * ((envelope_x(z)+envelope_y(z))/2*envelope_x(z)) / x - \
                    dgdz*xp / (beta*beta*g) - d2gdz2*x / (2*beta*beta*g) + \
                    Bz/Brho * yp + dBzdz/(2*Brho) * y
            dydz = yp
            dypdz = 2*P * ((envelope_x(z)+envelope_y(z))/2*envelope_y(z)) / y - \
                    dgdz*yp / (beta*beta*g) - d2gdz2*y / (2*beta*beta*g) - \
                    Bz/Brho * xp - dBzdz/(2*Brho) * x
        else:
            dxdz = xp
            dxpdz = 2*P / ((envelope_x(z)+envelope_y(z))/2*envelope_x(z)) * x - \
                    dgdz*xp / (beta*beta*g) - d2gdz2*x / (2*beta*beta*g) + \
