...
class Equations:
    ...
    def particle_prime(self,
        ...
        if x*x + y*y >= envelope_x(z)**2 + envelope_y(z)**2:
            dxdz = xp
            dxpdz = 2*P * (envelope_x(z)+envelope_y(z))/(envelope_x(z) * 2*x) - K_x*x - \
                    dgdz*xp / (beta*beta*g) - d2gdz2*x / (2*beta*beta*g)
            dydz = yp
            dypdz = 2*P * (envelope_x(z)+envelope_y(z))/(envelope_y(z) * 2*y) - K_y*y - \
                    dgdz*yp / (beta*beta*g) - d2gdz2*y / (2*beta*beta*g)
        else:
            dxdz = xp
            dxpdz = 2*P / ((envelope_x(z)+envelope_y(z))*envelope_x(z)) * 2*x - K_x*x - \
                    dgdz*xp / (beta*beta*g) - d2gdz2*x / (2*beta*beta*g)
            dydz = yp
            dypdz = 2*P / ((envelope_x(z)+envelope_y(z))*envelope_x(z)) * 2*y - K_y*y - \
                    dgdz*yp / (beta*beta*g) - d2gdz2*y / (2*beta*beta*g)
