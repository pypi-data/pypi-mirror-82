...
# ----------
# to use this:
#   python -m trlocal_backup transmission/conf dest_location
# ----------
...
from . import backup_transmission
...
def main(argv=None):
    ...
    logger = logging.getLogger('trlocal.backup')
    ...
    args = argv[1:]
    if len(args) != 2:
        return exit('module trlocal_backup required 2 parameters: <TRANSMISSION_CONF> <DEST_LOCATION>')
    trconf_location, dest_dir = args

    try:
        if not os.path.isdir(trconf_location):
            return exit(f'{trconf_location} is not a dir.')
