...
class SoeaAlgorithm(Algorithm):  # 单目标优化算法模板父类
    ...
    def draw(self, pop, EndFlag=False):
        ...
        if not EndFlag:
            ...
            # 绘制最终结果图
            if self.drawing != 0:
                metric = np.vstack([self.trace['f_avg'], self.trace['f_best']]).T
    ...
    def terminated(self, pop):
        ...
        self.timeSlot = time.time()  # 更新时间戳
        # 判断是否终止进化，由于代数是从0数起，因此在比较currentGen和MAXGEN时需要对currentGen加1
        if (self.MAXTIME is not None and self.passTime >= self.MAXTIME) or self.currentGen + 1 >= self.MAXGEN or self.trappedCount >= self.maxTrappedCount:
            return True
        else:
            self.currentGen += 1  # 进化代数+1
            return False
