...
class DockerService:
    ...
    def list_images(self):
        return [
            ...
            if di.labels.get('app', '') == 'pier'
        ]

    def pull_image(self, tag):
        fileobj = io.BytesIO(bytes(f'FROM {tag}', 'utf-8'))
        self.client.images.build(fileobj=fileobj, tag=tag, labels=self.APP_LABEl)

    def build_image(self, tag):
        path = os.path.abspath(os.getcwd())
        self.client.images.build(path=path, tag=tag, labels=self.APP_LABEl)
