...
class PodService:
    ...
    def create_service(self, deployment_name, ports, namespace='default'):
        labels = {"name": deployment_name, **self.APP_LABEL }

        ports = [{
            "name": f'{deployment_name}-{port}',
            "port": port,
            "targetPort": port
        } for port in ports]
        ...
        return self.core_v1_api.create_namespaced_service(namespace, body)

    def delete_service(self, deployment, namespace='default'):
        try:
