...
def mo_create(config_path='D:\data\py_data\conf.ini',kdc_url='http://47.103.221.12:8126/k3cloud',
    ...
    if (res['Result']['ResponseStatus']['IsSuccess']):
        info ={'status':True,'msg':res['Result']['ResponseStatus']['SuccessEntitys'][0]['Number'] +'生成成功!'}
    else:
        info = {'status': False, 'msg':  '生产订单生成失败!'}
    # 返回结果
    return (info)
# 复制产生新的生产订单
def mo_copy(config_path='D:\data\py_data\conf.ini',kdc_url='http://47.103.221.12:8126/k3cloud',
    ...
    if data_copy is None:
        info = {'status': False, 'msg':  '生产订单生成失败,源单' + FBillNo + '不存在！'}
    else:
        info = mo_create(config_path=config_path,kdc_url=kdc_url,data=data_copy,FBillType=FBillType)
    # 返回结果
    return (info)
# 修改现有的生产订单
def mo_update(config_path='D:\data\py_data\conf.ini',kdc_url='http://47.103.221.12:8126/k3cloud',
    ...
    # 返回结果
    if (res['Result']['ResponseStatus']['IsSuccess']):
        info ={'status':True,'msg':res['Result']['ResponseStatus']['SuccessEntitys'][0]['Number'] +'保存成功!'}
    else:
        info = {'status': False, 'msg':  '生产订单保存失败!'}
    # 返回结果
    return (info)
# 重新保存生产订单
...
def mo_submit(config_path='D:\data\py_data\conf.ini',kdc_url='http://47.103.221.12:8126/k3cloud',
    ...
    #print(res)
    if (res['Result']['ResponseStatus']['IsSuccess']):
        info ={'status':True,'msg':res['Result']['ResponseStatus']['SuccessEntitys'][0]['Number'] +'提交成功!'}
    else:
        info = {'status': False, 'msg': res['Result']['ResponseStatus']['Errors'][0]['Message'] }
    # 返回结果
    return (info)
# 生产订单审核
def mo_audit(config_path='D:\data\py_data\conf.ini',kdc_url='http://47.103.221.12:8126/k3cloud',
    ...
    print(res)
    if (res['Result']['ResponseStatus']['IsSuccess']):
        info = {'status': True, 'msg': FBillList[0] + '审核成功!'}
    else:
        info = {'status': False, 'msg': res['Result']['ResponseStatus']['Errors'][0]['Message']}
    # 返回结果
    return (info)
...
def mo_unAudit(config_path='D:\data\py_data\conf.ini',kdc_url='http://47.103.221.12:8126/k3cloud',
    ...
    print(res)
    if (res['Result']['ResponseStatus']['IsSuccess']):
        info = {'status': True, 'msg': res['Result']['ResponseStatus']['SuccessEntitys'][0]['Number'] + '反审核成功!'}
    else:
        info = {'status': False, 'msg': res['Result']['ResponseStatus']['Errors'][0]['Message']}
    # 返回结果
    return (info)
...
def mo_delete(config_path='D:\data\py_data\conf.ini',kdc_url='http://47.103.221.12:8126/k3cloud',
    ...
    res = json.loads(r)
    if (res['Result']['ResponseStatus']['IsSuccess']):
        info = {'status': True, 'msg': res['Result']['ResponseStatus']['SuccessEntitys'][0]['Number'] + '删除成功!'}
    else:
        info = {'status': False, 'msg': res['Result']['ResponseStatus']['Errors'][0]['Message']}
    # 返回结果
    return (info)
...
def mo_release(config_path='D:\data\py_data\conf.ini',kdc_url='http://47.103.221.12:8126/k3cloud',
    ...
    print(res)
    if res['Result']['ResponseStatus']['IsSuccess']:
        if len(res['Result']['ResponseStatus']['SuccessEntitys']) > 0:
            info = {'status': True, 'msg': ''.join(FBillList) + '生产订单下达成功!'}
        else:
            info = {'status': False, 'msg': ''.join(FBillList) + '生产订单下达失败!'}
    else:
        info = {'status': False, 'msg': ''.join(FBillList) + '生产订单下达失败!'}
    # 返回结果
    return (info)
...
if __name__ == '__main__':
    ...
    # print(mo_reSave(FBillNo='MO033541'))
   #print(str(datetime.datetime.now()) +'系统自动更新')
    ...
    # print(mo_submit(  FBillList=['MO033992']))
    # print(mo_delete(FBillList=['MO033992']))
    print(mo_release(FBillList=['MO033541']))