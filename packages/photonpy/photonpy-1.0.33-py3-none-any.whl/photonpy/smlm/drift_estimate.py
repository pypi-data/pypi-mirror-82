...
def minEntropy(positions, framenum, crlb, framesperbin, imgshape, 
    ...
    with Context(debugMode=debugMode) as ctx:
        ...
        with tqdm.tqdm() as pbar:
            ...
            coarse_drift,score = ppm.MinEntropyDriftEstimate(
                positions, framenum, initial_drift*1, sigmaPrecise, numIterations, step, maxdrift, 
        ...
        with tqdm.tqdm() as pbar:
            ...
            drift,score = ppm.MinEntropyDriftEstimate(
                positions, framenum, coarse_drift*1, crlb, numIterations, step, maxdrift, framesPerBin=framesperbin, 
        ...
        with tqdm.tqdm() as pbar:
            ...
            drift_set1,score_set1 = ppm.MinEntropyDriftEstimate(
                positions[set1], framenum[set1], coarse_drift*1, crlb_set1, numIterations, step, maxdrift, 
                framesPerBin=framesperbin,cuda=useCuda, progcb=update_pbar)

            drift_set2,score_set2 = ppm.MinEntropyDriftEstimate(
                positions[set2], framenum[set2], coarse_drift*1, crlb_set2, numIterations, step, maxdrift, 
                framesPerBin=framesperbin,cuda=useCuda,progcb=update_pbar)
        ...
        if display:
            ...
            def plotwnd(L):
                ...
                for i in range(ndims):
                    ...
                    ax[i].plot(coarse_drift[:L,i], label=f'Coarse Drift {axname}, binsize={fpbCoarse}')
