...
class Dataset:
    ...
    def estimateDriftMinEntropy(self, framesPerBin=10,maxdrift=3, dims=None, **kwargs):
        if dims is None:
            dims = self.dims
    ...
    def simulateBlinking(self, numframes, blinkGenerator, drift=None):
        ...
        for f,ontimes in enumerate(tqdm.tqdm(blinkGenerator, total=numframes)):
    ...
    def simulateMovie(self, psf : Estimator, tiff_fn, background=5, drift=None):
        ...
        with MultipartTiffSaver(tiff_fn) as tifwriter:
            ...
            for f,idx in enumerate(tqdm.tqdm(ipf, total=len(ipf))):
