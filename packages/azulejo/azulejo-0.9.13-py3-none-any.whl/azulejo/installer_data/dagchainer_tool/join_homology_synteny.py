...
def join_homology_synteny(synfile, homfile):
    ...
    outpath = synpath.parent / (synpath.name[:-len(synpath.suffix)] + ".hom.tsv")
    ...
    for cluster_id, subframe in syn.groupby(by=["cluster_id"]):
        ...
        if len(hom_counts) == 1:
            n_consistent += 1
    frac_inconsistent = (n_clusts - n_consistent) * 100. / n_clusts
    print(f"{n_consistent} consistent out of {n_clusts} clusters, {frac_inconsistent}% inconsistent")
