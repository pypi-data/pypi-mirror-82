...
class DependencyInstaller(object):
    ...
    def check_all(self, exe_paths=False):
        ...
        all_required = all(
            [
                ...
                for d in self.dependency_dict if self.dependency_dict[d]["required"]
            ]
        )
        all_optional = all(
            [
                self.dependency_dict[d]["installed"]
                for d in self.dependency_dict if not self.dependency_dict[d]["required"]
            ]
        )
        if all_required:
            if all_optional:
                ...
                optional_str = ""
            self.status_msg += f"All required {optional_str} dependencies are installed.\n"
        return all_required
    ...
    def install_list(self, deplist):
        ...
        self.check_all()
        if deplist == ("all",):
            deplist = [d for d in self.dependencies
