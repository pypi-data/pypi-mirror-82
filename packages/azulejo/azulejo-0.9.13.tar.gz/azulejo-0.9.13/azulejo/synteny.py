...
def synteny_anchors(
    ...
    anchor_mb.delete()
    anchor_frame = pd.DataFrame.from_dict([a for a in anchor_stats if a is not None])
...
def unique_anchors(setname, k):
    ...
    pos_set = [anchors[f"anchor.{i}.hash"].to_numpy().astype(int) for i in range(k)]
    intersections = [set(pos_set[i] ) & set( pos_set[i+1]) for i in range(k-1)]
    ...
    for i, row in anchors.iterrows():
        for pos in range(k-1):
            ...
            if hashval in intersections[pos]:
                ...
                n_uniq += row["n"]
    frac_unique = n_uniq * 100./n_total
