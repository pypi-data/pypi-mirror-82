...
def ingest_sequences(input_toml, click_loguru=None):
    ...
    if parallel:
        file_stats = bag.map(read_fasta_and_gff,
                             verbose=options.verbose).compute()
    else:
    ...
    logger.info(f'Edit table of proteomes at "{proteome_table_path}"' +
    ...
    if not frags_path.exists():
        logger.info(f'Edit fragment table at "{frags_path}" to rename fragments')
...
def read_fasta_and_gff(args, verbose=False):
    ...
    if n_fasta < MINIMUM_PROTEINS:
        ...
        logger.error(f"Number of proteins read {len(fasta_props)} is too small")
        sys.exit(1)
    ...
    for feat_type in POSSIBLE_FEATURES:
        filtered_features = annotation.filter_feature_of_type(
            [feat_type]
        )
        n_features = len(filtered_features.df)
        if n_features > MINIMUM_PROTEINS:
            if verbose:
                logger.debug(f"Using {n_features} features of type '{feat_type}' in {gff_url}")
            break
    if n_features < MINIMUM_PROTEINS:
        logger.error(f"Not enough {n_features} of types" +
    ...
    try:
        ...
        logger.error(f"Badly-formed {feat_type} features in GFF file {gff_url}")
        sys.exit(1)
    ...
    for id_col in POSSIBLE_ID_COLS:
        ...
        if n_joint > MINIMUM_PROTEINS:
            if verbose:
                logger.debug(f"Found {n_joint} FASTA IDs in GFF feature '{id_col}'")
            n_missing_in_gff = n_fasta - n_joint
            if n_missing_in_gff > 0:
                logger.warning(f"{n_missing_in_gff} FASTA IDs not found in GFF file {gff_url}")
            break
    n_uniq_features = len(feat_cols)
    proteome_stats["gff.dups"] = n_features - n_uniq_features
    if n_joint < MINIMUM_PROTEINS:
        logger.error(f"Not enough ID's found ({n_joint}) that match FASTA IDS" +
