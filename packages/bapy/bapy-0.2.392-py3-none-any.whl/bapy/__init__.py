...
class Env(Asdict):
    ...
    def __init__(self, prefix: Text = str()):
        ...
        self.context_ping = contextvars.ContextVar('ping')
        ...
        self.context_ping.set(asyncio.Semaphore(self.file.int(upper_prefix('PING', prefix=self.prefix), Sem.ping)))
    ...
    async def max(self):
        s = self.context_max.get()
        try:
            s.acquire()
            self.context_max.set(s)
            yield s
        finally:
            s.release()
            self.context_max.set(s)
    ...
    async def atlas(self):
        s = self.context_atlas.get()
        try:
            s.acquire()
            self.context_atlas.set(s)
            yield s
        finally:
            s.release()
            self.context_atlas.set(s)
    ...
    @contextlib.asynccontextmanager
    async def ssh(self):
        s = self.context_ssh.get()
        try:
            s.acquire()
            self.context_ssh.set(s)
            yield s
        finally:
            s.release()
            self.context_ssh.set(s)
    ...
    @contextlib.asynccontextmanager
    async def os(self):
        s = self.context_os.get()
        try:
            s.acquire()
            self.context_os.set(s)
            yield s
        finally:
            s.release()
            self.context_os.set(s)
    ...
    @contextlib.asynccontextmanager
    async def nmap(self):
        s = self.context_nmap.get()
        try:
            s.acquire()
            self.context_nmap.set(s)
            yield s
        finally:
            s.release()
            self.context_nmap.set(s)
    ...
    @contextlib.asynccontextmanager
    async def ping(self):
        s = self.context_ping.get()
        try:
            s.acquire()
            self.context_ping.set(s)
            yield s
        finally:
            s.release()
            self.context_ping.set(s)
