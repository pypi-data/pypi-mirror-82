...
async def is_global_ipl(ip: Text, sem_max: asyncio.Semaphore = asyncio.Semaphore(),
                        sem: asyncio.Semaphore = asyncio.Semaphore()) -> OptionalBool:
    """
    Checks if Valid and Global IP.

    Args:
        ip: ip.
        sem_max: max process sem.
        sem: ping.
...
async def is_ip_up(ip: Text, sem_max: asyncio.Semaphore = asyncio.Semaphore(),
                   sem: asyncio.Semaphore = asyncio.Semaphore()) -> OptionalBool:
    """
    Pings host.
    ...
    pings = 3
    if await is_global_ipl(ip, sem_max, sem):
        async with sem_max:
            async with sem:
