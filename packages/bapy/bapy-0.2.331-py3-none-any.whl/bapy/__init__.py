...
PROTO_IP = Proto(socket.IPPROTO_TCP, socket.IPPROTO_UDP)
...
class Sem:
    ATLAS: UnionTextInt = 499
    DB: UnionTextInt = 500
    IO: UnionTextInt = 500
    MAX: UnionTextInt = 1000
    NMAP: UnionTextInt = 50
    OS: UnionTextInt = 400
    PING: UnionTextInt = 200
    SOCKET: UnionTextInt = 200
    atlas: asyncio.Semaphore = dataclasses.field(init=False)
    db: asyncio.Semaphore = dataclasses.field(init=False)
    io: asyncio.Semaphore = dataclasses.field(init=False)
    max: asyncio.Semaphore = dataclasses.field(init=False)
    nmap: asyncio.Semaphore = dataclasses.field(init=False)
    os: asyncio.Semaphore = dataclasses.field(init=False)
    ping: asyncio.Semaphore = dataclasses.field(init=False)
    socket: asyncio.Semaphore = dataclasses.field(init=False)

    def __post_init__(self):
        self.atlas = asyncio.Semaphore(self.ATLAS)
        self.db = asyncio.Semaphore(self.DB)
        self.io = asyncio.Semaphore(self.IO)
        self.max = asyncio.Semaphore(self.MAX)
        self.nmap = asyncio.Semaphore(self.NMAP)
        self.os = asyncio.Semaphore(self.OS)
        self.ping = asyncio.Semaphore(self.PING)
        self.socket = asyncio.Semaphore(self.SOCKET)
...
async def is_global_ip(ip: Text, semaphore: Sem = sem) -> OptionalBool:
    """
...
async def is_ip_up(ip: Text, semaphore: Sem = sem, p=None) -> OptionalBool:
    """
    Pings host.
    ...
    pings = 3
    if await is_global_ip(ip, semaphore):
        async with semaphore.max:
            async with semaphore.ping:
                ...
                if rc == 0:
                    ...
                    rv = None
    # FIXME: que hago con las invalid IPs
    if p:
        pass
...
async def is_port_proto_open(port: UnionTextInt = 27017, ip: Text = LOCALHOST, proto: UnionTextInt = PROTO_IP.tcp,
                             status: Bool = False, semaphore: Sem = sem) -> Union[Bool, ProtoStatus]:
    """
    Port Open.
    ...
    if isinstance(proto, str):
        proto_ip = getattr(PROTO_IP, proto, PROTO_IP.tcp)
    proto_name = PROTO_NAME.tcp if proto_ip == PROTO_IP.tcp else PROTO_NAME.udp

    ip_port = ip_port_tuple(port, ip)
    if is_ip_up(ip, semaphore):
        async with semaphore.max:
            async with semaphore.socket:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM, proto_ip) as s:
                    if status:
                        return ProtoStatus(ip, port, proto_name, not bool(s.connect_ex(ip_port)))
                    return not bool(s.connect_ex(ip_port))
...
# FIXME:
def print_and_save_json(d, name):
    cprint(json.dumps(d, indent=4, sort_keys=True))
    if System.MACOS and path.env.username == User.name:
        with (path.report / f'{name}.json').open(mode='w') as f:
            f.write(json.dumps(d, indent=4, sort_keys=True))
            green(f'{(path.report / f"{name}.json")} updated', e=True)
            stdout, stderr, rc = cmd(f'upload.sh {path.name}')
            ic(stdout, stderr, rc)
            if rc == 0:
                green(f'{path.name} uploaded', e=True)
            else:
                path.log.error(fm(stdout, stderr, rc))
...
async def which_open(ip: Text = LOCALHOST, semaphore: Sem = sem, log=None, p=None) -> UnionDictList:
    """
    Scan host or hosts.
    ...
    tcp_task = tuple(is_port_proto_open(port, ip, status=True, semaphore=semaphore) for ip in ip for port in ALL_PORTS)
    udp_task = tuple(is_port_proto_open(port, ip, PROTO_IP.udp, True, semaphore)
                     for ip in ip for port in ALL_PORTS)
