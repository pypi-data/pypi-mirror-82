...
class Env(Asdict):
    ...
    def __init__(self, prefix: Text = str()):
        ...
        self.sem_context = contextvars.ContextVar('sem')
    ...
    async def sem(self):
        if not (sem := self.sem_context.get()):
            self.sem_context.set(Sem(*[self.file.int(upper_prefix(key, prefix=self.prefix), value)
                                       for key, value in Sem.defaults().items()]))
            try:
                async with sem:
                    self.sem_context.set(sem)
                    yield sem
                yield sem
            finally:
                self.sem_context.set(sem)
...
def paths(file: Text) -> Path:
    ...
    try:
        package_path.attrs_set(
            ...
            prefix=prefix, work=work, env=env, sem=env.sem,
