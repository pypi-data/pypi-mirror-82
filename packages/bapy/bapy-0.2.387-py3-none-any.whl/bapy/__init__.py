...
class Env(Asdict):
    ...
    def __init__(self, prefix: Text = str()):
        ...
        self.context_max = contextvars.ContextVar('max')
        self.context_atlas = contextvars.ContextVar('atlas')
        self.context_ssh = contextvars.ContextVar('ssh')
        self.context_os = contextvars.ContextVar('os')
        self.context_nmap = contextvars.ContextVar('nmap')

        self.context_max.set(asyncio.Semaphore(self.file.int(upper_prefix('MAX', prefix=self.prefix), Sem.max)))
        self.context_atlas.set(asyncio.Semaphore(self.file.int(upper_prefix('ATLAS', prefix=self.prefix), Sem.atlas)))
        self.context_ssh.set(asyncio.Semaphore(self.file.int(upper_prefix('SSH', prefix=self.prefix), Sem.ssh)))
        self.context_os.set(asyncio.Semaphore(self.file.int(upper_prefix('OS', prefix=self.prefix), Sem.os)))
        self.context_nmap.set(asyncio.Semaphore(self.file.int(upper_prefix('NMAP', prefix=self.prefix), Sem.nmap)))
    ...
    async def max(self):
        try:
            async with self.context_max:
                self.context_max.set(self.context_max)
                yield self.context_max
        finally:
            self.context_max.set(self.context_max)

    @contextlib.asynccontextmanager
    async def atlas(self):
        try:
            async with self.context_atlas:
                self.context_atlas.set(self.context_atlas)
                yield self.context_atlas
        finally:
            self.context_atlas.set(self.context_atlas)
    ...
    async def ssh(self):
        try:
            async with self.context_ssh:
                self.context_ssh.set(self.context_ssh)
                yield self.context_ssh
        finally:
            self.context_ssh.set(self.context_ssh)
    ...
    async def os(self):
        try:
            async with self.context_os:
                self.context_os.set(self.context_os)
                yield self.context_os
        finally:
            self.context_os.set(self.context_os)
    ...
    async def nmap(self):
        try:
            async with self.context_nmap:
                self.context_nmap.set(self.context_nmap)
                yield self.context_nmap
        finally:
            self.context_nmap.set(self.context_nmap)
