...
from dns.exception import SyntaxError, Timeout
from dns.name import Name
from dns.resolver import Resolver, NXDOMAIN, resolve, NoNameservers, NoAnswer
...
class Ip:
    ...
    async def DNSClass(self) -> Optional[Name]:
        ...
        if self.ip:
            try:
                rv = await sync_to_async(from_address)(self.ip)
                self.dns['dns_name'] = rv.to_text()
                _ = await self.PTRClass
            except (NXDOMAIN, SyntaxError):
    ...
    async def PTRClass(self) -> Optional[Resolver]:
        ...
        if self.ip and self.dns['dns_name']:
            try:
                rv = await sync_to_async(resolve)(self.dns['dns_name'], "PTR")
                rv = rv[0]
                self.dns['ptr'] = rv.to_text()
            except (NXDOMAIN, Timeout, NoNameservers, NoAnswer):
