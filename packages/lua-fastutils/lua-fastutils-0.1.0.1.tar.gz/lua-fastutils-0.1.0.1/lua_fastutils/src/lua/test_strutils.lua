...
function TestStrUtils:test11()
    local header_lines = {
        "reqid: 1234",
        "reqts: 2345",
        "reqcode: 3456",
    }
    local headers = string.parse_http_headers(header_lines)
    assertEquals(headers["reqid"], "1234")
    assertEquals(headers["reqts"], "2345")
    assertEquals(headers["reqcode"], "3456")
end

function TestStrUtils:test12()
    assertEquals(string.url_path_match("http://abc.com/api/ping?ts=1234", "/api/ping"), true)
    assertEquals(string.url_path_match("://abc.com/api/ping?ts=1234", "/api/ping"), true)
    assertEquals(string.url_path_match("/api/ping?ts=1234", "/api/ping"), true)
    assertEquals(string.url_path_match("/api/ping", "/api/ping"), true)

    assertEquals(string.url_path_match("http://abc.com/api/echo?ts=1234", "/api/ping"), false)
    assertEquals(string.url_path_match("://abc.com/api/echo?ts=1234", "/api/ping"), false)
    assertEquals(string.url_path_match("/api/echo?ts=1234", "/api/ping"), false)
    assertEquals(string.url_path_match("/api/echo", "/api/ping"), false)
end
