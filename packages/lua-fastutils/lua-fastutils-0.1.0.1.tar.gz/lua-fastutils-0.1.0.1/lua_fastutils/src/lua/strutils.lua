...
function _M.parse_http_headers(header_lines)
    local headers = {}
    for _, header_line in pairs(header_lines) do
        local k, v = string.split2(header_line, ":")
...
function _M.url_path_match(url, path)
    local schema, left = _M.split2(url, "://")
    local host, left = _M.split2(left, "/")
    local url_path, params = _M.split2(left, "?")
