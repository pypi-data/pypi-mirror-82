...
from rslib.data.pytorch.tfrecord.util import example_pb2, iterator_utils
    ...
    for record in record_iterator:
        ...
        for key, typename in description.items():
            if key not in all_keys:
                raise KeyError("Key {} doesn't exist (select from {})!")
            ...
            if typename is not None:
                ...
                if tf_typename != inferred_typename:
                    ...
                    raise TypeError("Incompatible type '{typename}' for `{key}` "
                                    "(should be '{reversed_mapping[inferred_typename]}').")
