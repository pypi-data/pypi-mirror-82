...
def ptn_rnn_decoder(cell,
    ...
    with tf.variable_scope("decoder_rnn") as scope:
        ...
        if (mode in ['SAMPLE', "GREEDY", "CUSTOM"]):
            ...
            def beam_sample(accum_logits, logits, point_mask, state, pre_output_idxs, i):
                ...
                if i + 1 < 3:
                    ...
                    t_mask = t0_mask
                    if i + 1 == 5:
                        t_mask = tf.math.logical_or(t1_mask, t2_mask)
                else:
                    l_mask = l3_mask
                    t_mask = t0_mask
                    if i + 1 == 8:
                        t_mask = t1_mask
                point_mask = [tf.math.logical_and(tf.math.logical_and(l_mask, t_mask),
                                                  tf.math.logical_and(selected_mask,
            ...
            accum_logits, point_mask, state, output_idx, output_idxs = \
                beam_sample(accum_logits, logits, point_mask, state, None, 0)
            for i in range(1, res_length):
                ...
                accum_logits, point_mask, state, output_idx, output_idxs = \
                    beam_sample(accum_logits, logits, point_mask, state, output_idxs, i)
