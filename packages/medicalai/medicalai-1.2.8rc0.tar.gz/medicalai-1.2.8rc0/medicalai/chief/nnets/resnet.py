...
def resnet(img_input=(224,224,3),classes=3,name = 'ResNet50', **kwargs):
  """
  Loaded the RESNET network, ensuring the head FC layer sets are left off
  ...
  if name in resnetMethods:
    print('[INFO]: {} Network Initialized'.format(name))
    name =appsLink+name
  else:
    print('[WARNING]: Unknown Name {}\n[WARNING] Network Initialized with ResNet50'.format(name))
    name =appsLink+'ResNet50'
  aiApp = eval(name)

  baseModel = aiApp(weights="imagenet", include_top=False,
