...
class AddressLineMixin(CommonMixin, LangScriptMixin, LocalTypeMixin):
    """Corresponds to EAD3 addressline.

    The address_line field must support the following markup (or
    markup that can be converted to): abbr, emph, expan, foreign, lb,
    ptr, ref. These further may have markup children of their own.
...
class DateRangeMixin(CommonMixin, LangScriptMixin, LocalTypeMixin):
    ...
    fromdate_notafter = models.CharField(
        blank=True, max_length=32, validators=[validate_token])
    fromdate_notbefore = models.CharField(
        blank=True, max_length=32, validators=[validate_token])
    fromdate_script = deepcopy(script)
    fromdate_standarddate = models.CharField(
        blank=True, max_length=32, validators=[validate_token])
    ...
    todate_notafter = models.CharField(
        blank=True, max_length=32, validators=[validate_token])
    todate_notbefore = models.CharField(
        blank=True, max_length=32, validators=[validate_token])
    todate_script = deepcopy(script)
    todate_standarddate = models.CharField(
        blank=True, max_length=32, validators=[validate_token])
...
class MaintenanceSection(models.Model):
    ...
    maintenanceagency_descriptivenote = models.TextField()
...
class SourcesSection(models.Model):
    ...
    sources_lastdatetimeverified = deepcopy(lastdatetimeverified)
...
class DIdSection(models.Model):
    ...
    repositories = models.ManyToManyField(
        'Repository', help_text=constants.REPOSITORY_HELP)
...
class EAD(BaseMixin, CommonMixin, LangScriptMixin, RelatedEncodingMixin,
    ...
    languagedeclarations = models.ManyToManyField('LanguageDeclaration')
...
class Address(CommonMixin, LangScriptMixin):
    """Corresponds to EAD3 address.

    EAD3 specifies that an address must have addressline elements
    within it; validation of this within Django must be done within a
    form.
...
class MaintenanceEvent(CommonMixin, EncodingAnalogMixin, LangScriptMixin):
    ...
    eventdatetime_standarddatetime = models.DateTimeField()
...
class Repository(CommonMixin, EncodingAnalogMixin, LabelMixin, LangScriptMixin,
    ...
    On XML serialisation, if there are associated
    RepositoryAddressLine objects, create the address with whatever
...
class RepositoryAddressLine(AddressLineMixin):
...
class TitleProper(CommonMixin, EncodingAnalogMixin, LangScriptMixin,
    ...
    titlestmt = models.ForeignKey(
        EAD, on_delete=models.CASCADE, related_name='titleproper_set')
