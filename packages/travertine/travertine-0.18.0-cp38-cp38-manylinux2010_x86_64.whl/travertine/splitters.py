...
class RequestSplitter(BaseSplitter):
    __doc__ = _("Split by each request.")
...
class UnitSplitter(BaseSplitter):
    __doc__ = _("Split by quantity (so that each item can be counted).")
...
class UnitRequestSplitter(BaseSplitter):
    __doc__ = _("Split by request and units.")
    ...
    def __call__(self, demand: Demand, env: Environment) -> Iterable[Demand]:
        return [
            demand.replace(
                requests=tuple(
                    request.replace(quantity=1) for _ in range(request.quantity)
                )
            )
            for request in demand.requests
        ]
