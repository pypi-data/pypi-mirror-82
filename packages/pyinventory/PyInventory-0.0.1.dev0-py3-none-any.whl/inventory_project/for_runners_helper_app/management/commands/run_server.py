...
class Command(RunServerCommand):
    ...
    help = "Run PyInventory with django developer server"
    ...
    def handle(self, *args, **options):
        ...
        if "RUN_MAIN" not in os.environ:
            ...
            if distance_model_count == 0:
                # call: inventory.management.commands.fill_basedata.Command
    ...
    def run(self, **options):
        ...
        if sys.platform in ("win32", "cygwin"):
            # Bugfix for:
            #   can't open file 'C:\Program Files\PyInventory\Scripts\inventory':
            #   [Errno 2] No such file or directory
            executable = Path(Path(sys.argv[0]).parent, "inventory-script.py")
