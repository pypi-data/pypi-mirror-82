...
def orders_list(options):
    ...
    # accounts_data = remote.api.get("/accounts")
    ...
    if payload:
        data = remote.api.get(endpoint, json=payload)
    else:
...
def order_info(options):
    data = remote.api.get("/order/{order}".format(
...
def order_delete(options):
    ...
    remote.api.delete("/orders/{order}".format(
        order=options.first("order")))
...
def order_update(options):
    ...
    order = remote.api.get(f"/order/{order_id}")
    ...
    data = remote.api.patch(f"/orders/{order_id}", json=payload)
