...
class SbModel:
    ...
    def input_data_list(self):
        model_input_data_query = MODEL_INPUT_DATA_TEMPLATE_QUERY.format(MODEL_NAME=self.model_name).format(**self.query_dict)
        predict_input_data_query = PREDICT_INPUT_DATA_TEMPLATE_QUERY.format(MODEL_NAME=self.model_name).format(**self.query_dict)
        model_input_dict = {'query': model_input_data_query, 'bucket': self.bucket, 'path': self.input_train_path}
        predict_input_dict = {'query': predict_input_data_query, 'bucket': self.bucket, 'path': self.input_predict_path}
