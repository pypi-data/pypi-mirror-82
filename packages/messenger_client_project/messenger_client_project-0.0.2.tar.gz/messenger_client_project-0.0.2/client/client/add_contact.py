...
class AddContactDialog(QDialog):
    ...
    def __init__(self, transit, database):
        super().__init__()
    ...
    def possible_contacts_update(self):
        ...
        contacts_list = set(self.database.get_client_contacts())
        users_list = set(self.database.get_known_clients())
        # Удалим сами себя из списка пользователей, чтобы нельзя было добавить
        # самого себя
        users_list.remove(self.transit.username)
    ...
    def update_possible_contacts(self):
        ...
        '''
        try:
            self.transit.update_list_of_clients()
        except OSError:
            pass
        else:
