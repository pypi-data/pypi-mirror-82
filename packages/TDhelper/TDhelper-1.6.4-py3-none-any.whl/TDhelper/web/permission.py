...
class perACL:
    def __init__(self,rpc_key, params_container_class,tokenKey="usr-token"):
    ...
    def AccessControlLists(self, eventKey= None, debug= False):
        def decorator(func):
            @functools.wraps(func)
            def wapper(*args, **kwargs):
                validate_state= True
                if not debug:
                    platkey=func.__qualname__.split('.')[0].upper()
                    if eventKey:
                        _handleName = platkey+"."+func.__qualname__.split('.')[0].upper()+'_'+ eventKey.upper()
                    else:
                        _handleName = platkey+"."+func.__qualname__.replace('.', '_').upper()
                    ...
                    if not params_instance:
                        for k, v in kwargs:
                            if isinstance(v, self._params_container):
                                ...
                                break
                    if not params_instance:
                        return Response("can found context.",status= 415)
                    if isinstance(params_instance, Request):
                        if self._tokenKey in params_instance._request.headers:
                            ...
                            return Response("http headers can not found '%s' key."%self._tokenKey,status= 415)
                    ...
                    if token:
                        if self._rpc:
                            validate_ret= self._rpc.call(self._rpc_key,**{"token":token,"event":_handleName})
                            if validate_ret:
                                if validate_ret['state']==200:
                                    validate_state= True
                                else:
                                    return Response("access error.(%s)"%validate_ret['msg'])
                            else:
                                return Response("access http error.")
                        else:
                            return Response("rpc handle is none.", status=404)
                    else:
                        return Response("token(%s) is None.")
                if validate_state:
