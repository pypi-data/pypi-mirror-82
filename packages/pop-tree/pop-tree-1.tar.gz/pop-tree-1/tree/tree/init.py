...
Plugin = namedtuple("Plugin", ("funcs", "data"))
...
def cli(hub: pop.hub.Hub):
    ...
    if hub.OPT.tree.sub:
        if "." in hub.OPT.tree.sub:
            hub.pop.sub.add(hub.OPT.tree.sub)
        else:
...
def recurse(hub: pop.hub.Hub, sub: pop.hub.Sub) -> Dict[str, Any]:
    """
    ...
    for loaded in sub._subs:
        ret[loaded] = data_.NamespaceDict()
        loaded_sub = getattr(sub, loaded)
        ret[loaded][sub._subname] = hub.tree.init.recurse(loaded_sub)

        for mod in loaded_sub._loaded:
            loaded_mod = getattr(loaded_sub, mod)
            ret[loaded][sub._subname][mod] = Plugin(
                funcs=hub.tree.init.funcs(loaded_mod),
                data=hub.tree.init.data(loaded_mod),
            )
    return ret
