...
def parse_mod(hub, mod):
    ...
    return Plugin(functions=hub.tree.init.funcs(mod), variables=hub.tree.init.data(mod))
...
def traverse(hub) -> Dict[str, Any]:
    ...
    """
    root = data_.NamespaceDict(hub={})
    for sub in hub._subs:
        loaded_sub = getattr(hub, sub)
        root.hub[sub] = hub.tree.init.recurse(loaded_sub)

        for mod in loaded_sub._loaded:
            loaded_mod = getattr(loaded_sub, mod)
            root.hub[sub][mod] = hub.tree.init.parse_mod(loaded_mod)
    return root
