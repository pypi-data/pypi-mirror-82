...
class EnrichmentAPIClient(Client):
    """Client for adsquare Enrichment API.

    The API documentation can be found on the Platform wiki:
    https://github.com/rockabox/rbx/wiki/adsquare#enrichment-api

    """
    ENDPOINT = 'http://3.248.53.133:80/api/v2/'

    def __init__(self, username, password):
        """The Enrichment API uses Basic Authentication."""
        super().__init__()
    ...
    def get_audiences(self, latitude, longitude):
        """Retrieve the audiences associated with the given coordinates.

        Returns the list of audience IDs if any were found:
        >>> {'audiences': [471614, 286391]}

        Or None otherwise.
        """
        response = self.request('post', 'audience/query', data={
            'latitude': latitude,
            'longitude': longitude,
        })
        if not response or 'audiences' not in response:
            return None
