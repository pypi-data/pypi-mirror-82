...
class PuppetStub(betterproto.ServiceStub):
    ...
    async def message_file_stream(self, *, id: str = "") -> AsyncIterator[FileBoxChunk]:
        request = MessageFileStreamRequest()
        request.id = id

        async for response in self._unary_stream(
            "/wechaty.Puppet/MessageFileStream", request, FileBoxChunk,
        ):
            yield response
    ...
    async def message_image_stream(
        self, *, id: str = "", type: "ImageType" = 0
    ) -> AsyncIterator[FileBoxChunk]:
        request = MessageImageStreamRequest()
        request.id = id
        request.type = type

        async for response in self._unary_stream(
            "/wechaty.Puppet/MessageImageStream", request, FileBoxChunk,
        ):
            yield response
    ...
    async def message_send_file_stream(
        ...
        request_iterator: Union[
            AsyncIterable["FileBoxChunk"], Iterable["FileBoxChunk"]
        ],
