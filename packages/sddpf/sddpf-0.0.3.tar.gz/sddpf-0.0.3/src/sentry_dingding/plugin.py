...
class DingDingPlugin(NotificationPlugin):
    ...
    def post_process(self, group, event, *args, **kwargs):
        ...
        issue_data = requests.get("http://web-middle.ruibogyl.work/sentry/findIssueTargetUrl",request_data_1,
            headers=header,verify=False).json()

        print issue_data['data']['url']

        request_data_2 = {
            'url': issue_data['data']['url']
        }
        response = requests.get("http://web-middle.ruibogyl.work/sentry/getDingDingAssignee",request_data_2,
            headers=header,verify=False).json()
