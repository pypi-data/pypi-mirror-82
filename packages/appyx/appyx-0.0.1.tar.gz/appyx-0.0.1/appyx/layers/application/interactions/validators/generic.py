...
class IsDecimal(ParameterValidator):
    def _validation_result(self, argument):
        validation_result = Result()
        condition = isinstance(argument, Decimal)
        if not condition:
            validation_result.add_error(self.error_message())
        return validation_result
    ...
    @classmethod
    def error_message(cls):
        return 'Este campo debe ser instancia de Decimal'
    ...
    def _validation_result(self, argument):
        ...
        if content_type not in self._valid_content_types:
            validation_result.add_error(
                f"The content type {content_type} is not supported. It should be {self._valid_content_types}")
...
class IsOneOfThese(ParameterValidator):
    ...
    def _validation_result(self, element):
        ...
        if not is_one_of_these:
            validation_result.add_error(
                f'Value {element} must be one of these elements: {self._list_of_valid_elements}')
