...
class MongoEventSourceRepository(
    ...
    def append_to_stream(self, obj):  # noqa: D102
        ...
        while True:
            try:
                ...
                known_event_ids = [
                    event["event_id"] for event in known_event_stream
                ]
                max_known_version = (
    ...
    def __save_memento(self, obj):
        try:
            ...
            if not obj.is_json_serializable(memento_payload):
                raise ValueError(f"Memento should be json serializable")
    ...
    def __have_memento(self, object_id):
        return (
            self.__memento_collection.count_documents({"object_id": object_id})
            > 0
        )
