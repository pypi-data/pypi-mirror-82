...
class Tailor:
    def __init__(self, media_path):
        ...
        self.output = self.media_path.split(".")[0] + "_SUB." + self.media_path.split(".")[1]
        self.work_dir = "\\".join(media_path.split("\\")[:-1])
        self.token = stamp()
    ...
    def cut(self, stamp: tuple, frag_idx: int, suffix=""):
        ...
        cmd = "ffmpeg -ss {} -i {} -t {} -c:v copy -c:a copy {}".format(":".join([start[i:i + 2] for i in range(0, 5, 2)]),
    ...
    def concat(self, fragments):
        ...
        media_list_path = "{}\\concat{}.txt".format(self.work_dir, self.token)
...
if __name__ == "__main__":
    frgs = []
    frgs.append(("000005", "000010"))
    frgs.append(("000015", "000020"))
    Tailor("F:\\download\\20\\video.mp4").launch(frgs)
