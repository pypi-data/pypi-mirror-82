...
class HFSFileSystem(file_system.FileSystem):
  ...
  ROOT_DIRECTORY_IDENTIFIER_NUMBER = 1

  TYPE_INDICATOR = definitions.TYPE_INDICATOR_HFS

  _HFSPLUS_PRIVATE_DATA_IDENTIFIER = 16
  ...
  def __init__(self, resolver_context):
    ...
    """
    super(HFSFileSystem, self).__init__(resolver_context)
  ...
  def FileEntryExistsByPathSpec(self, path_spec):
    ...
    try:
      # The check for the "HFS+ Private Data" is needed to prevent get_file_entry_by_path
      # raising "ValueError: embedded null character".
      if location is not None and identifier != self._HFSPLUS_PRIVATE_DATA_IDENTIFIER:
