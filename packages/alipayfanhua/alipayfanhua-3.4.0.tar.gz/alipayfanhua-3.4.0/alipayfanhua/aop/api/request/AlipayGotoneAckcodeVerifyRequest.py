...
class AlipayGotoneAckcodeVerifyRequest(object):
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'alipayfanhuagotone.ackcode.verify'
        params[P_VERSION] = self.version
        if self.biz_model:
            params[P_BIZ_CONTENT] = json.dumps(obj=self.biz_model.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
        if self.ack_code:
            if hasattr(self.ack_code, 'to_alipay_dict'):
                params['ack_code'] = json.dumps(obj=self.ack_code.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
