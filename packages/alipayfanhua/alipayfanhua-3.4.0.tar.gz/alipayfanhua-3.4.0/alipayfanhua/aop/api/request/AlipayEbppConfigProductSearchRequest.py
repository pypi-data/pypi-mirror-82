...
class AlipayEbppConfigProductSearchRequest(object):
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'alipayfanhuaebpp.config.product.search'
        ...
        if self.field_scene:
            if hasattr(self.field_scene, 'to_alipay_dict'):
                params['field_scene'] = json.dumps(obj=self.field_scene.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
