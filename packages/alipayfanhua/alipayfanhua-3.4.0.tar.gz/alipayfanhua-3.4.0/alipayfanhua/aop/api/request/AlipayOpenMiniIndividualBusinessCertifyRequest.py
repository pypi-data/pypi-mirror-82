...
class AlipayOpenMiniIndividualBusinessCertifyRequest(object):
    ...
    def get_params(self):
        params = dict()
        params[P_METHOD] = 'alipayfanhuaopen.mini.individual.business.certify'
        ...
        if self.license_pic:
            if hasattr(self.license_pic, 'to_alipay_dict'):
                params['license_pic'] = json.dumps(obj=self.license_pic.to_alipay_dict(), ensure_ascii=False, sort_keys=True, separators=(',', ':'))
            else:
