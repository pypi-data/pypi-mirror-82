...
from alipayfanhua.aop.api.domain.VoucherQueryInfo import VoucherQueryInfo


class AlipayMarketingVoucherdetailListQueryResponse(AlipayResponse):

    def __init__(self):
        super(AlipayMarketingVoucherdetailListQueryResponse, self).__init__()
    ...
    def vouchers(self, value):
        if isinstance(value, list):
            self._vouchers = list()
            for i in value:
                if isinstance(i, VoucherQueryInfo):
                    self._vouchers.append(i)
                else:
                    self._vouchers.append(VoucherQueryInfo.from_alipay_dict(i))

    def parse_response_content(self, response_content):
        response = super(AlipayMarketingVoucherdetailListQueryResponse, self).parse_response_content(response_content)
