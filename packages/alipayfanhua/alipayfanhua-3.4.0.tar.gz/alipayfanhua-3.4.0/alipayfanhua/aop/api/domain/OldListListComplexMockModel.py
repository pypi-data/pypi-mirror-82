...
from alipayfanhua.aop.api.domain.OldComplextMockModel import OldComplextMockModel


class OldListListComplexMockModel(object):
    ...
    def cm_list(self, value):
        if isinstance(value, list):
            self._cm_list = list()
            for i in value:
                if isinstance(i, OldComplextMockModel):
                    self._cm_list.append(i)
                else:
                    self._cm_list.append(OldComplextMockModel.from_alipay_dict(i))
