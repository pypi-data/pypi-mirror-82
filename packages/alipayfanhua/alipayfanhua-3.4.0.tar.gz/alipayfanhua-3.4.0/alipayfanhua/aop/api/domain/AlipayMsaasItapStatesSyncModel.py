...
from alipayfanhua.aop.api.domain.StatesSyncPayload import StatesSyncPayload


class AlipayMsaasItapStatesSyncModel(object):
    ...
    def payload(self, value):
        if isinstance(value, list):
            self._payload = list()
            for i in value:
                if isinstance(i, StatesSyncPayload):
                    self._payload.append(i)
                else:
                    self._payload.append(StatesSyncPayload.from_alipay_dict(i))
    @property
    def request_id(self):
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayMsaasItapStatesSyncModel()
        if 'payload' in d:
            o.payload = d['payload']
        if 'request_id' in d:
