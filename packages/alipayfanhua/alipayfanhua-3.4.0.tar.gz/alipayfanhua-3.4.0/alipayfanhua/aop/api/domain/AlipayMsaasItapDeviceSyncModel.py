...
from alipayfanhua.aop.api.domain.DeviceSyncPayload import DeviceSyncPayload


class AlipayMsaasItapDeviceSyncModel(object):
    ...
    def payload(self, value):
        if isinstance(value, list):
            self._payload = list()
            for i in value:
                if isinstance(i, DeviceSyncPayload):
                    self._payload.append(i)
                else:
                    self._payload.append(DeviceSyncPayload.from_alipay_dict(i))
    @property
    def request_id(self):
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayMsaasItapDeviceSyncModel()
        if 'payload' in d:
            o.payload = d['payload']
        if 'request_id' in d:
