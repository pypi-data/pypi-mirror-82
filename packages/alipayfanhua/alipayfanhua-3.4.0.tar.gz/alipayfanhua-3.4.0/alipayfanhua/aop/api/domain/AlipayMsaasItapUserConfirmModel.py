...
from alipayfanhua.aop.api.domain.UserConfirmPayload import UserConfirmPayload


class AlipayMsaasItapUserConfirmModel(object):
    ...
    def payload(self, value):
        if isinstance(value, list):
            self._payload = list()
            for i in value:
                if isinstance(i, UserConfirmPayload):
                    self._payload.append(i)
                else:
                    self._payload.append(UserConfirmPayload.from_alipay_dict(i))
    @property
    def request_id(self):
    ...
    def from_alipay_dict(d):
        if not d:
            return None
        o = AlipayMsaasItapUserConfirmModel()
        if 'payload' in d:
            o.payload = d['payload']
        if 'request_id' in d:
