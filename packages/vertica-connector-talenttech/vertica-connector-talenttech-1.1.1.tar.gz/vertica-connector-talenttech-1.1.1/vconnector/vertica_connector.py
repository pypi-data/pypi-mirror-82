...
class VerticaConnector:
    ...
    def insert(self, table_name, data, schema=None, table_suffix=None):
        """
        ...
        columns = self.get_columns(table_name=table_name, schema=schema)
    ...
    def get_columns(self, table_name, schema):
        ...
        res = cur.execute(sql)
        rows = res.fetchall()
        return dict(zip([row[0] for row in rows], [row[1] for row in rows]))
