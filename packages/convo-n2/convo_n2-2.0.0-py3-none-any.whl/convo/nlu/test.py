...
def write_intent_successes(
    ...
    if successes:
        convo.shared.utils.io.dump_obj_as_json_to_file(successes_filename, successes)
...
def write_intent_errors(
    ...
    if errors:
        convo.shared.utils.io.dump_obj_as_json_to_file(errors_filename, errors)
...
def evaluate_response_selections(
    ...
    from convo.test import get_evaluation_metrics
    ...
    if output_directory:
        ...
        convo.shared.utils.io.dump_obj_as_json_to_file(report_filename, report)
...
def write_incorrect_entity_predictions(
    ...
    if errors:
        convo.shared.utils.io.dump_obj_as_json_to_file(error_filename, errors)
...
def evaluate_entities(
    ...
    for extractor in extractors:
        ...
        if output_directory:
            ...
            convo.shared.utils.io.dump_obj_as_json_to_file(
...
def run_evaluation(
    ...
    test_data = convo.shared.nlu.training_data.loading.load_data(
    ...
    if output_directory:
        convo.shared.utils.io.create_directory(output_directory)
...
def cross_validate(
    ...
    if isinstance(nlu_config, str):
        nlu_config = convo.nlu.config.load(nlu_config)

    if output:
        convo.shared.utils.io.create_directory(output)
...
def compare_nlu(
    ...
    for run in range(runs):
        ...
        convo.shared.utils.io.write_text_file(test.nlu_as_markdown(), test_path)
