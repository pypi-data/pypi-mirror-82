...
def multilook(inputs, window_size=3, output=None, *args, **kwargs):
    ...
    if np.iscomplexobj(inputs):
        return filters.uniform_filter(inputs.real, window_size, output=output, *args, **kwargs) + 1j * filters.uniform_filter(inputs.imag, window_size, output=output, *args, **kwargs)
    else:
        return filters.uniform_filter(input.real, window_size, output=output, *args, **kwargs)
