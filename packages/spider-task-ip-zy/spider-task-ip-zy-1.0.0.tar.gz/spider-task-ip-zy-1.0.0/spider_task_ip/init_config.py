...
def ip_config(redis_config: dict = None, values: list = None):
    """
    ...
    for value in values:
        ...
        redis_key = f'(ip_config)hash_{value["project_name"]}_{value["spider_name"]}'
        redis_client.hmset(name=redis_key, mapping=value)
        logging.info(f'ip队列名字: (ip_queue)list_{value["project_name"]}_{value["spider_name"]}')
