...
First define the data set schema:
...
schema = horkos.schema_from_yaml('http-requests-schema.yaml')
schema.process({'path': '/foo/bar'})
# ValueError: Casting errors - params is required, method is required, ...
schema.process({
    'path': '/foo/bar',
    'params': '{"foo": "bar"}',
    'method': 'BAD',
...
# ValueError: Check errors - "BAD" in method did not pass choice check, ...
schema.process({
    'path': '/foo/bar',
    'params': '{"foo": "bar"}',
    'method': 'GET',
...
```
$ horkos check -s http-requests-schema.yaml foo.csv
