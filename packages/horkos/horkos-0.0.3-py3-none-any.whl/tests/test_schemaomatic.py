...
def test_schema_from_dict_bad_checker(check: dict):
...
def test_schema_from_dict_unknown_checker():
    ...
    with pytest.raises(errors.SchemaValidationError) as err:
        schemaomatic.schema_from_dict(raw_schema, strict=False)

    msg = 'In "fields.fieldId.checks[0]" check name "unknown" is unknown.'
    assert msg in str(err.value)


def test_schema_from_dict_custom_checker():
    ...
    schemaomatic.schema_from_dict(
        raw_schema, strict=False, custom_checkers=custom
    )
