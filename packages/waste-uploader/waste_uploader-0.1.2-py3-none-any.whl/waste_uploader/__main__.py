...
def request_upload(auth, bundle: Bundle) -> UploadReq:
    request_upload_url = f'{API_GATEWAY_URL}/prod/upload?project={bundle.project}&platform={bundle.platform}'
    req = requests.post(request_upload_url, verify=True, auth=auth)
...
def distribute(auth, bundle: Bundle, file_hash):
    ...
    payload = {'release_notes': bundle.release_notes}
    try:
...
def notify(auth, bundle: Bundle, file_hash):
    ...
    bundle_sch = BundleSchema()
    payload = bundle_sch.dumps(bundle)
    payload.update({
        'slack_url': slack_url,
...
def main():
    ...
    if check_result == 'success':
        ...
        auth = aws_auth()
        upload_req = request_upload(auth, bundle)
        file_path = os.environ['FILE_PATH']
        upload(upload_req, file_path)
        distribute(auth, bundle, upload_req.file_hash)
