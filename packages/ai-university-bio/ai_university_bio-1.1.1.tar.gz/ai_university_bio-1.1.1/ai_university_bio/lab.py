...
class Laboratory():
    def __init__(self):
        ...
        self.scaler = weights.scaler_weights
        self.result = []
    ...
    def StartProcess(self, iteration, InA,	InB, InC, InD, InE, InF, InG, InH, InI, InJ, InK, InL):
        ...
        for it in range(epohs): # Пробегаемся по всем эпохам
            ...
            plt.ylim((0,0.03))
            plt.hlines(0.02 - self.min_value, -.5, .5, color='r')
            plt.hlines(0.02 - self.mean_value, -.5, .5, color='y')
            plt.hlines(0.02 - self.max_value, -.5, .5, color='g')
            #plt.xticks(x_l, ['Модель'])
            plt.show()
    ...
    def getResult(self):
        ...
        print('Результат:')
        for i in range(6):
            print(title[i],': ', self.result[i]/self.scaler_weights[i])
