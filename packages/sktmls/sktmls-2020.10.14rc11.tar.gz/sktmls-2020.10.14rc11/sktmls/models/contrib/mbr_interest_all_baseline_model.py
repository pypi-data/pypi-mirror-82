...
class MbrInterestAllBaselineModel(MLSRuleModel):
    ...
    def __init__(
        ...
        - features: (list(str)) 피쳐 리스트 (필수 피쳐: `user_id`, `prefer_category`)
        ...
        my_model_v1 = MbrInterestAllBaselineModel(
            model_name="my_model",
            model_version="v1",
            features=["user_id", "prefer_category", "available_benefit_activity", "available_benefit_asianfood", "available_benefit_beauty"],
                ...
                candidates.append(
                    {
                        ...
                        "type": self.item_type,
                        "props": {
                            "score": str(round(random.random() + (1 if category in prefer_category else 0), 4)),
                            "context_id": "context_preference" if category in prefer_category else "context_default",
                            "context_nm": "고객선호" if category in prefer_category else "#",
                        },
                    }
                )

        num_pick = min(self.num_pick, len(candidates))
        top_candidates = sorted(candidates, key=lambda k: k["props"]["score"], reverse=True)[:num_pick]
