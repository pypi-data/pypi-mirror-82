        ...
        if append:
            if os.path.isfile(outfile):
                ...
                if len(colname_string) > 0:
                    ...
                    lines = list(delimiter.join(list(str(elem_dict[colname]) for colname in colnames))
                                 for elem_dict in list_of_dicts)
                else:
                    colnames = list(list_of_dicts[0].keys())
                    lines = [delimiter.join(colnames)] + \
                        list(delimiter.join(list(str(elem_dict[colname]) for colname in colnames))
                             for elem_dict in list_of_dicts)
