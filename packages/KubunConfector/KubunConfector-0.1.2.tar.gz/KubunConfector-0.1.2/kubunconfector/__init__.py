...
class Confector():
	...
	def checkSchemata(self):
		...
		for propertyIdent, (_sourceTag, linkTarget) in linkProps.items():
			errorMessage = f"Property {propertyIdent} targets invalid foreign property {linkTarget.targetIdent} in tag: {linkTarget.targetTag}"
			assert linkTarget.targetTag in self.schemata, errorMessage
			s = self.schemata[linkTarget.targetTag]
			if linkTarget.targetIdent == 'title':
				linkToTargetTypeName.update({propertyIdent: KubunString})
			else:
				targetProp = s.getProperty(linkTarget.targetIdent)
	...
	def pretty_print(self): # I'll admit: It's not that pretty haha
		...
		for _, (sourceTag, linkTarget) in self.collectOutboundLinks().items():
			linksSimple[sourceTag].append(linkTarget.targetTag)
			maxLen = max(max(maxLen, len(linkTarget.targetTag)), len(sourceTag))
