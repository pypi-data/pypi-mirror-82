		...
		for line in self.configLines:
			...
			if configParam is None:
				if not line.optional:
					raise Exception(
						f"Config is missing some Parameter: Property: { paramName } : { line.name }, { line.conftype }")
			...
			# TODO: Also check for unnecessary options? Re-Serialize this!
				...
				try:
					typeguard.check_type(
						'variablename', configParam, line.conftype)
				except TypeError:
					raise Exception(
...
DateFormat = NewType(
	'DateFormat', Literal['MonthSlashYear', 'DayDotMonthDotYear', 'Year'])
...
class Property():
	...
	def pretty_print(self, depth=0):
		...
		if self.kubunType.isLink():
			targetTag = self.config['target_tag']
...
class Schema():
	...
	def getProperty(self, propertyIdent: KubunIdentifier) -> Property:
		...
		if not type(propertyIdent) is KubunIdentifier:
