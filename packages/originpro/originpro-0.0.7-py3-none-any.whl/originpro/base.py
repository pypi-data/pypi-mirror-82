...
class BaseObject:
    ...
    def get_int(self, prop):
        ...
        return int(self.obj.GetNumProp(prop))
    ...
    def method_int(self, name, arg=''):
        ...
        return int(self.obj.DoMethod(name, arg))
    ...
    def name(self, value):
        """
        change the short name of an object, if there is a conflict, Origin will change to next available name automatically
        Examples:
            gp=op.new_graph()
            gp.name='Test'
            gp=op.new_graph()
...
class BasePage(BaseObject):
    ...
    def lt_exec(self, lt):
        ...
        """
        strLT = 'win -o {0}'.format(self.obj.GetName())
        strLT += ' {'
        strLT += lt
        strLT += '}'
        po.LT_execute(strLT)
    ...
    def is_open(self):
        ...
        Examples:
            b = wb.is_open()
        """
        return True if self.get_int('Win') > 2 else False
