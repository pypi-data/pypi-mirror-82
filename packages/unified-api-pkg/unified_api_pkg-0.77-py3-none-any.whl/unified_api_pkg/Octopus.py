    ...
    if result  < 100000:
        ...
        if pollType == 2:
            ...
            if __checkSameCard(transData,list_output_string[2]):
                ...
                writeOCTLogString("PollEx", "Present false card, API handle reject as 100022")
                return 100022
...
def topUpRetry(timeInterval,transData,printOut):
    result = topUpForRetry(transData["ECRREF"],inputAmountStringToLongOCT(transData["AMT"]),timeInterval,transData)
    return result

#No poll
def subsidyRetry(timeInterval,transData,printOut):
    result = topUpForRetry(transData["ECRREF"],0,timeInterval,transData)
    return result
