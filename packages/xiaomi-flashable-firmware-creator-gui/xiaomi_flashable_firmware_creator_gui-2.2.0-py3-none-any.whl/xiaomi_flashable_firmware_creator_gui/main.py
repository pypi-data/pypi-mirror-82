...
class MainWindowUi(QMainWindow):
    ...
    def __init__(self, lang, translator):
        ...
        self.groupbox_drop = DropSpace(self.window_body, self.status_box, self.tr)
    ...
    def re_translate_ui(self):
        ...
        self.statusBar().showMessage(self.tr("Status Box", "Ready"))
    ...
    def select_file(self):
        ...
        if not filepath:
            self.statusBar().showMessage(self.tr("Status Box", "Please select a file!"))
        ...
        self.statusBar().showMessage(self.tr("Status Box", f"File {self.filename} is selected"))
    ...
    def enter_url(self):
        ...
        Enter URL Dialog
        """
        dialog = InputDialog(self.tr('Enter URL Dialog', 'Remote Zip URL'),
                             self.tr('Enter URL Dialog', 'Enter a MIUI zip direct link:'),
                             self.tr('Enter URL Dialog', 'Set URL'),
                             self.tr('Enter URL Dialog', 'Cancel'),
                             parent=self.window_body)
        if dialog.exec_() == QDialog.Accepted:
            url = dialog.textValue()
            if "http" not in url or "ota.d.miui.com" not in url:
                message_box = MessageBox(self.tr('Popup Message', 'Error'),
                                         self.tr('Popup Message', 'Not a valid URL.'),
                                         self.tr('Popup Message', 'OK'), box_type="Warning",
                                         parent=self.window_body)
                message_box.exec_()
            ...
            self.status_box.setText(
                self.tr("Status Box", f"Remote file {self.filename} is selected."))
    ...
    def create_zip(self):
        ...
        if not self.filepath:
            error_box = MessageBox(self.tr('Popup Message', 'Error'),
                                   self.tr('Popup Message', 'You must select a ROM zip first!'),
                                   self.tr('Popup Message', 'OK'), box_type="Critical",
                                   parent=self.window_body)
            error_box.exec_()
        ...
        try:
            ...
            if str(err) == "Nothing found to extract!":
                message_box = MessageBox(self.tr('Popup Message', 'Error'),
                                         self.tr('Popup Message', 'Unsupported operation for MTK!'),
                                         self.tr('Popup Message', 'OK'), box_type="Warning",
                                         parent=self.window_body)
                message_box.exec_()
                self.status_box.setText(
        ...
        if extracted:
            ...
            message_box = OutputMessageBox(
                self.tr('Popup Message', 'Done'),
                self.tr('Popup Message', f'All Done! Output zip is {new_zip}'),
                self.tr('Popup Message', 'OK'), self.tr('Popup Message', 'Show in folder'),
