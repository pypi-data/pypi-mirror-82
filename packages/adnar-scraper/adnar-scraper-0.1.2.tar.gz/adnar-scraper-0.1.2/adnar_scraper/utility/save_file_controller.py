...
class SaveFileController:
    ...
    def save_text_vec_model(self, vectorizer, features):
        ...
        DataLoader.create_if_folder_not_exists(absolute_path)

        print('Text vectorizer & feature saved : ' + saving_time)

        DataLoader.save_pickle_data(data=vectorizer, file_path=absolute_path + 'text_vectorizer')
        DataLoader.save_pickle_data(data=features, file_path=absolute_path + 'text_features')
