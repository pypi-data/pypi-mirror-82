...
def find_datestr(s):
    arr = []
    rtn = []
    arr.extend(re.findall(r'(?:\D)([1|2]\d{3}-\d{2}-\d{2})(?:\D)*', s))
    _arr = re.findall(r'(?:\D)([1|2]\d{3}\d{2}\d{2})(?:\D)*', s)
    ...
    if len(arr) == 0:
        arr.extend(re.findall(r'(?:\D)([1|2]\d{3}-\d{2})(?:\D)*', s))
        _arr = re.findall(r'(?:\D)([1|2]\d{3}\d{2})(?:\D)*', s)
