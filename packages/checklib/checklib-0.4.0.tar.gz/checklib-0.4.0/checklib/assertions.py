...
class CheckerAssertionsMixin:
    def assert_eq(self, a, b, public, status=checklib.status.Status.MUMBLE):
        if a != b:
            self.cquit(status, public, f'Equality assertion failed: {a} ({type(a)}) != {b} ({type(b)})')

    def assert_neq(self, a, b, public, status=checklib.status.Status.MUMBLE):
        if a == b:
            self.cquit(status, public, f'Inequality assertion failed: {a} ({type(a)}) != {b} ({type(b)})')

    def assert_gt(self, a, b, public, status=checklib.status.Status.MUMBLE):
        if not (a > b):
            self.cquit(status, public, f'Inequality assertion failed: {a} <= {b}')

    def assert_gte(self, a, b, public, status=checklib.status.Status.MUMBLE):
        if not (a >= b):
            self.cquit(status, public, f'Inequality assertion failed: {a} < {b}')

    def assert_in(self, what, where, public, status=checklib.status.Status.MUMBLE):
        if what not in where:
            self.cquit(status, public, f'Contains assertion failed: {what} not in {where}')

    def assert_nin(self, what, where, public, status=checklib.status.Status.MUMBLE):
        if what in where:
            self.cquit(status, public, f'Not contains assertion failed: {what} not in {where}')

    def assert_in_list_dicts(self, lst, key, value, public, status=checklib.status.Status.MUMBLE):
        found = False
        for d in lst:
            if key in d and d.get(key) == value:
                found = True
