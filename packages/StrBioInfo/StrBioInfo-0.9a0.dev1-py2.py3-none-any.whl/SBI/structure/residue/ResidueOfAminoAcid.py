...
class ResidueOfAminoAcid(Residue):
    ...
    def make_to_CA(self):
        self.remove_side_chain()
        self.remove_backbone()
        self._cb = None
        self._n  = None
        self._c  = None
        self._o  = None
        self.add_atom(self._ca)

    def to_alanine(self):
        # TODO: This a VERY temporary solution, should be improved and generalized
        if self._type in ['ALA', 'GLY']:
            return
        tmp = []
        for at in self._sidechain_atoms:
            if at.name in ['CB', 'O']:
                tmp.append(at)
                break
        self._sidechain_atoms = tmp
        self._type = 'ALA'
        self._sidechain_coordinates = None
        if len(tmp) > 0:
            self._add_to_matrix(tmp[0])
