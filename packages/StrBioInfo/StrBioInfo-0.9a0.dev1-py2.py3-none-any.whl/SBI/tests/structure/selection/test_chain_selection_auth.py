...
class TestChainSelection( object ):
    ...
    def test_noNMR_withFabric( self ):
        ...
        assert dfA.chain_identifiers == ['B', 'C', 'D', 'E']
        assert dfA.chain_count == 4
        ...
        dfA = sele.selection(self.dftypes)
        assert isinstance(dfA, PDBFrame)
        assert dfA.chain_identifiers == ['B', 'D', 'E']
        assert dfA.chain_count == 3
        ...
        dfA = sele.selection(self.dftypes)
        assert isinstance(dfA, PDBFrame)
        assert dfA.chain_identifiers == ['A', 'B', 'C', 'D', 'E']
        assert dfA.chain_count == 5
        ...
        assert dfA.chain == 'A'
        # Unselect all
        sele = selectors.Selector('~Chain:A,B,C,D,E')
        assert isinstance(sele, selectors.NotSelector)
        dfA = sele.selection(self.dftypes)
        assert isinstance(dfA, PDBFrame)
        ...
        # Empty selection by trying to get individual ligands (not in auth)
        sele = selectors.Selector('ChainType:ligand')
        dfL = sele.selection(self.dftypes)
        assert isinstance(dfL, ChainFrame)
        assert len(dfL) == 0

        # In auth, selecting ligand-only chains will not work even for chains
        # with actual ligands.
        sele = selectors.Selector('ChainType:ligand')
        dfL = sele.selection(self.dflignt)
        assert isinstance(dfL, ChainFrame)
        assert len(dfL) == 0
