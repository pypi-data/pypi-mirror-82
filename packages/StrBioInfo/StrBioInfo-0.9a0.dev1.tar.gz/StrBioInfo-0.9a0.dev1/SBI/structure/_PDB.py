...
class _PDB(StorableObject):
    ...
    #
    def get_chain_by_id(self, id): # *DONE
        """
    ...
    def duplicate(self, hetero=True, water=False, NMR=False): # *DONE
        """
    ...
    def tmpclean(self, cluster_by_alternative_id=False, exclude_chains = None):
        ...
        for chain in self.chains:
            if (chain.chain not in chainsNIDs) and chain_change:
                self._chain_id.add(chain.chain)
                chain.alternative_id = chain.chain
    ...
    def translate(self, vector=None): # *DONE
        """
    ...
    def write(self, output_file=None, format='PDB', force=None, clean=False, extra=True):
        """
        ...
        if format == 'PDB':
            self._write_PDB_file(pdb_file=outfile, clean=clean, extra=extra)
    ...
    def _write_PDB_file(self, pdb_file, clean=False, extra=True):
        """
        Print a crystal into a PDB formated file
        """
        # out_fd = pdb_file.descriptor
        # out_fd.write(self._represent_COMPND())
        pdb_file.write(self.PDB_format(clean=clean, extra=True) + "\n")
        pdb_file.close()

    def PDB_format(self, clean=False, terminal=True, extra=True):
        """
        ...
        if clean:
            self.clean()
        for chain in self._chains:
            lines.append(chain.PDB_format(terminal=terminal, extra=extra))
        lines.append("END")
        return "\n".join(lines)
