...
class ChainOfProtein(Chain):
    ...
    def change_convention_names(self, current_convention, new_convention):
        from ...data import AminoAcids
        for res in self.aminoacids:
            for atom in res.atoms:
                atom.name = AminoAcids.rename_atom(res.single_letter, atom.name, current_convention, new_convention)

    def convert_to_alanine(self):
        for r in self.aminoacids:
            r.to_alanine()
