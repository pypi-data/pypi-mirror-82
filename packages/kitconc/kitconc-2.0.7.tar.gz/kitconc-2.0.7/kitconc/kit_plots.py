...
try:
    ...
    raise ImportWarning('matplotlib not installed')
...
class HeatMap():
    ...
    def plot(self, x_labels,y_labels,values):
        ...
        plt.setp(ax.get_xticklabels(), rotation=self.xrotation, ha="right", rotation_mode="anchor")
        if self.annotation == True:
            for i in range(len(y_labels)):
                for j in range(len(x_labels)):
                    t = ax.text(j, i, values[i, j], ha="center", va="center", color=self.annotation_color)
        plt.imshow(values,norm=matplotlib.colors.LogNorm(),interpolation='nearest',cmap=self.cmap)
        plt.show()
