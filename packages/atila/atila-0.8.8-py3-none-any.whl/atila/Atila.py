import os
from . import collectors, cookie, session, grpc_collector, ws_executor
...
class Atila (mixin.MixIn):
    ...
    def get_collector (self, request, path_info):
        ...
        if ct.startswith ("multipart/form-data"):
            return collectors.MultipartCollector
        if not ct.startswith ("application/grpc") and not ct.startswith ("application/octet-stream"):
            return
        ...
        if options.get ('input_stream') and ct.startswith ("application/octet-stream"):
            return collectors.StreamCollector
