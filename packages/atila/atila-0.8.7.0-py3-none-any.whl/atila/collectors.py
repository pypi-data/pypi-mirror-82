...
class StreamCollector (collectors.FormCollector, corequest):
	...
	def then (self, func, was = None):
		self.was = was
		if self.end_of_data or len (self.buffer) >= self.buffer_size:
			return func (self.was, self)
		self.callback = func
		return Revoke ()

	def fetch (self):
		if not self.buffer and self.end_of_data:
			return b''
		b, self.buffer = self.buffer [:self.buffer_size], self.buffer [self.buffer_size:]
		return b
	...
	def do_callback (self):
		if self.callback and (self.end_of_data or len (self.buffer) >= self.buffer_size):
