...
def _exists_model(model_name: Optional[str],
    ...
    """
    if not model_name and not working_model_id:
        return None
    ...
    if model_name and model_name in model_id_by_name:
...
def _hack_n_features_from_sklearn_model(model) -> int:
    ...
    try:
        ...
        logger.debug(f'Unable to determine input_shape because of: {e}')
        logger.info(f'🤞 Unable to determine input_shape, but not stopping 🤞')
