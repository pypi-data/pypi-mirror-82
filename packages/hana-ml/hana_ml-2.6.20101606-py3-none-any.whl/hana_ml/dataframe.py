...
try:
    from shapely import wkb, wkt    # Should not fail if requirements.txt has been installed
except ImportError:
    pass
...
class DataFrame(object):#pylint: disable=too-many-public-methods
    ...
    def _df(self, select_statement, name=None, propagate_safety=False):
        ...
        # is way too verbose.
        retval = DataFrame(self.connection_context, select_statement, name, self.geo_cols)
        if propagate_safety:
    ...
    def collect(self):
        ...
        try:
            ...
            if self.geo_cols:
                ...
                for geo in list(self.geo_cols.keys()):
                    pandas_df = self._transform_geo_column(pandas_df=pandas_df, col=geo)
                return pandas_df
        except dbapi.Error as exc:
...
def create_dataframe_from_pandas(connection_context, pandas_df, table_name, schema=None, force=False, replace=False, \
                                 ...
                                 primary_key=None, not_nulls=None):                             # Graph variables
    """
