...
class ZWMongo(object):
    ...
    def groupby(self, coll, key=None, conds=None, reverse=False, limit=0):
        with self.get_connection() as conn:
            rtn = conn.groupby(coll, key, conds, reverse, limit)
        return rtn
...
class ZWMongoConnection(object):
    ...
    def groupby(self, coll, key='_id', conds=None, reverse=False, limit=0):
        conds = conds or {}
        group = {
            '_id': "$%s" % key,
            'count': {'$sum': 1}
