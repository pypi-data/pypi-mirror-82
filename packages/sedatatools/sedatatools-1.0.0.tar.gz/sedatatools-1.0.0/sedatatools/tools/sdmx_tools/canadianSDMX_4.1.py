...
def getConceptValuesFromStructuresDesc(structureFile):
    ...
    for event, element in sourceFileForParsing:
        if event == 'end' and (element.tag == '{http://www.SDMX.org/resources/SDMXML/schemas/v2_0/structure}Dimension' or element.tag == '{http://www.SDMX.org/resources/SDMXML/schemas/v2_0/structure}Attribute' or element.tag == '{http://www.SDMX.org/resources/SDMXML/schemas/v2_0/structure}TimeDimension'):
            if 'codelist' in element.attrib: #added because TimeDimension sometimes doesn't have codelist attribute
...
def createTables(structureFile, genericFile, catalogueId, eliminateDim, tableDim, varDim, outputFolder, metaVar):
    ...
    createPosition = [] #this will store exact position where value must be written (combination of table, column and row)
    tableName = []

    #check if its a canadian census

    for event, element in etree.iterparse(genericFile):
        if event == 'end' and element.tag == '{http://www.SDMX.org/resources/SDMXML/schemas/v2_0/message}Sender' and element.attrib['id'] == 'OECD':
            print("Data from european central bank.")  # TODO continue from here
        elif event == 'end' and element.tag == '{http://www.SDMX.org/resources/SDMXML/schemas/v2_0/message}Sender' and element.attrib['id'] != 'STC':
            print('Unsuported format, not a Canadian Census!!!')
            sys.exit()

    sourceFileForParsing = etree.iterparse(genericFile)
    value = 0 # this will hold the actual value
    excludeNode = False
    ...
    for event, element in sourceFileForParsing:
        if event == 'end' and element.tag == '{http://www.SDMX.org/resources/SDMXML/schemas/v2_0/generic}Value' and element.attrib['concept'] in eliminateDim and element.attrib['value'] != '1': #changed == to in to work with lists
            ...
            if excludeNode == False:
                writeToTable(tableName, geo, value, outputFolder, catalogueId)
            tableName = []
            createPosition = []
            value = 0
            excludeNode = False
...
def menu():
    ...
    parser.add_option('-o', '--out', dest = 'outputFolder', help='Output folder, default [%default]', metavar='outputFolder', default = 'c:/Projects/ContentProduction/Canadian Census/scripts/CanadianSDMX/data/test_ouput/') # test why it separated files after large job is done
    ...
    group = optparse.OptionGroup(parser, "List concepts","Example: canadianSDMX -c catalogueId")
    group.add_option('-l', '--list', dest = 'structList', help='Catalogue Id for which to list variable numbers. If used with -f and -g catalogueId put "nocatalogue".', metavar='catalogueId')
    group.add_option('-f', '--structure-file', dest = 'sf', help='Manualy define full path to structure file.', metavar='structure_file')
    group.add_option('-g', '--generic-file', dest = 'gf', help='Manualy define full path to generic file.', metavar='generic_file')
...
def main():
    ...
    if len(argv) in [3, 5, 6, 7] and argv[1] == '-l':
        if opt.structList == 'nocatalogue':
            opt.structList = '' # in case no argument for -l flag provided, set to empty string
        sourceFolder = opt.sourceFolder
        if os.path.isfile(sourceFolder + 'Structure_' + opt.structList + '.xml') and os.path.isfile(sourceFolder + 'Generic_' + opt.structList + '.xml'):
            structureFile = sourceFolder + 'Structure_' + opt.structList + '.xml'
            genericFile = sourceFolder + 'Generic_' + opt.structList + '.xml'
        elif os.path.isfile(opt.sf) and os.path.isfile(opt.gf): # TODO document this and add it to a list of parameters, possibiliti to add custom structure and generic files
            structureFile = opt.sf
            genericFile = opt.gf
            listConceptsPrint(structureFile, genericFile) # this function print concepts for Canadian Census
        else:
            print("Structure or data files are missing on provided location?") # set more meaningfull message later
            sys.exit()
    elif len(argv) not in [1, 2, 3, 5, 9, 11, 12, 13, 15]:
        print("Wrong number of arguments, use -h to get help!")
        sys.exit()
    else:
        #
        # ## remove when run from cmd
        # #
        # opt.catalogueId = '98-313-XCB2011025'
        # opt.tableDim =  '4'
        # opt.varDim = '0'
        # opt.eliminateDim = '1'
        # outputFolder = 'c:/Projects/ContentProduction/Canadian Census/scripts/CanadianSDMX/data/test_ouput/'
        # opt.catalogueId = '98-314-XCB2011028' #'98-312-XCB2011047'
        # opt.tableDim =  '6'
        # opt.varDim = '4,5'
        # opt.eliminateDim = '0,2,3'
        # outputFolder = 'c:/Projects/ContentProduction/Canadian Census/scripts/CanadianSDMX/data/test_ouput/'
        # ###
        #
        # outputFolder = 'c:/users/datasoft/'
        # opt.catalogueId = 'XXX'
        # opt.tableDim =  '2'
        # opt.varDim = '1'
        catalogueId  = opt.catalogueId
        print("Reading from catalogue file", catalogueId, sourceFolder)
        genericFile = sourceFolder + 'Generic_'+ catalogueId +'.xml'
        print("Reading from generic file", genericFile)
        ...
        #print('tabela', tableDim, varDim, eliminateDim)
