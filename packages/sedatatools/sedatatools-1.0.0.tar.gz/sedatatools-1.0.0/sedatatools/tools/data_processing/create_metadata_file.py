...
def get_table_metadata_from_db(server, dbname, user, password, project_year, trusted_connection):
    ...
    cursor = conn.cursor()
    cursor.execute(
        'SELECT * FROM table_names where projectYear = ' + project_year,
    )
...
def prepare_environment(config_file):
    ...
    connection_string = f"Server=prime; database=; uid={config['username']};pwd={config['password']};Connect Timeout=1;Pooling=True"
...
def verify_config(config_path_value):
    ...
    if len(config['userName']) == 0 or 'userName' not in config.keys():
