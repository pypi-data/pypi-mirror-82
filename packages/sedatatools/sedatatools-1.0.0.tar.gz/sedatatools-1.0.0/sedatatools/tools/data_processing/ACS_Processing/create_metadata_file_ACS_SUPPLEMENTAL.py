...
def getTableFipses(tableFips, geoLevelInfo):
    """
    This will create a string with list of FIPS columns (SL050_FIPS, SL040_FIPS, etc.) based on nesting information (DB Conn. Info) to be used as primary keys.
    ...
    for i in reversed(range(lowFipsPos)):
        if geoLevelInfo[i][4] == 0 or geoLevelInfo[i][2] == '0': # stop adding sumlevs if you find geo level with indent 0 or fips length of 0
            break
        if geoLevelInfo[i][4] == geoLevelInfo[i+1][4] or geoLevelInfo[i][4] >= geoLevelInfo[lowFipsPos][4]: # if you find sumlev with same indent as previous or greaterr than wanted then skip it
            continue

        fullFipsList.append(geoLevelInfo[i][0] +'_FIPS')

    fullFipsList = ','.join(list(reversed(fullFipsList)))
    return fullFipsList
...
def getTableMetadataFromDb(server, dbname, user, password, projectYear, trustedConnection):
    """
    ...
    :param projectYear: project year for which the data is relevant
    :param trustedConnection: flag if server creadentials are needed
    ...
    for ext in extension:
        for el in tableNames:
            if ext in el[0]:
                metaData.append([el[0].replace(ext, ''),el[1]]) # remove extension and dataset ID (everything until first '_')

    for i in metaData:
        metaTableDictionary[i[1]] = i[0]
...
def getTablesFromDb(server, dbname, projectYear, user, password, trustedConnection):
    """
    ...
    cursor.execute("SELECT name FROM sys.objects WHERE type_desc = 'USER_TABLE' AND name <> 'table_names' and left(name,1) <> '_' AND name <> 'sysdiagrams' ORDER BY modify_date")
    ...
    for i in tableList:
        ...
        cursor.execute(sql)
        listOfTypes = cursor.fetchall()
        varTypes = [checkDataType(datum[0]) for datum in listOfTypes]
    ...
    for key,value in dictTablesAndVars.items():
        splitPos = [i for i, el in enumerate(key) if '_' == el][-1] + 1 # find last _
        values = []
        for v in value:
            # skip system required names
            # if len([j for j,l in enumerate(v[0]) if l == '_']) == 0:
            #     continue
            values.append([v[0],v[1]])
        dictTablesAndVarsUnique[key[splitPos:]] = values

    return dictTablesAndVarsUnique
    ...
    with open(variableDescriptionLocation) as file:
        ...
        for var in list(reader):
            if(len(var)== 0):
                ...
                continue
            if len(var) == 2:
                fileContentDict[var[0]] = [var[1]] # remove any commas if they exist
            else:
                fileContentDict[var[0]] = [var[1], var[2]] # remove any commas if they exist
...
def getDatasets(connectionString, dbname, geoLevelInfo, projectId, user, password, server, trustedConnection):
    """
    ...
    for i in sumLevs:
        geoIdSuffix = getGeoIdDbTableName(i[0], dbname, user, password, server, projectId, trustedConnection)

        result.append(E.dataset(GUID = str(uuid.uuid4()),
        GeoTypeName = i[0], # SL040
        DbConnString = connectionString,
        DbName = dbname,
        GeoIdDbTableName = i[0] + geoIdSuffix, # tablename e.g. 'LEIP1912_SL040_PRES_001'
        IsCached = 'false',
        DbTableNamePrefix = i[0] + '_', # tablename prefix e.g. 'LEIP1912_SL040_PRES_'
        DbPrimaryKey = getTableFipses(i[0]+'_FIPS',geoLevelInfo), # this is fixed
        DbCopyCount = '1'))
    return result

def getGeoIdDbTableName(sumlev, dbname, user, password, server, projectId, trustedConnection):
    """
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT TOP 1 name FROM sys.objects WHERE type_desc = 'USER_TABLE' AND name <> 'table_names' and left(name,1) <> '_' and name like '%" + sumlev + "%' ORDER BY name")
...
def getVariables(variables, variableDescription, metaTableName):
    ...
    removalVars = ['NAME', 'SUMLEV', 'grouped', 'v1', 'Geo_level', 'Geo_level_name','QName', 'TYPE', 'Geo', 'FIPS', 'Geo_orig', 'Indent', 'REGION', 'PLACE', 'NATION', 'ND', 'COUNTY', 'NECATA', 'CNECATA', 'DIVISION', 'STATE', 'CD', 'CS', 'ANRC', 'AIAANAHHL', 'MSAMSA', 'PC1', 'MD', 'CSA', 'PC', 'UA', 'PUMA', 'SDE', 'SDS', 'SDU']
    ...
    for i in variables:
        ...
        variableDescForRetrieve = variableDescription[i[0]]
        # [pos:]]
        ...
        label = variableDescForRetrieve[0], # find variable description in dictionary, by text after pr. id and order
        qLabel='',
        indent = variableDescForRetrieve[1] if len(variableDescForRetrieve) == 2 else '0', # TODO add indent info from variable desc. file
        dataType = varType,
        dataTypeLength = '0',# default to zero
        formatting = formatingValue,
...
def getTables(server, dbname, variableDescription, user, password, projectYear, trustedConnection):
    """
    ...
    """
    tableList = getTablesFromDb(server, dbname, projectYear, user, password, trustedConnection)
    tableList = collections.OrderedDict(sorted(tableList.items()))
    tableMetaDictionary = getTableMetadataFromDb(server, dbname, user, password, projectYear, trustedConnection)
    E = ElementMaker()
    result = []
    ...
    for k,v in tableList.items():
        ...
        global tableCounter
        tableCounter += 1  # this defines how table Id will be presented in metadata
        tableSuffix = k.split('_')[-1] # get last element of string after _ to be table suffix
        splitPos = [ i for i, el in enumerate(tableMetaDictionary[tableSuffix]) if '_' == el][-1]
        metaTableName = tableMetaDictionary[tableSuffix][:splitPos]
        ...
        E.tables(
          E.table( # get tables
            ...
            TableUniverse=""
              ...
              *getVariables(v, variableDescription, metaTableName)
            ,
              GUID = str(uuid.uuid4()),
              ...
              title = tableMetaDictionary[tableSuffix][splitPos+1:],
              titleWrapped =  tableMetaDictionary[tableSuffix][splitPos+1:],
              universe  ='none',
              Visible = 'true',
...
def getGeoIdVariables(geoLevelInfo):
    ...
    nestingInformation = geoLevelInfo 
...
def getGeoIdTables(geoLevelInfo):
    ...
    :return:
    """
    getNestingInformation = geoLevelInfo 
    E = ElementMaker()
    result = []
...
#@profile
def createMetadataXML(connectionString, server, dbname,projectName, projectYear, metadataFileName, geoLevelInfo, projectId, variableDescription, outputDirectory, user, password, trustedConnection):
    ...
    page = E.survey(
      ...
      E.GeoSurveyDataset(
        ...
        E.datasets(
          *getDatasets(connectionString, dbname, geoLevelInfo, projectId, user, password, server, trustedConnection)
        ),
        E.iterations (
        ...
        *getTables(server, dbname, variableDescription, user, password, projectYear, trustedConnection),
          GUID = str(uuid.uuid4()),
          SurveyDatasetTreeNodeExpanded = 'true',
          TablesTreeNodeExpanded = 'true',
          IterationsTreeNodeExpanded = 'false',
        ...
        E.Categories(
            E.string(
                projectName # change into something more appropriate if possible
            )
        ),
          GUID = str(uuid.uuid4()),
          Visible = 'true',
          ...
          name = dbname,#metadataFileName, changed from projectName when meaning of project name changed, changed to dbname because ProjectId is empty in config file when you dont want to get prefix in sql db table or column
...
def prepareEnvironment(configFile):
    ...
    # check if there is propper number of columns in a file, maybe this could be moved to separate function?
    for v in variableDescription.values():
        colNr = len(v)
        if(colNr not in [1,2]):
            print('Number of columns in variable descripton file/s is not ok!')
            sys.exit()

    colNrPrev = [i for i in variableDescription.values()]
    for i in variableDescription.values():
        if (len(i) != len(max(colNrPrev,key=len))):
            print('Number of columns changes in the file!?')
            sys.exit()

    return [connectionString, server,dbname, projectName, projectYear, metadataFileName, geoLevelInfo, projectId, variableDescription, outputDirectory, user, password, trustedConnection]
...
def verifyConfig(configPath):
    ...
    if not os.path.isfile(configPath):
        print("Error: Config file doesn't exist on selected location: "+configPath +" !")
        return False
    ...
    if 'projectId' not in config.keys():
