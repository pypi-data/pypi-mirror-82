...
def getTablesFromDb(server, dbname, projectYear, user, password, trustedConnection):
    ...
    cursor.execute("SELECT name FROM sys.objects WHERE type_desc = 'USER_TABLE' AND name <> 'table_names' and left(name,1) <> '_' and name like '%" + str(projectYear) + "%' AND name <> 'sysdiagrams' ORDER BY modify_date")
...
def getVariables(variables, variableDescription, metaTableName):
    ...
    for i in variables:
        ...
        variableDescForRetrieve = variableDescription[i[0][pos:]]
