...
def create_engine_for_db(db_name):
    ...
    engine = create_engine(
        f'mssql+pyodbc://{user_name}:{password}@PRIME/{db_name}?driver=ODBC+Driver+11+for+SQL+Server',
...
def verify_config_file(config_file_path):
    ...
    if not (os.path.isfile(config['variableDescriptionLocation']) or
            os.path.isdir(config['variableDescriptionLocation'])):
...
def main():
    ...
    logging.basicConfig(
        filename=config['outputDirectory'] +
