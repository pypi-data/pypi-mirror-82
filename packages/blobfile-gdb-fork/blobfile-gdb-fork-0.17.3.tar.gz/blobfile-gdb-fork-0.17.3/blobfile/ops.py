...
class TokenManager:
    ...
    def get_token(self, key: Any, force_expire=False) -> Any:
        with self._lock:
            now = time.time()
            if (
...
def _execute_azure_api_request(req: Request) -> urllib3.HTTPResponse:
    ...
    def build_req() -> Request:
        ...
        r = azure.make_api_request(
            req,
            auth=global_azure_access_token_manager.get_token(key=(account, container), force_expire=cnt==1),
        )
        cnt += 1
        return r
    ...
    for attempt, backoff in enumerate(
        ...
        try:
            ...
            if resp.status in req.success_codes:
                ...
                if resp.status not in req.retry_codes and (attempt == 0 and resp.status != 403):
                    raise err
        except (
