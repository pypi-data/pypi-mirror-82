...
class IU_TreeBasedIndex(Index):
    ...
    def _count_props(self):
        ...
        self.single_node_record_format = (
            self.pointer_format + self.key_format + self.pointer_format
        )
        self.node_format = (
            self.elements_counter_format
            + self.flag_format
            + self.pointer_format
            + (self.key_format + self.pointer_format) * self.node_capacity
        )
        self.leaf_format = (
            self.elements_counter_format
        ...
        self.node_heading_format = self.elements_counter_format + self.flag_format
        ...
        self.elements_counter_size = struct.calcsize("<" + self.elements_counter_format)
        self.pointer_size = struct.calcsize("<" + self.pointer_format)
        self.leaf_heading_size = struct.calcsize("<" + self.leaf_heading_format)
        self.node_heading_size = struct.calcsize("<" + self.node_heading_format)
    ...
    def insert(self, doc_id, key, start, size, status=None):
        ...
        self._insert_new_record_into_leaf(
            nodes_stack.pop(), key, doc_id, start, size, status, nodes_stack, indexes
        )
    ...
    def _read_leaf_nr_of_elements_and_neighbours(self, leaf_start):
        ...
        data = self.buckets.read(self.elements_counter_size + 2 * self.pointer_size)
    ...
    def _read_leaf_nr_of_elements(self, start):
        ...
        nr_of_elements = struct.unpack("<" + self.elements_counter_format, data)
        return nr_of_elements[0]

    def _read_single_node_key(self, node_start, key_index):
        self.buckets.seek(
            self._calculate_key_position(node_start, key_index, self.TYPE_NODE)
        )
            ...
            if curr_key_index < nr_of_elements:
                ...
                if not next_leaf:  # end of leaf linked list
                    ...
                    (
                        ...
                        prev_leaf,
                        next_leaf,
                    ) = self._read_leaf_nr_of_elements_and_neighbours(next_leaf)
                    curr_key_index = 0
    ...
    def _node_linear_key_search(self, key, start, start_index, end_index):
        ...
        data = self.buckets.read(
            (end_index - start_index + 1) * (self.key_size + self.pointer_size)
        )
        curr_key = struct.unpack("<" + self.key_format, data[: self.key_size])[0]
    ...
    def _choose_next_candidate_index_in_leaf(
        self, leaf_start, candidate_start, buffer_start, buffer_end, imin, imax
    ):
        ...
        return (
            self._calculate_key_position(
                leaf_start, (imin + imax) // 2, self.TYPE_LEAF
            ),
            (imin + imax) // 2,
            move_buffer,
        )

    def _choose_next_candidate_index_in_node(
        self, node_start, candidate_start, buffer_start, buffer_end, imin, imax
    ):
        if buffer_start > candidate_start:
            move_buffer = MOVE_BUFFER_PREV
        ...
        return (
            self._calculate_key_position(
                node_start, (imin + imax) // 2, self.TYPE_NODE
            ),
            (imin + imax) // 2,
            move_buffer,
        )
    ...
    def _find_key_in_leaf_for_update(self, key, doc_id, leaf_start, nr_of_elements):
        if nr_of_elements == 1:
            return self._find_key_in_leaf_with_one_element(
                key, leaf_start, doc_id=doc_id
            )
        else:
            return self._find_key_in_leaf_using_binary_search(
                key, leaf_start, nr_of_elements, mode=MODE_FIRST, doc_id=doc_id
            )

    def _find_index_of_first_key_equal_or_smaller_key(
        self, key, leaf_start, nr_of_elements
    ):
        if nr_of_elements == 1:
            return self._find_key_in_leaf_with_one_element(
                key, leaf_start, mode=MODE_FIRST, return_closest=True
            )[:2]
        else:
            return self._find_key_in_leaf_using_binary_search(
                key, leaf_start, nr_of_elements, mode=MODE_FIRST, return_closest=True
            )[:2]

    def _find_index_of_last_key_equal_or_smaller_key(
        self, key, leaf_start, nr_of_elements
    ):
        if nr_of_elements == 1:
            return self._find_key_in_leaf_with_one_element(
                key, leaf_start, mode=MODE_LAST, return_closest=True
            )[:2]
        else:
            return self._find_key_in_leaf_using_binary_search(
                key, leaf_start, nr_of_elements, mode=MODE_LAST, return_closest=True
            )[:2]

    def _find_index_of_first_key_equal(self, key, leaf_start, nr_of_elements):
        if nr_of_elements == 1:
            return self._find_key_in_leaf_with_one_element(
                key, leaf_start, mode=MODE_FIRST
            )[:2]
        else:
            return self._find_key_in_leaf_using_binary_search(
                key, leaf_start, nr_of_elements, mode=MODE_FIRST
            )[:2]

    def _find_key_in_leaf_with_one_element(
        self, key, leaf_start, doc_id=None, mode=None, return_closest=False
    ):
        (
            curr_key,
            curr_doc_id,
            ...
            curr_size,
            curr_status,
        ) = self._read_single_leaf_record(leaf_start, 0)
        if key != curr_key:
    ...
    def _find_key_in_leaf_using_binary_search(
        ...
        while imax != imin and imax > imin:
            (
                ...
                curr_size,
                curr_status,
            ) = self._read_single_leaf_record(leaf_start, candidate_index)
            ...
            if key < curr_key:
                ...
                (
                    ...
                    leaf_start, candidate_start, buffer_start, buffer_end, imin, imax
                )
            elif key == curr_key:
                if mode == MODE_LAST:
        ...
        (
            ...
            curr_size,
            curr_status,
        ) = self._read_single_leaf_record(leaf_start, chosen_key_position)
        if key != curr_key:
        ...
        if doc_id and doc_id == curr_doc_id and curr_status == self.STATUS_O:
            ...
            if (
                mode == MODE_FIRST and imin < chosen_key_position
            ):  # check if there isn't any element with equal key before chosen one
                matching_record_index = self._leaf_linear_key_search(
                    key,
                    self._calculate_key_position(leaf_start, imin, self.TYPE_LEAF),
                    imin,
                    chosen_key_position,
                )
            else:
            ...
            (
                ...
                curr_size,
                curr_status,
            ) = self._read_single_leaf_record(leaf_start, matching_record_index)
            ...
            if doc_id is not None and doc_id != curr_doc_id:
                leaf_start, nr_of_elements, matching_record_index = self._match_doc_id(
                    doc_id, key, matching_record_index, leaf_start, nr_of_elements
                )
                (
                    curr_key,
                    curr_doc_id,
    ...
    def _find_place_in_leaf_with_one_element(self, key, leaf_start):
        ...
        if curr_status == self.STATUS_D:
            return (
                ...
                False,
                True,
            )  # leaf start, index of new key position, nr of rec to rewrite, full_leaf flag, on_deleted flag
        else:
            if key < curr_key:
        ...
        if curr_status == self.STATUS_D:
            ...
            if chosen_key_position > 0:
                (
                    ...
                    curr_size,
                    curr_status,
                ) = self._read_single_leaf_record(leaf_start, chosen_key_position - 1)
    ...
    def _find_key_in_node_with_one_element(self, key, node_start, mode=None):
        l_pointer, curr_key, r_pointer = self._read_single_node_key(node_start, 0)
        if key < curr_key:
            return 0, l_pointer
        elif key > curr_key:
            return 0, r_pointer
    ...
    def _find_key_in_node_using_binary_search(
        self, key, node_start, nr_of_elements, mode=None
    ):
        imin, imax = 0, nr_of_elements - 1
        buffer_start, buffer_end = self._set_buffer_limits()
        (
            ...
            candidate_index,
            move_buffer,
        ) = self._choose_next_candidate_index_in_node(
            node_start,
            self._calculate_key_position(
                node_start, (imin + imax) // 2, self.TYPE_NODE
            ),
        ...
        while imax != imin and imax > imin:
            ...
            if key < curr_key:
                ...
                (
                    ...
                    node_start, candidate_start, buffer_start, buffer_end, imin, imax
                )
            elif key == curr_key:
                if mode == MODE_LAST:
                    if move_buffer == MOVE_BUFFER_NEXT:
        ...
        if (
            mode == MODE_FIRST and imin < chosen_key_position
        ):  # check if there is no elements with equal key before chosen one
    ...
    def _update_leaf_ready_data(
        self, leaf_start, start_index, new_nr_of_elements, records_to_rewrite
    ):
        self.buckets.seek(leaf_start)
        self.buckets.write(struct.pack("<h", new_nr_of_elements))
        start_position = self._calculate_key_position(
            leaf_start, start_index, self.TYPE_LEAF
        )
    ...
    def _update_leaf(
        ...
        if nr_of_records_to_rewrite == 0:  # just write at set position
            self.buckets.seek(
                self._calculate_key_position(
                    leaf_start, new_record_position, self.TYPE_LEAF
                )
            )
            self.buckets.write(
                struct.pack(
            ...
            for status in records_to_rewrite[
                4::5
            ]:  # don't write back deleted records, deleting them from list
            ...
            self.buckets.write(
                struct.pack(
                    "<"
                    + (nr_of_records_to_rewrite + 1) * self.single_leaf_record_format,
                    new_key,
                    new_doc_id,
                    new_start,
                    new_size,
        ...
        if (
            not on_deleted
        ):  # when new record replaced deleted one, nr of leaf elements stays the same
    ...
    def _read_leaf_neighbours(self, leaf_start):
        ...
        prev_l, next_l = struct.unpack("<" + 2 * self.pointer_format, neihbours_data)
    ...
    def _create_new_root_from_leaf(
        ...
        blanks = (
            (self.node_capacity - new_leaf_size)
            * self.single_leaf_record_size
            * b"\x00"
        )
        left_leaf_start_position = self.data_start + self.node_size
        right_leaf_start_position = self.data_start + self.node_size + self.leaf_size
        self.buckets.seek(self.data_start + self.leaf_heading_size)
        # read old root
        data = self.buckets.read(self.single_leaf_record_size * self.node_capacity)
        ...
        data_to_write = self._prepare_new_root_data(
        ...
        if nr_of_records_to_rewrite > half_size:
            ...
            if nr_of_records_to_rewrite:
                records_before = leaf_data[
                    old_leaf_size * 5 : -nr_of_records_to_rewrite * 5
                ]
    ...
    def _split_leaf(
        ...
        if create_new_root:  # leaf is a root
            ...
            self._create_new_root_from_leaf(
                leaf_start,
                nr_of_records_to_rewrite,
                new_leaf_size,
                old_leaf_size,
            ...
            if (
                nr_of_records_to_rewrite > half_size
            ):  # insert key into first half of leaf
                self.buckets.seek(
                    self._calculate_key_position(
                        leaf_start,
                        self.node_capacity - nr_of_records_to_rewrite,
                ...
                self.buckets.seek(
                    self._calculate_key_position(
                        leaf_start, old_leaf_size, self.TYPE_LEAF
                    )
                )
                data = self.buckets.read(
                ...
                records_to_rewrite = struct.unpack(
                    "<" + (new_leaf_size - 1) * self.single_leaf_record_format, data
                )
                # remove deleted records, if succeded abort spliting
                if self._update_if_has_deleted(
                    leaf_start,
                    ...
                    return None
                key_moved_to_parent_node = records_to_rewrite[-(new_leaf_size - 1) * 5]
                ...
                if index_of_records_split:
                    records_before = records_to_rewrite[:-index_of_records_split]
                    records_after = records_to_rewrite[-index_of_records_split:]
                else:
    ...
    def _update_if_has_deleted(
        self, leaf_start, records_to_rewrite, start_position, new_record_data
    ):
        """
    ...
    def _prepare_new_root_data(
        self, root_key, left_pointer, right_pointer, children_flag=None
    ):
        ...
        new_root += (
            (self.key_size + self.pointer_size) * (self.node_capacity - 1) * b"\x00"
        )
        return new_root

    def _create_new_root_from_node(
        self,
        node_start,
        children_flag,
        ...
        data = self.buckets.read(
            self.pointer_size + self.node_capacity * (self.key_size + self.pointer_size)
        )
        ...
        if nr_of_keys_to_rewrite == new_node_size:
            ...
            if nr_of_keys_to_rewrite:
                keys_before = old_node_data[
                    (old_node_size + 1) * 2 : -nr_of_keys_to_rewrite * 2
                ]
        ...
        left_node += (
            (self.node_capacity - old_node_size)
            * (self.key_size + self.pointer_size)
            * b"\x00"
        )
        # adding blanks after new node
        right_node += (
            (self.node_capacity - new_node_size)
            * (self.key_size + self.pointer_size)
            * b"\x00"
        )
    ...
    def _split_node(
        ...
        if create_new_root:
            self._create_new_root_from_node(
                node_start,
                children_flag,
                nr_of_keys_to_rewrite,
                new_node_size,
            ...
            if (
                nr_of_keys_to_rewrite == new_node_size
            ):  # insert key into first half of node
                # reading second half of node
                self.buckets.seek(
                    self._calculate_key_position(
                        node_start, old_node_size, self.TYPE_NODE
                    )
                    + self.pointer_size
                )
                # read all keys with key>new_key
                ...
                old_node_data = struct.unpack(
                    "<"
                    + nr_of_keys_to_rewrite * (self.key_format + self.pointer_format),
                    data,
                )
                ...
                nr_of_keys_to_rewrite > half_size
            ):  # insert key into first half of node
                ...
                key_moved_to_parent_node = old_node_data[-(new_node_size + 1) * 2]
                self.buckets.seek(0, 2)
                new_node_start = self.buckets.tell()
                # prepare new node_data
                new_node = struct.pack(
                ...
                old_node_data = struct.unpack(
                    "<" + new_node_size * (self.key_format + self.pointer_format), data
                )
    ...
    def insert_first_record_into_leaf(
        self, leaf_start, key, doc_id, start, size, status
    ):
        self.buckets.seek(leaf_start)
        ...
        self.buckets.write(
            struct.pack(
                "<" + self.single_leaf_record_format, key, doc_id, start, size, status
            )
        )
    ...
    def _insert_new_record_into_leaf(
        self, leaf_start, key, doc_id, start, size, status, nodes_stack, indexes
    ):
    ...
    def _update_node(
        self, new_key_position, nr_of_keys_to_rewrite, new_key, new_pointer
    ):
    ...
    def _insert_new_key_into_node(
        self, node_start, new_key, old_half_start, new_half_start, nodes_stack, indexes
    ):
        parent_key_index = indexes.pop()
        (
            nr_of_elements,
            children_flag,
        ) = self._read_node_nr_of_elements_and_children_flag(node_start)
        parent_prev_pointer = self._read_single_node_key(node_start, parent_key_index)[
            0
        ]
        if (
            parent_prev_pointer == old_half_start
        ):  # splited child was on the left side of his parent key, must write new key before it
        ...
        if nr_of_elements == self.node_capacity:
            ...
            self._update_node(
                new_key_position, nr_of_keys_to_rewrite, new_key, new_half_start
            )
    ...
    def _find_leaf_to_insert(self, key):
        ...
        if self.root_flag == self.TYPE_LEAF:
            ...
            (
                nr_of_elements,
                curr_child_flag,
            ) = self._read_node_nr_of_elements_and_children_flag(self.data_start)
            ...
            while curr_child_flag == self.TYPE_NODE:
                (
                    nr_of_elements,
                    curr_child_flag,
                ) = self._read_node_nr_of_elements_and_children_flag(curr_pointer)
                curr_index, curr_pointer = self._find_last_key_occurence_in_node(
        ...
        if self.root_flag == self.TYPE_LEAF:
            ...
            while curr_child_flag == self.TYPE_NODE:
                (
                    nr_of_elements,
                    curr_child_flag,
                ) = self._read_node_nr_of_elements_and_children_flag(curr_position)
                curr_position = self._find_last_key_occurence_in_node(
                    curr_position, key, nr_of_elements
                )[1]
    ...
    def _find_key(self, key):
        ...
        (
            ...
            prev_leaf,
            next_leaf,
        ) = self._read_leaf_nr_of_elements_and_neighbours(containing_leaf_start)
        try:
    ...
    def _find_key_to_update(self, key, doc_id):
        ...
        try:
            (
                ...
                size,
                status,
            ) = self._find_key_in_leaf_for_update(
    ...
    def update(self, doc_id, key, u_start=0, u_size=0, u_status=None):
        ...
        (
            ...
            old_size,
            old_status,
        ) = self._find_key_to_update(key, doc_id)
        if u_start:
            old_start = u_start
        if u_size:
            old_size = u_size
    ...
    def delete(self, doc_id, key, start=0, size=0):
        containing_leaf_start, element_index = self._find_key_to_update(key, doc_id)[:2]
    ...
    def _find_key_many(self, key, limit=1, offset=0):
        ...
        (
            ...
            prev_leaf,
            next_leaf,
        ) = self._read_leaf_nr_of_elements_and_neighbours(leaf_with_key)
        try:
            ...
            if key_index >= 0:
                ...
                if prev_leaf:
                    ...
                    (
                        ...
                        prev_leaf,
                        next_leaf,
                    ) = self._read_leaf_nr_of_elements_and_neighbours(prev_leaf)
    ...
    def _find_key_equal_and_smaller(self, key, limit=1, offset=0):
        ...
        try:
            (
                leaf_with_key,
                key_index,
            ) = self._find_index_of_last_key_equal_or_smaller_key(
                key, leaf_with_key, nr_of_elements
            )
            (
                nr_of_elements,
            ...
            leaf_with_key = prev_leaf
            key_index = self._read_leaf_nr_of_elements_and_neighbours(leaf_with_key)[0]
        curr_key = self._read_single_leaf_record(leaf_with_key, key_index)[0]
        if curr_key > key:
            key_index -= 1
        while offset:
    ...
    def _find_key_between(
        self, start, end, limit, offset, inclusive_start, inclusive_end
    ):
        """
        ...
        if inclusive_start:
            ...
            (
                leaf_with_key,
                key_index,
            ) = self._find_index_of_first_key_equal_or_smaller_key(
            ...
            (
                ...
                curr_size,
                curr_status,
            ) = self._read_single_leaf_record(leaf_with_key, key_index)
            if curr_key <= start:
                key_index += 1
        while offset:
            if key_index < nr_of_elements:
    ...
    def get_between(
        self, start, end, limit=1, offset=0, inclusive_start=True, inclusive_end=True
    ):
        if start is None:
            ...
            return self._find_key_between(
                start, end, limit, offset, inclusive_start, inclusive_end
            )
    ...
    def all(self, limit=-1, offset=0):
        ...
        (
            ...
            prev_leaf,
            next_leaf,
        ) = self._read_leaf_nr_of_elements_and_neighbours(leaf_start)
        key_index = 0
        while offset:
            if key_index < nr_of_elements:
                curr_key, doc_id, start, size, status = self._read_single_leaf_record(
