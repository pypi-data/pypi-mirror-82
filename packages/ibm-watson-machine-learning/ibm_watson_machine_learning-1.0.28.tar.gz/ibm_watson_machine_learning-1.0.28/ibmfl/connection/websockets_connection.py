...
class WSConnection(FLConnection):
    ...
    async def PartySendLoop(self):
        ...
        while not self.stopped:
            ...
            try:
                async with websockets.connect(uri, max_size=2 ** 24, ssl=ssl_context_party, extra_headers=headers) as websocket:
                    logger.debug(
    ...
    async def PartyRecvLoop(self):
        ...
        while not self.stopped:
            try:
                async with websockets.connect(uri, close_timeout=100, max_size=2 ** 24, ssl=ssl_context_party2, extra_headers=headers) as websocket:
