...
def setup_nfs_env(wml_credentials, dataset_file_name, remote_directory='datasets'):
    ...
    if res['services_status'] == 'FAILED':
        if started_earlier:
            res = run_ssh(f"""
                    curl -kiv --location --request DELETE '{host}/zen-data/v1/volumes/volume_services/{volume_name}' \
                        --header 'Authorization: Bearer {token}'
                """, format='json')

            raise Exception(f'Volume `{volume_name}` failed to run. As it has been in failed state longer than during this run, the instance was stopped.')
        else:
            raise Exception(f'Volume `{volume_name}` failed to run.')
