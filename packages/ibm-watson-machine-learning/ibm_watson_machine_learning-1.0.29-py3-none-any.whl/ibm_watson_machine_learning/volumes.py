...
class Volume(WMLResource):
    ...
    def list(self):
        ...
        asset_list = asset_details['service_instances']
        volume_values = [
            (m[u'display_name'],
             m[u'id'],
             m['metadata']['existing_pvc_name'],
             m['provision_status']) for m in asset_list]

        self._list(volume_values, [u'NAME', u'ID', u'PVC_NAME', u'PROVISION_STATUS'], None, _DEFAULT_LIST_LENGTH)
    ...
    def delete(self, volume_id):
        ...
        if not self._ICP:
            response = requests.delete(self._href_definitions.volume_service_href(volume_id),
                                       headers=self._client._get_headers(zen=True))
        else:
            response = requests.delete(self._href_definitions.volume_service_href(volume_id),
                                      headers=self._client._get_headers(zen=True), verify=False)
