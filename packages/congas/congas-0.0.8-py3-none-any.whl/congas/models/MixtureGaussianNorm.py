...
class MixtureGaussianNorm(Model):
    ...
    params = {'K': 2, 'batch_size': None, 'cnv_sd' : 2,
              'mixture': None, 'a' : 0. , 'b' : 100., 'init_prec' : 0.1 , 'init_mean' : None}
    data_name = set(['data'])
    ...
    def guide(self,MAP = False,*args, **kwargs):
        if(MAP):
            ...
            def guide_ret(*args, **kwargs):
                ...
                cnv_var = pyro.param("param_cnv_var", lambda: torch.ones(1) * self._params['init_prec'],
                                      constraint=constraints.positive)
    ...
    def init_fn(self):
        def init_function(site):
            ...
            if site["name"] == "norm_prec":
                return self._params['init_prec'] * torch.ones(self._params['K'], self._data['data'].shape[0])
