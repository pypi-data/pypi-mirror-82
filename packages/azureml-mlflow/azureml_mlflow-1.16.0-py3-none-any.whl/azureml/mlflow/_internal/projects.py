...
def _azureml_arg_format(mlflow_command):
    ...
    mlflow_command = [arg for arg in mlflow_command if arg not in invalid_args]
    for arg in range(len(mlflow_command)):
        if ".py" in mlflow_command[arg]:
            script = mlflow_command[arg]
        elif "--" in mlflow_command[arg]:
            if "=" in mlflow_command[arg]:
                parameters = mlflow_command[1:]
                return script, parameters
            else:
                parameters.append(mlflow_command[arg] + " " + mlflow_command[arg + 1])
                mlflow_command[arg + 1] = ""
        elif "--" not in mlflow_command[arg] and mlflow_command[arg] != "":
            parameters.append(mlflow_command[arg])
    return script, parameters
