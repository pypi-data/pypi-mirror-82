...
from common.my_variables import MESSAGE_TEXT, SENDER
...
try:
    ...
    class ClientMainWindow(QMainWindow):
        ...
        def __init__(self, database, transport, keys):
            ...
            # дешифрование сообщений с предзагруженным ключём
            self.decrypter = PKCS1_OAEP.new(keys)

            # кофигурация окна дизайнера
            ...
            # удаление контакта
            self.ui.btn_remove_contact.clicked.connect(
                self.delete_contact_window)
            self.ui.menu_del_contact.triggered.connect(
                self.delete_contact_window)
            ...
            # двойной клик по листу контактов - отправка в обработчик
            self.ui.list_contacts.doubleClicked.connect(
        ...
        def set_disabled_input(self):
            ...
            # кнопки неактивны до выбора получателя
        ...
        def history_list_update(self):
            '''Метод - заполнение истории сообщений'''
            # отсортирована по дате
            list = sorted(
                self.database.get_history(
                    self.current_chat),
                key=lambda item: item[3])
            # создать модель, если она не создана
            if not self.history_model:
                self.history_model = QStandardItemModel()
                self.ui.list_messages.setModel(self.history_model)
            # очистка от старых записей
            self.history_model.clear()
            # взять не более 20 последних записей
            length = len(list)
            start_index = 0
            if length > 20:
                start_index = length - 20

            # заполнение модели записями и разделение вчщдяших и исходящих цветовым фоном
            # выбор их с конца не более 20
        ...
        def select_active_user(self):
            '''Метод - обработка двойного клика по контакту'''
            self.current_chat = self.ui.list_contacts.currentIndex().data()
            self.set_active_user()

        def set_active_user(self):
            '''Метод - установка активного собеседника'''
            # Запрашиваем публичный ключ пользователя и создаём объект
            # шифрования
            try:
            ...
            # Если ключа нет то ошибка, что не удалось начать чат с
            # пользователем
        ...
        def add_contact_action(self, item):
            '''Метод - обработка добавления, сообщает серверу и обновляет таблицу
            и список контактов'''
        ...
        def add_contact(self, new_contact):
            '''Метод - добавление контакта в базы'''
            try:
        ...
        def delete_contact(self, item):
            ...
            try:
                ...
                item.close()
                # если удалён активный пользователь, то деактивируем поля
                # ввода.
        ...
        def send_message(self):
            '''Метод - отправка сообщения пользователю'''
        ...
        def message(self, message):
            ...
            # Декодируем строку, при ошибке выдаём сообщение и завершаем
            # функцию
            try:
        ...
        def connection_lost(self):
            '''Слот потери соединения с ним выдает сообщение об ошибке и завершает
             работу приложения'''
            self.messages.warning(
                self,
                'Сбой соединения',
