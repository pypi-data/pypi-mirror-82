...
class Repo(object):
    ...
    def __exec(self, cmd, cwd=None, timeout=3600, retry=2, ignore_errors=False):
        ...
        for _ in range(retry):
            ...
            logging.info(cmd)
            try:
                noerror, output = util.run_command(cmd, cwd=cwd, stdout=True, timeout=timeout)
                if ignore_errors:
                    return output
    ...
    def get_latest_commit_message(self, repo_dir=None):
        ...
        return self.__exec("git log -n 1", cwd=repo_dir, timeout=1)
    ...
    def get_branch_name(self, repo_dir=None):
        ...
        if not repo_dir:
            repo_dir = self.repo_dir
        return self.__exec("git rev-parse --abbrev-ref HEAD", cwd=repo_dir, timeout=2, ignore_errors=True).replace("\n", "").strip()
    ...
    def get_head_hash(self):
        ...
        return self.__exec("git rev-parse HEAD", timeout=1, ignore_errors=True).replace("\n", "").strip()
