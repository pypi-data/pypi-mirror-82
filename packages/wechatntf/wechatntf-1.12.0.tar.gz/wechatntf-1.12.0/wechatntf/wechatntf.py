...
class wechatntf():
    ...
    def wechatsend(self, summary="", content="", contentType=1):
        ...
        try:
            self.config.read('/etc/wechatntf_config.cfg')  # 读取配置文件信息
            self.data["appToken"] = self.config.get('DEFAULT', 'appToken')
        except Exception as ret:
            raise Exception("Missing '/etc/wechatntf_config.cfg', "
                            "or lacking corresponding configuration information in '/etc/wechatntf_config.cfg'")
