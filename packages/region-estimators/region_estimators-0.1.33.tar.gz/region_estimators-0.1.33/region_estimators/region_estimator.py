...
class RegionEstimator(object):
    ...
    def __init__(self, sensors, regions, actuals):
        ...
        # Check measurement columns have numeric or null data.
        for column in list(actuals.columns):
            if column not in ['timestamp', 'sensor_id']:
                ...
                assert pd.to_numeric(df_temp[column], errors='coerce').notnull().all(), \
                    "actuals['" + column + "'] column contains non-numeric values (null values accepted)."

        # Check that each sensor ID is present in the sensors dataframe index.
        allowed_sensors = sensors.index.values
        print(allowed_sensors)
        #assert  , "Each sensor ID must match a sensor_id in sensors."
