...
def oef_rank_china(info_type='单位净值',fund_type='全部类型'):
    ...
    df1c=df1b[['基金代码','基金简称','单位净值','累计净值','申购状态','赎回状态','手续费']]
    ...
    if info_type == '手续费':
        ...
        dfprint=df[['基金简称','基金代码','基金类型','手续费','单位净值','累计净值']]
        print("\n===== 中国开放式基金排名：手续费最高前十名 =====")          
    ...
    print(dfprint10)
    print("  净值日期:",nvdate,'\b.')
    print("  共找到具有净值信息的开放式基金数量:",len(dfprint),'\b. ',end='')
...
def mmf_rank_china():
    ...
    dfb=dfa[['基金代码','基金简称','7日年化%','成立日期','基金经理','手续费']]
    ...
    #print(dfprint10.to_string(index=False))
    print(dfprint10)
    print("共找到具有收益率信息的货币型基金数量:",len(dfprint))
