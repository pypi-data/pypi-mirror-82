...
from numpy import square,array,zeros
...
class BaseDecisionTree(MultiOutputMixin, BaseEstimator, metaclass=ABCMeta):
    ...
    def lvig_base_impurity(self, x, y, data_choose_bool):
        '''
        ...
        TVI = np.zeros((1,data_choose_bool.shape[1], x.shape[1]))
        for inde in range(data_choose_bool.shape[0]):
        ...
        return TVI
    def lvig_based_impurity_cython_version(self, X, y, grouping, subs_num, check_input = True):
        X = np.array(X)
        y = np.array(y).ravel()
        X = self._validate_X_predict(X, check_input)
    ...
    def lvig_based_accuracy(self, tree, x, y, data_choose_bool):
        '''
        ...
        sse = square(y-self.predict(x))
        # generate a array to contain the results
        tree_feature_importance_array = zeros((1, data_choose_bool.shape[0], x.shape[1]))
        # traverse all subspace data sets
        for each_set in range(data_choose_bool.shape[0]):
            ...
            for feature_k in range(x.shape[1]):
                ...
                spse_sp_set_k = square(chose_y-self..predict(un_permutated_x_copy))
