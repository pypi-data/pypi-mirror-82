...
def git_filter_repo(files):
    cmd_git_filter_repo = subprocess.run(["git", "filter-repo", "--force", "--invert-paths", "--path"] + files)
...
def remove_from_history(items):
    p_items = [Path("data") / item["resource_path"] for item in items]
    git_filter_repo(p_items)

    for item in items:
        write_know_resources(item, status="deleted")
...
def main():
    ...
    if updated_items:
        ...
        remove_from_history(updated_items)
