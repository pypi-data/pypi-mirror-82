...
node_stats_props = \
    {
        ...
        'create_time': 1, 'start_time': 1, "end_time": 1, 
        'queue_duration': 1, 'run_duration': 1, 
...
def get_node_property_dicts():
    ...
    std_cols_desc = {
        ...
        "created": "when the node and job were created",
        "started": "when the controller began running on the node",
        "end_time": "when the controller finished running on the node",
        "queue_duration": "the number of seconds that this node was queued before running",
        "run_duration": "the number of seconds that this node (controller) was running for",
...
def build_node_filter_dict(store, ws_name, node_list, user_to_actual, builder, workspace, args):
    fd = {}
    option_filters = ["status"]   # "application"
    ...
    for name in option_filters:
        job_helper.build_filter_part(fd, args, name, user_to_actual[name])
