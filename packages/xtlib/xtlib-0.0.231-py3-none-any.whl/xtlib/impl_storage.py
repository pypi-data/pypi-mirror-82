...
class ImplStorage(ImplBase):
    ...
    def plot(self, runs, col_list, x, layout, break_on, title, show_legend, plot_titles, legend_titles, 
        ...
        max_workers, x_int, y_int, child, outer, parent, bins, plot_type):
        ...
        # determine the source of our columns
        sd = {}
        for col in col_list:
        ...
        if not sd:
            ...
            errors.general_error("cannot mix plot source types between std/hparam/metrics: {}".format(col_list))
        else:
            col_source = list(sd)[0]

        if col_source == "std":
            # remove "std" prefix from col names
            col_list = [col[4:] for col in col_list]
        ...
        if x is None:
            # provide default column name for x
            if col_source == "metrics":
                x = self.config.get("general", "step-name")

        qfe.set_explicit_options({"parent": False})

        if parent is None and col_source == "metrics":
            parent = False 
        ...
        if col_source == "metrics":
            ...
            # add specified std or hparams.* cols to requested fields
            for col in col_list:
                col_dict[col] = 1
            if x:
                col_dict[x] = 1
        ...
        record_count = len(run_log_records)
        ...
        pb = plot_builder.PlotBuilder(self.store, run_names, col_list, x_col, layout, break_on, title, 
            ...
            legend_suffix, col_source, x_int, y_int, bins, plot_args)
