...
class FilterWidget:
    def __init__(
        ...
        self.group_nonscore_fields = group_nonscore_fields
        self.unstack_fiedls = unstack_fields
    ...
    def do_filter(self, *args):
        ...
        if self.extract_grid_params.value:
            ...
            if self.group_nonscore_fields:
                nonscore_columns = set(self.df.columns) - set(score_names)
                # check the max here
                # ideally, rows should be unique at this point
                groups = self.df.fillna('N/A').groupby(list(nonscore_columns))
