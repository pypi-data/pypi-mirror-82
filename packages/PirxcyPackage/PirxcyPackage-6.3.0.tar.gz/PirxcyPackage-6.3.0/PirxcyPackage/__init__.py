...
async def event_party_member_join(member: fortnitepy.PartyMember) -> None:
    delay.sleep(2)
    await client.party.send(f'Coded By Pirxcy  Made For (data['nickname']) \n\Get Your Bot on discord.gg/wxEBMTZ')

@client.event
async def event_party_member_join(member: fortnitepy.PartyMember) -> None:
    delay.sleep(1)
    await client.party.send(f'There is {party_size} / {party_max_size} in the Party')
