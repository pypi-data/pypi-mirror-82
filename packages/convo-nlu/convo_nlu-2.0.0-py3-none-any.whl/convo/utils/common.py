...
from convo.shared.constants import DEFAULT_LOG_LEVEL, ENV_LOG_LEVEL
...
def read_global_config(path: Text) -> Dict[Text, Any]:
    ...
    try:
        return convo.shared.utils.io.read_config_file(path)
    except Exception:
...
def write_global_config_value(name: Text, value: Any) -> bool:
    ...
    """
    # need to use `convo.constants.GLOBAL_USER_CONFIG_PATH` to allow patching
    # in tests
    config_path = convo.constants.GLOBAL_USER_CONFIG_PATH
    try:
        os.makedirs(os.path.dirname(config_path), exist_ok=True)

        c = read_global_config(config_path)
        c[name] = value
        convo.shared.utils.io.write_yaml(c, convo.constants.GLOBAL_USER_CONFIG_PATH)
        return True
