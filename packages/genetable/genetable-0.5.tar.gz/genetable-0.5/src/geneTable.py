...
def main():
    ...
    # class calling
    feattab = entrez(term = args.string,
                     type = "ft",
                     db   = "nuccore",
                     cache = args.cache).feature_table( keyword = args.qmarkers
                                                          ,cutOff = args.cutoff  )
    if not feattab:
        sys.stderr.write("Empty feature table under these --qmarkers parameters: %s\n" %  ", ".join(args.qmarkers) )
        sys.stderr.flush()
        exit()
    ...
    dictToCSV(dic = feattab, outname = outname, sep = "\t", quiet = args.silent)
