...
class Rccola:
    ...
    def main(self):
        ...
        NLOAD, NLAYER, NDATA1, NDATA2, NSTRNI, NSTRNC, NITT = astuple(self.input.second_row)
        T, B, TC, BC = astuple(self.input.conc_sect_info)
        if TYPES != 3:
            GT, NB1, AS1, NB2, AS2, NBAR, AF = astuple(self.input.reinf_place_info_TYPES12)
        else:
            NL, NBAR, YS, AS, NBS, IBUC, YSS, ASS, NBSS = astuple(self.input.reinf_place_info_TYPES3)
        FYS, YMS, ESH, YSH, ESU, FSU, ESDYN = astuple(self.input.long_reinf_props)
        FYSTRP, ASSTRP, S, XL, XSTRP, ESUSTRP = astuple(self.input.trans_reinf_info)
        ...
        if ICSTRS == 2:
            FCJS, SS, PPX, NSP = astuple(self.input.conc_unconfined_props_ICSTRS2)
        STRN = self.input.strains
        if TYPEC == 3:
            AA, N1, A1, AAc, N1c = astuple(self.input.geometry_conc_crossec_info)

        abc = ['', 'A', 'B', 'C']
        default_norm_axials = np.array([1., 0.8, 0.6, 0.5, 0.4, 0.35, 0.3, 0.25, 0.2, 0.15, 0.1, 0.])
        ...
        self.outtxt.append(" R E I N F O R C E D   C O N C R E T E   C O L U M N   A N A L Y S I S  ")
        self.outtxt.append(" PROGRAM RCCOLA-90    PROPERTIES OF REINFORCED CONCRETE CROSS-SECTIONS")
        self.outtxt.append("")
        self.outtxt.append("")
        self.outtxt.append(" ***********************************************************************")
        ...
        if TYPES == 3:
            ...
            for i in range(0, NL):
                self.outtxt.append(f"{YS[i]:8.3f}{NBS[i]:8}{AS[i]:14.4f}{IBUC[i]}")
        ...
        if ISPALL != 0:
            if ICSTRS == 1:  # User defined
                ...
                if NSP == 0:
                    ...
                    ECS = np.append(ECS, [kentpark71.ε50u, kentpark71.ε50u + 0.00001, 0.05])
                ...
                self.outtxt.append(f"     TIE SPACING    = {S:10.4f} m")
                self.outtxt.append(f"     TIE/CORE RATIO = {PPX:10.6f} vol/vol")
        ...
        self.outtxt.append(f"  Strain at first hoop fracture    :{ECMAND:12.6f}")
        self.outtxt.append(f"  Strain at buckling of long. bars :{ECBUC:12.6f}")
        self.outtxt.append(f"")
        self.outtxt.append(f"  Ultimate conc. strain considered :{ECULT:12.6f}")
        ...
        if TYPEC == 1:  # Rectangular
            ...
            if ITCONF != 1:
                if ISPALL != 1:
                    ...
                    self.outtxt.append(f"      --------CONFINED CORE--------- -------UNCONFINED COVER-------")
                    self.outtxt.append(f" LAYER          DEPTH           AREA          DEPTH           AREA")
                    self.outtxt.append(f"                 (m)           (m**2)          (m)           (m**2)")
                    self.outtxt.append(f"")
                    for i in range(1, NLAYER + 1):
                        self.outtxt.append(f"{i:6}{Y[i]:15.5f}{A[i]:15.5f}{YC[i]:15.5f}{AC[i]:15.5f}")
            else:
                self.outtxt.append(f"      -------INITIAL SECTION-------- -------CONFINED SECTION-------")
                self.outtxt.append(f" LAYER          DEPTH           AREA          DEPTH           AREA")
                self.outtxt.append(f"")
                for i in range(1, NLAYER + 1):
                    self.outtxt.append(f"{i:6}{Y[i]:15.5f}{A[i]:15.5f}{YC[i]:15.5f}{AC[i]:15.5f}")
        ...
        for i in range(1, J + 1):
            ...
            if i > 0:
                if IND != 1:
                    self.outtxt.append(f"{i:3}{STRN[i]:15.5f}{XPA[i]:15.3f}   INITIAL CROSS-SECTION")
                    if abs(PCA[i]) > 0.00001:
                        self.outtxt.append('+' + ' ' * 50 + f"PL.CENTROID ={PCA[i]:10.4f}m")
                    if XPA[i] > XXPMAX:
                        XXPMAX = XPA[i]

                if ITCONF > 0:
                    self.outtxt.append(f"{i:3}{STRN[i]:15.5f}{XPC[i]:15.3f}   CONFINED CROSS-SECTION")
                    if XPC[i] > XXPMAX:
                        XXPMAX = XPC[i]
        ...
        KL = J + 1
        self.outtxt.append(f"{KL:3}{E2:14.5f}{XPA[KJ]:14.3f}   INITIAL CROSS-SECTION")

        if ITCONF > 0:
            JK = NSTRNC + 1
            XPC[JK] = -FYS * AST
            E3 = -FSU * AST
            self.outtxt.append(f"{KL:3}{E2:14.5f}{XPC[JK]:14.3f}   CONFINED CROSS-SECTION")

        KL = KL + 1
        self.outtxt.append(f"{KL:3}{E4:14.5f}{E1:14.3f}   INITIAL CROSS-SECTION")

        if ITCONF > 0:
            self.outtxt.append(f"{KL:3}{E4:14.5f}{E3:14.3f}   CONFINED CROSS-SECTION")
        ...
        self.outtxt.append("")
        self.outtxt.append("        εc        N        M   M/My    1/r    Yield    εs       x     V     M(V)")
        self.outtxt.append("      (m/m)     (MN)    (MN.m)       (rad/m)  Ductl.  (m/m)    (m)   (MN)  (MN.m)")
        ...
        if S != 0.0:
            VI, VMI = SHEAR(VI, VMI, ICOUNT, K, AG, G, TL, AST, XL, XP, F, ASTLI, DI, B, TYPEC, TYPES, ICONF)

        label.x970  # **************************************************** 970
        if ICOUNT < JJ:
            goto.x710

        DUCTI, AREA, DUCMXI = DUCTLE(PHII, XXMI, AREA, DUCTI, DUCMXI, K, 1, NSTRNI, NSTRNC)
        self.outtxt.append('')
        self.outtxt.append('')
        for i in range(1, 4):
        ...
        if S != 0.0:
            VC, VMC = SHEAR(VC, VMC, ICOUNT, K, AGC, G, TL, AST, XL, XP, F, ASTLC, DC, BC, TYPEC, TYPES, ICONF)

        label.x1000  # **************************************************** 1000
        if ICOUNT < JJJ:
            goto.x710
        DUCTC, AREA, DUCMXC = DUCTLE(PHIC, XXMC, AREA, DUCTC, DUCMXC, K, 2, NSTRNI, NSTRNC)
