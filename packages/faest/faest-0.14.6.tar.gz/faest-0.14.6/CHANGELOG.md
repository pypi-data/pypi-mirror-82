...
* Support `faest.get(..., proxies=...)` on top-level request functions. (Pull #1198)
* Display instances with nicer import styles. (Eg. <faest.ReadTimeout ...>) (Pull #1155)
...
* If a transport error occurs while streaming the response, raise an `faest` exception, not the underlying `httpcore` exception. (Pull #1190)
...
* The `faest.URL(...)` class now raises `faest.InvalidURL` on invalid URLs, rather than exposing the underlying `rfc3986` exception. If a redirect response includes an invalid 'Location' header, then a `RemoteProtocolError` exception is raised, which will be associated with the request that caused it. (Pull #1163)
...
* Our HTTP/2 support is now fully optional. **You now need to use `pip install faest[http2]` if you want to include the HTTP/2 dependancies.**
...
* Our exception hierarchy has been overhauled. Most users will want to stick with their existing `faest.HTTPError` usage, but we've got a clearer overall structure now. See https://www.python-faest.org/exceptions/ for more details.
...
* You should now use `faest.codes` consistently instead of `faest.StatusCodes`.
* Usage of `faest.Timeout()` should now always include an explicit default. Eg. `faest.Timeout(None, pool=5.0)`.
* When using `faest.Timeout()`, we now have more concisely named keyword arguments. Eg. `read=5.0`, instead of `read_timeout=5.0`.
* Use `faest.Limits()` instead of `faest.PoolLimits()`, and `limits=...` instead of `pool_limits=...`.
* The `faest.Limits(max_keepalive=...)` argument is now deprecated in favour of a more explicit `faest.Limits(max_keepalive_connections=...)`.
...
One notable aspect of the 0.14.0 release is that it tightens up the public API for `faest`, by ensuring that several internal attributes and methods have now become strictly private.
...
These changes should not affect users who have been working from the `faest` documentation.
...
* Drop `proxies=<transport instance>` in favor of `proxies=faest.Proxy(...)`.

See pull requests #1057, #1058.

### Added

* Added dedicated exception class `faest.HTTPStatusError` for `.raise_for_status()` exceptions. (Pull #1072)
* Added `faest.create_ssl_context()` helper function. (Pull #996)
...
* Use `faest.codes` consistently in favour of `faest.StatusCodes` which is placed into deprecation. (Pull #1088)
* Usage of `faest.Timeout()` should now always include an explicit default. Eg. `faest.Timeout(None, pool=5.0)`. (Pull #1085)
* Switch to more concise `faest.Timeout()` keyword arguments. Eg. `read=5.0`, instead of `read_timeout=5.0`. (Pull #1111)
* Use `faest.Limits()` instead of `faest.PoolLimits()`, and `limits=...` instead of `pool_limits=...`. (Pull #1113)
...
* Add `http2` option to `faest.Client`. (Pull #982)
...
* Include `faest.URLLib3ProxyTransport` in top-level API. (Pull #979)
...
We also now have [a public "Transport API"](https://www.python-faest.org/advanced/#custom-transports), which you can use to implement custom transport implementations against. This formalises and replaces our previously private "Dispatch API".
...
* Logging via HTTPCORE_LOG_LEVEL and FAEST_LOG_LEVEL environment variables
and TRACE level logging. (Pull https://git.sr.ht/~wsmith/httpcore#79)
...
* Reuse of connections on HTTP/2 in close concurrency situations. (Pull https://git.sr.ht/~wsmith/httpcore#81)
* Honor HTTP/2 max concurrent streams settings (Pull https://git.sr.ht/~wsmith/httpcore#89, https://git.sr.ht/~wsmith/httpcore#90)
...
The 0.13.0.dev2 is a *pre-release* version. To install it, use `pip install faest --pre`.
...
The 0.13.0.dev1 is a *pre-release* version. To install it, use `pip install faest --pre`.

### Fixed

* Passing `http2` flag to proxy dispatchers. (Pull #934)
* Use [`httpcore` v0.8.3](https://git.sr.ht/~wsmith/httpcore/releases/tag/0.8.3)
which addresses problems in handling of headers when using proxies.

## 0.13.0.dev0 (April 30th, 2020)

The 0.13.0.dev0 is a *pre-release* version. To install it, use `pip install faest --pre`.
...
The 0.12 release tightens up the API expectations for `faest` by switching to private module names to enforce better clarity around public API.

All imports of `faest` should import from the top-level package only, such as `from faest import Request`, rather than importing from privately namespaced modules such as `from faest._models import Request`.
...
* Fix `faest.Proxy(url, mode="FORWARD_ONLY")` configuration. (Pull #788)
...
The 0.11 release reintroduces our sync support, so that `faest` now supports both a standard thread-concurrency API, and an async API.

Existing async `faest` users that are upgrading to 0.11 should ensure that:

* Async codebases should always use a client instance to make requests, instead of the top-level API.
* The async client is named as `faest.AsyncClient()`, instead of `faest.Client()`.
* When instantiating proxy configurations use the `faest.Proxy()` class, instead of the previous `faest.HTTPProxy()`. This new configuration class works for configuring both sync and async clients.

We believe the API is now pretty much stable, and are aiming for a 1.0 release sometime on or before April 2020.

### Changed

- Top level API such as `faest.get(url, ...)`, `faest.post(url, ...)`, `faest.request(method, url, ...)` becomes synchronous.
- Added `faest.Client()` for synchronous clients, with `faest.AsyncClient` being used for async clients.
- Switched to `proxies=faest.Proxy(...)` for proxy configuration.
- Network connection errors are wrapped in `faest.NetworkError`, rather than exposing lower-level exception types directly.

### Removed

- The `request.url.origin` property and `faest.Origin` class are no longer available.
...
- The `stream` argument has escalated from raising an error when used, to no longer being available. Use the `client.stream(...)` or `faest.stream()` streaming API instead.
...
sure to only import the top-level package eg. `import faest`, rather than
...
- Added `faest.stream()` API. Using `stream=True` now results in a warning. (Pull #600, #610)
...
- The timeout configuration is now named `faest.Timeout(...)`, not `faest.TimeoutConfig(...)`. The old version currently remains as a synonym for backwards compatability.  (Pull #591)
...
- `ASGIDispatch`, `WSGIDispatch` are now available in the `faest.dispatch` namespace. (Pull #407)
- `HTTPError` is now available in the `faest` namespace.  (Pull #421)
...
- Enforce using `faest.AsyncioBackend` for the synchronous client. (Pull #232)
- `faest.ConnectionPool` will properly release a dropped connection. (Pull #230)
- Remove the `raise_app_exceptions` argument from `Client`. (Pull #238)
- `DecodeError` will no longer be raised for an empty body https://git.sr.ht/~wsmithd with Brotli. (Pull #237)
- Added `http_versions` parameter to `Client`. (Pull #250)
- Only use HTTP/1.1 on short-lived connections like `faest.get()`. (Pull #284)
- Convert `Client.cookies` and `Client.headers` when set as a property. (Pull #274)
- Setting `FAEST_DEBUG=1` enables debug logging on all requests. (Pull #277)
