...
def NNCNN9(linear_feature_columns, dnn_feature_columns, 
    ...
    for i in range(1, l + 1):
        ...
        x = tf.keras.layers.Conv2D(filters=f_filters[0], kernel_size=(1, 1), strides=(1, 1), padding='same',
                                             activation='relu', use_bias=True, kernel_initializer='he_normal')(x)
    ...
    x = DNN(dnn_hidden_units, dropout_rate=dnn_dropout,l2_reg=l2_reg_dnn, seed=seed,)(x)
    final_logit = tf.keras.layers.Dense(1, use_bias=False, kernel_initializer='he_normal')(x)
