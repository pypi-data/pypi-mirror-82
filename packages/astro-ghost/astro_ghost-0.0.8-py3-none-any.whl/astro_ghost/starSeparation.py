...
def separateStars_STRM(df, model_path='.', plot=0, verbose=0, starcut='gentle'):
    ...
    #modelName = model_path + "/Star_Galaxy_RealisticModel_GHOST_PS1ClassLabels.sav"
    stream = pkg_resources.resource_stream(__name__, 'Star_Galaxy_RealisticModel_GHOST_PS1ClassLabels.sav')
    if verbose:
        print("Loading model %s."%modelName)
    model = pickle.load(open(stream, 'rb'))
