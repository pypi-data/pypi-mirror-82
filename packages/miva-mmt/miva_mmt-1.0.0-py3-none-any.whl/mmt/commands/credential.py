...
class CredentialAddCommand( Command ):
	...
	def run( self ):
		if self._is_token:
			self.configmanager.credential_add_token( self.args.get( 'key' ), self.args.get( 'url' ), self.args.get( 'token' ), self.args.get( 'signing_key' ) )
			self.configmanager.save()
		elif self._is_ssh:
			self.configmanager.credential_add_ssh( self.args.get( 'key' ), self.args.get( 'url' ), self.args.get( 'ssh_username' ), os.path.abspath( os.path.expanduser( self.args.get( 'ssh_private_key' ) ) ) )
			self.configmanager.save()
...
class CredentialUpdateCommand( Command ):
	...
	def run( self ):
		...
		if credential.is_token():
			self.configmanager.credential_update_token( credential, self.args.get( 'credential_key' ), self.args.get( 'url' ), self.args.get( 'token' ), self.args.get( 'signing_key' ) )
			self.configmanager.save()
		elif credential.is_ssh():
			ssh_private_key = self.args.get( 'ssh_private_key' )
			...
			self.configmanager.credential_update_ssh( credential, self.args.get( 'credential_key' ), self.args.get( 'url' ), self.args.get( 'ssh_username' ), ssh_private_key )
			self.configmanager.save()
