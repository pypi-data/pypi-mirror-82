...
class Command( abc.ABC ):
	...
	def load_remote( self ) -> Remote:
		if self._cached_remote is not None:
			return self._cached_remote

		remote = self.configmanager.remote_lookup( self.configmanager.remote_key )
	...
	def load_credential( self ) -> typing.Union[ CredentialToken, CredentialSSH ]:
		if self._cached_credential is not None:
			return self._cached_credential

		remote		= self.load_remote()
		credential	= self.configmanager.credential_lookup( remote.credential_key )

		if credential is None:
			raise Error( 'MMT-COMMAND-00011', f'Credential key \'{remote.credential_key}\' does not exist' )

		self._cached_credential = credential

		return self._cached_credential

	def build_request_client( self, credential_key: str ) -> typing.Union[ merchantapi.client.Client, merchantapi.client.SSHClient ]:
		...
		if credential is None:
			raise Error( 'MMT-COMMAND-00012', f'Credential key \'{credential_key}\' does not exist' )
	...
	def send_request( self, request: merchantapi.abstract.Request ) -> merchantapi.abstract.Response:
		if request.get_client() is None:
			remote = self.load_remote()
			client = self.build_request_client( remote.credential_key )
			client.set_option( 'default_store_code', remote.store_code )
