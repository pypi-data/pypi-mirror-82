			...
			try:
				...
				for key, value in data.get( 'credentials', {} ).items():
					if value.get( 'token' ) is not None:
						self._credentialmanager.add_token( key, value[ 'url' ], value[ 'token' ], value[ 'signing_key' ] )
					elif value.get( 'username' ) is not None:
						self._credentialmanager.add_ssh( key, value[ 'url' ], value[ 'username' ], value[ 'filepath' ] )
