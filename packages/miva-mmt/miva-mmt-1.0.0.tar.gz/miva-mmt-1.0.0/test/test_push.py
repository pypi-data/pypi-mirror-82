...
class Test( MMTTest ):
	...
	def test_push_external_editor( self ):
		def verify_external_editor_commit_file_contents():
			...
			temp_ouput_file				= tempfile.NamedTemporaryFile( delete = False )
			custom_external_editor_file	= self._create_custom_external_editor( f'cat $1 > {temp_ouput_file.name}' )
			...
			try:
				...
				with open( temp_ouput_file.name ) as fh:
					data = fh.read()
				...
				try:
					...
					os.unlink( temp_ouput_file.name )
				except:
					pass
