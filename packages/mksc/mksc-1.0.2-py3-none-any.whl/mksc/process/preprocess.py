...
def load_data():
    ...
    if bool(cfg.get('configuration', 'engine_url')):
        sql = cfg.get('configuration', 'sql')
        engine = cfg.get('configuration', 'engine_url')
        data = pd.read_sql(sql, engine)
    else:
        # TODO 兼容csv\xls\pickle等格式文件
        # data = read_file(file)
        file = cfg.get('configuration', 'data_file')
