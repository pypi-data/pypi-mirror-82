...
def binning(label, feature):
    ...
    for c in numeric_var:
        binning = pd.concat([feature[c], label], axis=1)
        # 统计空值分布
        binning_data_na = binning[binning[c].isna()]
        ...
        # cart树确定分箱边界
        binning_data = binning.dropna(subset=[c])
        ...
        # 统计分箱结果
        df = pd.merge(pd.cut(binning[c], boundary), binning[label_name], right_index=True, left_index=True)
        df['count'] = 1
