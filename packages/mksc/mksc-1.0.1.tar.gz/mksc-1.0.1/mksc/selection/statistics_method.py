...
def get_distinct_value(feature, threshold=0.9):
    ...
    for c in feature:
        distinct_rate = len(feature[c].drop_duplicates())/feature[c].count()
...
def get_cor_drop(feature, iv_result):
    """
    ...
    cor = cor[(abs(cor) > 0.7) & (cor != 1)].dropna(how='all', axis=1).dropna(how='all')
