class consoleCanvas:
    def __init__(self):
        ...
        self.li=["⠀", "⠁", "⠂", "⠃", "⠄", "⠅", "⠆", "⠇", "⠈", "⠉", "⠊", "⠋", "⠌", "⠍", "⠎", "⠏",
            ...
            "⣠", "⣡", "⣢", "⣣", "⣤", "⣥", "⣦", "⣧", "⣨", "⣩", "⣪", "⣫", "⣬", "⣭", "⣮", "⣯",
            "⣰", "⣱", "⣲", "⣳", "⣴", "⣵", "⣶", "⣷", "⣸", "⣹", "⣺", "⣻", "⣼", "⣽", "⣾", "⣿"]
    def ProduceCanvas(self,Width, height,k=0):
        #生成画布
...
def cvShow(img,dx=2,q=100):
    try:
        import cv2
    except :
        print("[错误] cv2导入失败")
    ret,thresh1=cv2.threshold(cv2.cvtColor(img,cv2.COLOR_RGB2GRAY),q,q,cv2.THRESH_BINARY)
    x, y = img.shape[0:2]
    img_test1 = cv2.resize(thresh1, (int(y / dx), int(x / dx)))
    x1, y1 = img_test1.shape[0:2]
    A=consoleCanvas()
    A.ProduceCanvas(x1,y1)
    for h in  range(y1):
        for w in  range(x1):
            if (img_test1[w][h]):
                A.reviseCanvas([w,h],1)
    A.show() 
