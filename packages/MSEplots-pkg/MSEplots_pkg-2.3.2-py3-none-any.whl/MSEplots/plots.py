...
def msed_plots(pressure,temperature,mixing_ratio,altitude,h0_std=2000,ensemble_size=20,ent_rate=np.arange(0,2,0.05),
    ...
    mse_bp[(mse_p-mse_s.magnitude)<=0] = np.nan 
    mse_nbp[(mse_p-mse_s.magnitude)>0] = np.nan 
    ...
    # Distribution of surface parcel h offsets
        ...
        for h0offset in h0offsets:
            ...
            for eps in entrainment_rates: 
                ...
                level_nonboy = len(np.where(hent[:ELindex_ent].magnitude-mse_s[:ELindex_ent].magnitude<0)[0])
                #print(level_nonboy)
                if (p[ELindex_ent].magnitude < 800) and (level_nonboy > 5):
                    # Plot the non-buoyant entraining parcels
                    #plt.plot( hent[0:ELindex_ent+2], p[0:ELindex_ent+2], linewidth=0.6, color='m')
                    continue
                else:
                    # Plot the curve        
